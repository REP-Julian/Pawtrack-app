import{a as te,d as L,Q as Z,I as Y,s as Be,c as Re}from"./appwrite-BTLpE_3q.js";const P="pawtrack_db",X="pets",K="applications",_e="vet_appointments",Ie="recycle_bin",ct="activity_logs",Te="pawtrack_storage";let E="",Pe="",H="",R={},ae="",h=[],B=[],S=[],_=[],Ne=null,ue=[],C={};document.addEventListener("DOMContentLoaded",async()=>{var lt;try{const e=await te.get();E=e.name,Pe=e.email,H=e.$id,R=e.prefs||{},ae=((lt=e.prefs)==null?void 0:lt.phone)||"";const a=document.getElementById("userNameDisplay");a&&(a.innerText=E?"Welcome, "+E+"!":"Welcome!");try{h=(await L.listDocuments(P,X)).documents.map(n=>({...n,id:n.$id})),B=(await L.listDocuments(P,K,[Z.equal("user_id",H)])).documents.map(n=>({...n,id:n.$id})),S=(await L.listDocuments(P,_e,[Z.equal("user_id",H)])).documents.map(n=>({...n,id:n.$id})),_=(await L.listDocuments(P,Ie,[Z.equal("owner",E)])).documents.map(n=>({...n,id:n.$id}))}catch(t){console.warn("Database collections not fully setup yet. Using empty arrays.",t)}}catch(e){console.error("User not logged in",e),window.location.href="/PawTrackLogin.html";return}async function $(e,a,t){try{await L.createDocument(P,ct,Y.unique(),{user_id:H,action:e||"Activity recorded",target:a||"",icon:t||"fa-paw",timestamp:Date.now().toString()})}catch(s){console.warn("Failed to persist activity log to Appwrite:",s)}}function qe(){document.body.style.zoom="1",document.body.style.width="100%",document.body.style.height="100%",navigator.userAgent.toLowerCase().includes("firefox")&&(document.body.style.transform="none")}let ie;function w(e,a,t=!1,s=null){const r=document.getElementById("customPopupOverlay"),i=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),o=document.getElementById("customPopupActions");i.innerText=e,n.innerText=a,i.className=t?"custom-popup-title title-error":"custom-popup-title title-success",o.style.display="none",r.style.display="flex",clearTimeout(ie),ie=setTimeout(()=>{z(s)},5e3),document.getElementById("customPopupClose").onclick=()=>{clearTimeout(ie),z(s)}}function pt(e,a,t){const s=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),i=document.getElementById("customPopupMessage"),n=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-success",i.innerHTML=`<p style="margin-bottom:10px;">${a}</p><input type="text" id="customPromptInput" style="width:100%; padding:12px; border-radius:8px; border:2px solid #cbd5e1; outline:none; font-size:1rem; font-weight:600; color:#0f172a;" autocomplete="off">`,n.style.display="flex",s.style.display="flex",clearTimeout(ie),document.getElementById("btnPopupConfirm").innerText="Submit",document.getElementById("btnPopupConfirm").onclick=()=>{const o=document.getElementById("customPromptInput").value;z(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",o.trim()!==""&&t(o.trim())},document.getElementById("customPopupClose").onclick=()=>{z()},document.getElementById("btnPopupCancel").onclick=()=>{z(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure"}}function se(e,a,t){const s=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),i=document.getElementById("customPopupMessage"),n=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-error",i.innerHTML=`<p>${a}</p>`,n.style.display="flex",s.style.display="flex",clearTimeout(ie),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",document.getElementById("btnPopupConfirm").onclick=()=>{z(),t&&t()},document.getElementById("customPopupClose").onclick=()=>{z()},document.getElementById("btnPopupCancel").onclick=()=>{z()}}function z(e){document.getElementById("customPopupOverlay").style.display="none",e&&e()}window.addEventListener("resize",qe),qe();const ne=[{initials:"MD",name:"Dr. Miguel Antonio Dela Cruz",specialty:"General Veterinary Practitioner",phone:"0917-555-0101",email:"mdelacruz@pawtrack.ph",schedule:"Mon-Fri: 8am-4pm",clinic:"Quezon City Main Clinic",exp:"15 yrs exp",rating:"4.8"},{initials:"JS",name:"Dr. Joanna Marie R. Santos",specialty:"Veterinary Surgeon",phone:"0917-555-0102",email:"jmsantos@pawtrack.ph",schedule:"Tue-Sat: 10am-6pm",clinic:"Makati Pet Hospital",exp:"12 yrs exp",rating:"4.9"},{initials:"PV",name:"Dr. Paulo C. Villanueva",specialty:"Veterinary Oncologist",phone:"0917-555-0103",email:"pvillanueva@pawtrack.ph",schedule:"Mon-Thu: 9am-5pm",clinic:"BGC Animal Center",exp:"8 yrs exp",rating:"4.7"}];let O=0,V=[],Q=[],U=0,N=null,ee=[],ke=3;const W=document.querySelectorAll(".sidebar-nav .nav-btn"),v=document.getElementById("mainDisplayPanel"),ze=document.getElementById("profileTemplate").innerHTML,Le=document.getElementById("cartItemsContainer"),mt=document.getElementById("cartTotalDisplay"),oe=(e,a,t)=>`
    ...
    `;function re(){let e="";if(typeof h<"u"&&h.length>0){const a=h.filter(t=>t.status==="Available");a.length>0?a.forEach(t=>{const s=t.gender==="Female"?"gender-female":"gender-male",i=t.owner===E?`
        ...
        `}const Ce=()=>{let e="";return typeof _<"u"&&_.length>0?_.forEach(a=>{const t=a.gender==="Female"?"gender-female":"gender-male";e+=`
                    <div class="pet-item-wrapper">
    ...
    `;function $e(){let e="";typeof S<"u"&&S.length>0?S.forEach(i=>{const n=i.status==="Approved",o=n?"appt-approved":"appt-pending",l=n?"badge-approved":"badge-pending",u=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],p=i.date.split("-"),g=p.length===3?u[parseInt(p[1])-1]:"TBD",f=p.length===3?p[2]:"??";e+=`
                    <div class="appt-mini-card ${o}">
                        ...
                        <div class="appt-details">
                            ...
                            <p>${i.vet_name}</p>
                            <span class="badge ${l}">${i.status} • ${i.time}</span>
                        </div>
    ...
    `;function ge(){const e=h.filter(t=>t.owner===E&&t.status==="Private");let a="";return e.length>0?e.forEach(t=>{const s=t.gender==="Female"?"gender-female":"gender-male",r=t.img||"https://ui-avatars.com/api/?name=Pet&background=e2e8f0&color=64748b";a+=`
        ...
        `,Promise.resolve({status:"success",matches:ue}).then(o=>{const l=document.getElementById("homeMatchAlertsContainer");if(l)if(o.status==="success"&&o.matches.length>0){let u="";o.matches.slice(0,3).forEach(p=>{let g=p.status==="approved"?"Approved Match!":"Pending Request",f=p.status==="approved"?"💌":"⏳";u+=`
        ...
        `;const o=n.querySelector(".toast-close");let l=null;const u=p=>{p&&p.stopPropagation(),l&&clearTimeout(l),n.style.animation="toastSlideOut 0.25s forwards",setTimeout(()=>{n.parentElement&&n.parentElement.removeChild(n)},250)};o.addEventListener("click",u),s&&n.addEventListener("click",()=>{u();const p=document.querySelector(`.sidebar-nav .nav-btn[data-target="${s}"]`);if(p)p.click();else if(s==="pets"){W.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="pets"]');g&&g.classList.add("active"),v.innerHTML=re()}else if(s==="applications"){W.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="applications"]');g&&g.classList.add("active"),v.innerHTML=le()}}),i.appendChild(n),l=setTimeout(u,7e3)}function vt(){if(document.getElementById("adoptionApplicationForm"))return"adopt-form";if(document.getElementById("petRegistrationForm")||document.getElementById("registerPetForm"))return"register-pet";if(document.getElementById("profileFullName")||document.querySelector(".profile-container"))return"profile";const e=document.querySelector(".sidebar-nav .nav-btn.active");return e?e.getAttribute("data-target"):"home"}function ce(){const e=vt();e==="home"?de():e==="pets"?v.innerHTML=re():e==="applications"?v.innerHTML=le():e==="profile"?ve():e==="breeding"&&typeof ge=="function"&&!document.getElementById("matchDetailsOverlay")&&(v.innerHTML=ge())}function yt(e){if(!e||!e.payload)return;const a=e.events||[],t=e.payload,s=a.some(n=>n.includes(".create")),r=a.some(n=>n.includes(".update")),i=a.some(n=>n.includes(".delete"));if(t.$collectionId===X||a.some(n=>n.includes(`.${X}.`))){const n=t.$id,o={...t,id:t.$id};if(i){const l=h.find(u=>u.id===n);h=h.filter(u=>u.id!==n),l&&l.status==="Available"&&l.owner!==E&&j("Pet Removed",`${l.name} is no longer listed for adoption.`,"fa-paw","pets","info")}else if(s){const l=h.findIndex(u=>u.id===n);l===-1?h.unshift(o):h[l]=o,o.status==="Available"&&o.owner!==E&&j("New Pet for Adoption! 🐾",`@${o.owner||"A caregiver"} just listed ${o.name} (${o.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}else if(r){const l=h.findIndex(p=>p.id===n),u=l!==-1?h[l]:null;l!==-1?h[l]=o:h.unshift(o),o.owner!==E&&(u&&u.status!=="Available"&&o.status==="Available"?j("Pet Available for Adoption! 🐾",`${o.name} (${o.breed}) is now available for adoption! Tap to view.`,"fa-heart","pets","default"):u&&u.status==="Available"&&o.status!=="Available"&&j("Pet Status Updated",`${o.name} is now ${o.status}.`,"fa-paw","pets","info"))}ce()}if(t.$collectionId===K||a.some(n=>n.includes(`.${K}.`))){const n=t.$id,o={...t,id:t.$id};if(o.user_id===H){if(i)B=B.filter(l=>l.id!==n);else if(s){const l=B.findIndex(u=>u.id===n);l===-1?B.unshift(o):B[l]=o}else if(r){const l=B.findIndex(p=>p.id===n),u=l!==-1?B[l].status:"";if(l!==-1?B[l]=o:B.unshift(o),u!==o.status){const p=o.status==="Approved";j(p?"🎉 Application Approved!":"Application Status Updated",`Your adoption application for ${o.pet_name} is now: ${o.status}!`,p?"fa-circle-check":"fa-clock","applications","app")}}ce()}}}let Ae=!1;async function ht(){if(!Ae){Ae=!0;try{const a=(await L.listDocuments(P,X)).documents.map(i=>({...i,id:i.$id})),t=new Set(h.map(i=>i.id)),s=a.filter(i=>!t.has(i.id)&&i.status==="Available"&&i.owner!==E);if(a.length!==h.length||a.some((i,n)=>{var o,l;return((o=h[n])==null?void 0:o.id)!==i.id||((l=h[n])==null?void 0:l.status)!==i.status})){if(h=a,s.length>0){const i=s[0];j("New Pet for Adoption! 🐾",`@${i.owner||"A caregiver"} just listed ${i.name} (${i.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}ce()}if(H){const n=(await L.listDocuments(P,K,[Z.equal("user_id",H)])).documents.map(l=>({...l,id:l.$id}));(n.length!==B.length||n.some((l,u)=>{var p,g;return((p=B[u])==null?void 0:p.id)!==l.id||((g=B[u])==null?void 0:g.status)!==l.status}))&&(B=n,ce())}}catch{}finally{Ae=!1}}}function bt(){try{if(Re&&typeof Re.subscribe=="function"){const e=`databases.${P}.collections.${X}.documents`,a=`databases.${P}.collections.${K}.documents`;Re.subscribe([e,a],t=>{yt(t)}),console.log("PawTrack: Appwrite Realtime connected for live adoption sync.")}}catch(e){console.warn("Realtime subscription fallback to polling:",e)}setInterval(ht,8e3)}bt();function J(){const e=ne[O],a=document.getElementById("vetInitials");if(!a)return;a.innerText=e.initials,document.getElementById("vetNameDisplay").innerText=e.name,document.getElementById("vetSpecialtyDisplay").innerText=e.specialty,document.getElementById("vetRatingDisplay").innerText=`⭐ ${e.rating} (${e.exp})`,document.getElementById("vetPhoneDisplay").innerText=e.phone,document.getElementById("vetEmailDisplay").innerText=e.email,document.getElementById("vetScheduleDisplay").innerText=e.schedule,document.getElementById("vetClinicDisplay").innerText=e.clinic;const t=document.getElementById("apptVetIdHidden").value,s=document.getElementById("btnSelectVet");t===O.toString()?(s.innerText="✅ Confirmed",s.classList.add("selected")):(s.innerText="Confirm",s.classList.remove("selected"))}function pe(){const e=document.getElementById("cartBadge");e&&(e.innerText=V.length),Le.innerHTML="";let a=0;V.length===0?Le.innerHTML='<p style="text-align:center; color:#64748b; font-weight:bold; margin-top: 50px;">Your cart is empty.</p>':V.forEach((t,s)=>{a+=parseFloat(t.price),Le.innerHTML+=`
                ...
                `}),mt.innerText=`₱ ${a.toFixed(2)}`}window.handleRewind=e=>{ee.splice(e,1),U=Math.max(0,U-1),Se()};function wt(){const e=document.getElementById("rewindList");e&&(e.innerHTML="",ee.length===0?e.innerHTML=`
                ...
                `}))}function Se(){if(Q.length===0||U>=Q.length){document.getElementById("datingContentLayout").style.display="none";const n=document.getElementById("instructionOverlay");n.style.display="flex",n.innerHTML='<i class="fa-solid fa-face-frown-open" style="color: #cbd5e1; font-size: 4rem; margin-bottom: 15px;"></i><p>No more matches available<br>for this pet at the moment.</p>';return}const e=Q[U];currentPhotoIndex=0;const a=document.getElementById("candThumbnails");a.innerHTML="",e.imgs.forEach((n,o)=>{a.innerHTML+=`<img src="${n}" class="cand-thumb ${o===0?"active":""}" data-index="${o}" alt="Photo ${o+1}">`}),We(e,0);const t=e.gender==="Male"?'<i class="fa-solid fa-mars" style="color: #60a5fa;"></i>':'<i class="fa-solid fa-venus" style="color: #f472b6;"></i>';document.getElementById("candNameAge").innerHTML=`${e.name}, ${e.age} ${t}`,document.getElementById("candBreedGender").innerHTML=`<i class="fa-solid fa-paw"></i> ${e.breed}`,document.getElementById("candDesc").innerText=e.desc,document.getElementById("candScoreText").innerText=`${e.score}% Match`,document.getElementById("candScoreBar").style.width=`${e.score}%`;const s=document.getElementById("candBadges");s.innerHTML="",e.badges.includes("vet")&&(s.innerHTML+='<span class="v-badge badge-vet"><i class="fa-solid fa-stethoscope"></i> Vet Verified</span>'),e.badges.includes("pedigree")&&(s.innerHTML+='<span class="v-badge badge-pedigree"><i class="fa-solid fa-dna"></i> Purebred</span>'),e.badges.includes("vax")&&(s.innerHTML+='<span class="v-badge badge-vax"><i class="fa-solid fa-syringe"></i> Vaccinated</span>');const r=document.getElementById("candTraits");r.innerHTML="",e.traits.forEach(n=>{r.innerHTML+=`<span class="trait-tag">${n}</span>`}),document.getElementById("candOwnerName").innerText=e.ownerName,document.getElementById("candOwnerInitial").innerText=e.ownerInitial,document.getElementById("candOwnerRating").innerText=`⭐ ${e.ownerRating}`,document.getElementById("candOwnerPairs").innerText=`${e.successPairs} Successful Pairs`,document.getElementById("barSize").style.width=`${e.compSize}%`,document.getElementById("txtSize").innerText=`${e.compSize}%`,document.getElementById("barEnergy").style.width=`${e.compEnergy}%`,document.getElementById("txtEnergy").innerText=`${e.compEnergy}%`,document.getElementById("barTemp").style.width=`${e.compTemp}%`,document.getElementById("txtTemp").innerText=`${e.compTemp}%`;const i=document.getElementById("litterPredictor");i.innerHTML="",e.litter.forEach(n=>{const o=n.c==="#FFFFFF"||n.c==="#FFF8DC"?"#0f172a":"white";i.innerHTML+=`<div class="color-swatch" style="background:${n.c}; color:${o};">${n.p}%</div>`}),document.getElementById("instructionOverlay").style.display="none",document.getElementById("datingContentLayout").style.display="flex",document.getElementById("treatCountDisplay").innerText=`${ke} Left`,wt()}function We(e,a){const t=document.getElementById("candImg");t.style.opacity=0,setTimeout(()=>{t.src=e.imgs[a],t.style.opacity=1},150),document.querySelectorAll(".cand-thumb").forEach((s,r)=>{r===parseInt(a)?s.classList.add("active"):s.classList.remove("active")})}W.forEach(e=>{e.addEventListener("click",function(){W.forEach(t=>t.classList.remove("active")),this.classList.add("active");const a=this.getAttribute("data-target");v.scrollTo({top:0,behavior:"smooth"}),a==="home"?de():a==="pets"?v.innerHTML=re():a==="applications"?v.innerHTML=le():a==="vet"?(v.innerHTML=$e(),J()):a==="shop"?(v.innerHTML=ut(),pe()):a==="breeding"?(v.innerHTML=ge(),N=null):a==="messages"&&(v.innerHTML=Xe(),Je())})}),document.getElementById("btnViewProfile").addEventListener("click",()=>{W.forEach(e=>e.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=ze,ve()}),document.getElementById("btnRegisterPet").addEventListener("click",()=>{W.forEach(r=>r.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=gt;const e=document.getElementById("imageDropZone"),a=document.getElementById("petImageInput"),t=document.getElementById("imagePreview"),s=document.getElementById("dropZoneText");e.addEventListener("click",()=>a.click()),Ne=null,a.addEventListener("change",function(){if(this.files&&this.files[0]){Ne=this.files[0];const r=new FileReader;r.onload=function(i){t.src=i.target.result,t.style.display="block",s.style.display="none"},r.readAsDataURL(this.files[0])}}),document.getElementById("btnCancelReg").addEventListener("click",()=>{de(),W[0].classList.add("active")})}),v.addEventListener("click",async e=>{if(e.target.closest("#btnMatchRegisterPet")){document.getElementById("btnRegisterPet").click();return}if(e.target.classList.contains("profile-tab-btn")){document.querySelectorAll(".profile-tab-btn").forEach(d=>d.classList.remove("active")),document.querySelectorAll(".profile-tab-content").forEach(d=>d.classList.remove("active")),e.target.classList.add("active");const c=e.target.getAttribute("data-tab");document.getElementById(c).classList.add("active")}const t=e.target.closest(".btn-view-app")||e.target.closest(".btn-view-pet");if(t){const c=t.getAttribute("data-target"),d=document.getElementById(c),m=t.closest(".app-card")||t.closest(".pet-card");d.style.display==="block"?(d.style.display="none",m.classList.remove("dropdown-open")):(d.style.display="block",m.classList.add("dropdown-open"))}const s=e.target.closest(".btn-close-pet");if(s){const c=s.getAttribute("data-target"),d=document.getElementById(c),m=d.previousElementSibling;d.style.display="none",m.classList.remove("dropdown-open")}const r=e.target.closest(".btn-adopt");if(r){const c=r.getAttribute("data-petid"),d=r.getAttribute("data-petname");v.innerHTML=Oe(c,d)}e.target.closest("#btnBackToPets")&&(v.innerHTML=re()),e.target.closest(".btn-edit-profile")&&(document.getElementById("editProfileModal").style.display="flex");const i=e.target.closest(".btn-archive-pet");if(i){const c=i.getAttribute("data-petid");se("Move to Bin?","Are you sure you want to move this pet to the Recycle Bin?",async()=>{const d=h.find(m=>m.id===c);if(!d){w("Error","Pet record not found.",!0);return}try{const m=await L.createDocument(P,Ie,Y.unique(),{name:d.name,breed:d.breed,gender:d.gender,owner:E,img:d.img||""});await L.deleteDocument(P,X,c),h=h.filter(y=>y.id!==c),_.unshift({...m,id:m.$id}),await $("Moved a pet to the Recycle Bin",d.name,"fa-trash-can"),w("Moved to Bin","Pet successfully moved to Recycle Bin.",!1,()=>{ce()})}catch(m){console.error("Appwrite Move to Bin Error:",m),w("Error",m.message,!0)}})}e.target.closest("#btnViewBin")&&(v.innerHTML=Ce()),e.target.closest("#btnEmptyBin")&&se("Empty Bin?","WARNING: Are you sure you want to permanently delete ALL pets in the Recycle Bin? This cannot be undone!",async()=>{try{for(const c of _)await L.deleteDocument(P,Ie,c.id);_=[],await $("Permanently emptied the Recycle Bin","","fa-dumpster-fire"),w("Bin Emptied","Recycle bin emptied successfully.",!1,()=>{v.innerHTML=Ce()})}catch(c){console.error("Appwrite Empty Bin Error:",c),w("Error",c.message,!0)}}),e.target.closest("#btnBackToProfile")&&(v.innerHTML=ze,ve());const n=e.target.closest(".btn-restore-pet");if(n){const c=n.getAttribute("data-petid"),d=_.find(m=>m.id===c);if(d)try{const m=await L.createDocument(P,X,Y.unique(),{name:d.name,breed:d.breed,gender:d.gender,age:"1 yr",status:"Available",health_status:"Healthy",owner:E,contact_number:ae||"0917-000-0000",personal_traits:"Friendly",reason_for_adoption:"Restored from Recycle Bin",img:d.img||""});await L.deleteDocument(P,Ie,c),_=_.filter(y=>y.id!==c),h.unshift({...m,id:m.$id}),await $("Restored a pet from the Recycle Bin",d.name,"fa-rotate-left"),w("Restored!","Pet has been restored to the active board!",!1,()=>{v.innerHTML=Ce()})}catch(m){console.error("Appwrite Restore Error:",m),w("Error",m.message,!0)}}const o=e.target.closest(".btn-cancel-app");if(o){const c=o.getAttribute("data-appid");se("Cancel Application?","Are you sure you want to cancel this application?",async()=>{try{await L.deleteDocument(P,K,c),B=B.filter(d=>d.id!==c),await $("Cancelled adoption application","","fa-file-circle-xmark"),w("Cancelled","Application successfully cancelled.",!1,()=>{v.innerHTML=le()})}catch(d){console.error("Appwrite Cancel App Error:",d),w("Error",d.message,!0)}})}const l=e.target.closest(".btn-cancel-vet");if(l){const c=l.getAttribute("data-appid");se("Cancel Appointment?","Are you sure you want to cancel this veterinary appointment?",async()=>{try{await L.deleteDocument(P,_e,c),S=S.filter(d=>d.id!==c),await $("Cancelled veterinary appointment","","fa-calendar-xmark"),w("Cancelled","Appointment successfully cancelled.",!1,()=>{v.innerHTML=$e(),J()})}catch(d){console.error("Appwrite Cancel Vet Error:",d),w("Error",d.message,!0)}})}const u=e.target.closest(".pet-select-card");if(u&&document.getElementById("apptPetSelectorHidden")){document.querySelectorAll(".pet-select-card").forEach(m=>m.classList.remove("selected")),u.classList.add("selected");const c=u.getAttribute("data-pet");document.getElementById("apptPetSelectorHidden").value=c;const d=myPetsDB[c];d&&(document.getElementById("apptOwner").value=d.owner,document.getElementById("apptPetType").value=d.type,document.getElementById("apptBreed").value=d.breed,document.getElementById("apptGender").value=d.gender,document.getElementById("apptWeight").value=d.weight)}if(e.target.closest("#btnPrevVet")&&(O=(O-1+ne.length)%ne.length,J()),e.target.closest("#btnNextVet")&&(O=(O+1)%ne.length,J()),e.target.closest("#btnSelectVet")){const c=ne[O];document.getElementById("apptVetIdHidden").value=O,document.getElementById("apptSelectedVetName").value=c.name,J()}if(e.target.classList.contains("btn-add-cart")){V.push({name:e.target.getAttribute("data-name"),price:e.target.getAttribute("data-price"),img:e.target.getAttribute("data-img")}),pe();const c=e.target.innerText;e.target.innerText="✓ Added",e.target.style.background="#10b981",e.target.style.color="white",e.target.style.borderColor="#10b981",setTimeout(()=>{e.target.innerText=c,e.target.style.background="white",e.target.style.color="#4f46e5",e.target.style.borderColor="#4f46e5"},1e3)}if(e.target.classList.contains("btn-buy-now")){const d=e.target.closest(".shop-card").querySelector(".btn-add-cart");V.push({name:d.getAttribute("data-name"),price:d.getAttribute("data-price"),img:d.getAttribute("data-img")}),pe(),document.getElementById("cartModalOverlay").style.display="flex"}e.target.closest("#btnOpenCart")&&(document.getElementById("cartModalOverlay").style.display="flex"),e.target.closest(".mode-btn")&&(document.querySelectorAll(".mode-btn").forEach(c=>c.classList.remove("active")),e.target.closest(".mode-btn").classList.add("active"),N&&document.querySelector(`[data-pet="${N}"]`).click());const p=e.target.closest(".pet-select-card");if(p&&document.getElementById("vetBookingForm")){document.querySelectorAll(".pet-select-card").forEach(m=>m.classList.remove("selected")),p.classList.add("selected");const c=p.getAttribute("data-petid");document.getElementById("apptPetSelectorHidden").value=c;const d=h.find(m=>m.id.toString()===c);d&&(document.getElementById("apptOwner").value=E,document.getElementById("apptPetType").value=d.breed&&d.breed.toLowerCase().includes("cat")?"Cat":"Dog",document.getElementById("apptBreed").value=d.breed||"Unknown",document.getElementById("apptGender").value=d.gender||"Unknown")}const g=e.target.closest(".my-pet-card");if(g&&!document.getElementById("apptPetSelectorHidden")){document.querySelectorAll(".my-pet-card").forEach(d=>d.classList.remove("active")),g.classList.add("active"),N=g.getAttribute("data-petid"),h.find(d=>d.id.toString()===N),document.getElementById("datingContentLayout").style.display="none";const c=document.getElementById("instructionOverlay");c.style.display="flex",c.innerHTML='<div class="radar-container"><div class="radar-ring ring1"></div><div class="radar-ring ring2"></div><div class="radar-ring ring3"></div><div class="radar-center"><i class="fa-solid fa-satellite-dish"></i></div></div><p>Scanning database for verified matches...</p>',Promise.resolve().then(()=>({status:"success",candidates:h.filter(m=>m.owner!==E).map(m=>({id:m.id,name:m.name,breed:m.breed,gender:m.gender,age:m.age,photos:[m.img],imgs:[m.img],owner:m.owner||"PawUser",ownerName:m.owner||"PawUser",score:94,traits:m.personal_traits?m.personal_traits.split(",").map(y=>y.trim()):["Playful","Friendly"],desc:m.reason_for_adoption||"Looking for a friend!",verified:!0}))})).then(d=>{d.status==="success"&&d.candidates.length>0?(Q=d.candidates,U=0,setTimeout(()=>{Se()},800)):c.innerHTML='<p style="color:#64748b; font-weight:bold; font-size:1.1rem;">No match candidates available right now. Invite friends or register more pets!</p>'})}const f=e.target.closest(".cand-thumb");if(f){const c=Q[U];We(c,f.getAttribute("data-index"))}const b=e.target.closest("#btnPassCandidate"),I=e.target.closest("#btnMatchCandidate"),T=e.target.closest("#btnTreatCandidate");if(b||I||T){const c=Q[U],d=h.find(y=>y.id.toString()===N);let m="pass";if(I&&(m="like"),T&&(m="super_like"),m==="super_like"&&ke<=0){w("Out of Treats","You have used all your Super Treats for today! Come back tomorrow.",!0);return}Promise.resolve({status:"success",isMatch:!0}).then(y=>{if(y.status==="success"){if(m==="super_like"&&ke--,ee.unshift(c),ee.length>5&&ee.pop(),m!=="pass"){$(m==="super_like"?"Sent a Super Treat to":"Sent a match request to",c.name,"fa-heart"),document.getElementById("matchImgLeft").src=d.img,document.getElementById("matchImgRight").src=c.imgs[0],document.getElementById("matchNameLeft").innerText=d.name,document.getElementById("matchNameRight").innerText=c.name;const A=y.isMatch?"✨ IT'S A MATCH! ✨":m==="super_like"?"Super Liked! 🦴 (Pending)":"Pending Owner Approval";document.getElementById("matchDateSpot").innerText="PawTrack Verified";const F=document.getElementById("matchIcebreaker");F.value=y.isMatch?"You matched! Let's plan a playdate!":"",F.placeholder=y.isMatch?"Type a message...":`Say hi to ${c.ownerName} while you wait...`,document.getElementById("btnContinueMatch").setAttribute("data-targetuser",c.ownerName),document.getElementById("matchOverlay").style.display="flex",activePairs.unshift({id:"p"+Date.now(),maleName:d.gender==="Male"?d.name:c.name,femaleName:d.gender==="Female"?d.name:c.name,maleImg:d.gender==="Male"?d.img:c.imgs[0],femaleImg:d.gender==="Female"?d.img:c.imgs[0],status:A,isApproved:y.isMatch,date:"Just Now",ownerUsername:c.ownerName})}U++,Se()}else w("Error",y.message,!0)}).catch(y=>console.error("Swipe Error:",y))}e.target.closest("#btnOpenPrefs")&&(document.getElementById("prefModal").style.display="flex"),e.target.closest("#btnViewActivePairs")&&fe(),e.target.closest("#btnBackToMatch")&&(v.innerHTML=ge(),N=null);const x=e.target.closest(".btn-accept-match"),k=e.target.closest(".btn-delete-pair");if(x||k){const c=x?"accept":"delete",d=(x||k).getAttribute("data-matchid");if(c==="delete"){const m=k.getAttribute("data-title")||"Cancel Request",y=k.getAttribute("data-msg")||"Are you sure you want to remove this match?";se(m,y,()=>{ue=ue.filter(A=>A.id!==d),fe()})}else{const m=ue.find(y=>y.id===d);m&&(m.status="approved"),fe()}}}),document.getElementById("btnCloseCart").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnContinueShopping").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnCheckout").addEventListener("click",()=>{if(V.length===0){w("Empty Cart","Your cart is empty! Please add some items first.",!0);return}$("Completed a Pet Shop checkout","","fa-bag-shopping"),V=[],pe(),document.getElementById("cartModalOverlay").style.display="none",w("Order Placed!","Order placed successfully! Thank you for shopping.",!1)}),document.getElementById("cartItemsContainer").addEventListener("click",e=>{const a=e.target.closest(".btn-remove-item");a&&(V.splice(parseInt(a.getAttribute("data-index")),1),pe())}),document.getElementById("btnContinueMatch").addEventListener("click",e=>{const a=e.target.getAttribute("data-targetuser"),t=document.getElementById("matchIcebreaker").value.trim();t&&a&&(C[a]||(C[a]=[]),C[a].push({text:t,type:"sent",time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}),$("Sent an Icebreaker message to",a,"fa-comment-dots")),document.getElementById("matchOverlay").style.display="none",fe()}),document.getElementById("btnSavePrefs").addEventListener("click",()=>{document.getElementById("prefModal").style.display="none",N&&document.querySelector(`[data-pet="${N}"]`).click()}),document.getElementById("prefModal").addEventListener("click",e=>{e.target===document.getElementById("prefModal")&&(document.getElementById("prefModal").style.display="none")}),v.addEventListener("submit",e=>{if(e.target.id==="vetBookingForm"){e.preventDefault();const a=document.getElementById("apptPetSelectorHidden").value,t=document.getElementById("apptVetIdHidden").value;if(!a){w("Error","Please select a pet for the appointment by clicking their picture!",!0);return}if(!t){w("Error","Please choose a veterinarian by clicking 'Confirm' on the profile card!",!0);return}const s=document.querySelector(`.pet-select-card[data-petid="${a}"] span`).innerText,r=document.getElementById("apptSelectedVetName").value,i=h.find(n=>{var o;return((o=n.id)==null?void 0:o.toString())===(a==null?void 0:a.toString())});(async()=>{try{const n=await L.createDocument(P,_e,Y.unique(),{pet_name:s,vet_name:r,status:"Upcoming",time:document.getElementById("apptTime").value||"",date:document.getElementById("apptDate").value||"",user_id:H,img:(i==null?void 0:i.img)||""});S.unshift({...n,id:n.$id}),await $("Booked vet visit for",s,"fa-user-doctor"),w("Request Sent!","Appointment scheduled successfully! It is now Upcoming.",!1,()=>{const o=document.querySelector('[data-target="vet"]');o?o.click():(v.innerHTML=$e(),J())})}catch(n){console.error("Appwrite Vet Booking Error:",n),w("Error",n.message,!0)}})()}if(e.target.id==="adoptionApplicationForm"){if(e.preventDefault(),!document.getElementById("adoptTerms").checked){w("Missing Requirement","You must agree to the terms and conditions.",!0);return}const a=document.getElementById("adoptPetId").value,t=document.getElementById("adoptPetName").value,s=h.find(r=>{var i;return((i=r.id)==null?void 0:i.toString())===(a==null?void 0:a.toString())});(async()=>{try{const r=await L.createDocument(P,K,Y.unique(),{pet_name:t,date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),status:"Pending Review",user_id:H,img:(s==null?void 0:s.img)||""});B.unshift({...r,id:r.$id}),await $("Applied to adopt",t,"fa-house-chimney-user"),w("Application Sent!","Successfully submitted! Status: PENDING REVIEW.",!1,()=>{const i=document.querySelector('[data-target="applications"]');i?i.click():v.innerHTML=le()})}catch(r){console.error("Appwrite Adoption Application Error:",r),w("Error",r.message,!0)}})()}(e.target.id==="petRegistrationForm"||e.target.id==="registerPetForm")&&(e.preventDefault(),(async()=>{var l,u,p,g,f;const a=document.getElementById("petImageInput"),t=Ne||a&&a.files&&a.files[0];let s="";if(t)try{w("Uploading Photo","Saving pet photo to PawTrack Storage...",!1);const b=await Be.createFile(Te,Y.unique(),t);s=Be.getFileView(Te,b.$id).toString()}catch(b){console.error("Storage upload failed:",b)}s||(s="https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=600&q=80");const r=document.getElementById("regForAdoption"),i=r?r.checked:!0,n=new FormData(e.target),o={name:n.get("name")||((l=document.getElementById("regPetName"))==null?void 0:l.value)||"Unnamed Pet",breed:n.get("breed")||((u=document.getElementById("regBreed"))==null?void 0:u.value)||"Mixed Breed",gender:n.get("gender")||((p=document.getElementById("regGender"))==null?void 0:p.value)||"Male",age:n.get("age")||((g=document.getElementById("regAge"))==null?void 0:g.value)||"1 yr",status:i?"Available":"Private",health_status:n.get("health_status")||"Healthy / Vaccinated",owner:E,contact_number:n.get("contact_number")||ae||"0917-000-0000",personal_traits:n.get("personal_traits")||((f=document.getElementById("regDesc"))==null?void 0:f.value)||"Friendly",reason_for_adoption:n.get("reason_for_adoption")||(i?"Looking for a home":"Personal pet"),img:s};try{const b=await L.createDocument(P,X,Y.unique(),o),I={...b,id:b.$id};h.unshift(I),await $(i?"Listed pet for adoption":"Registered private pet",o.name,"fa-shield-cat"),w("Success!",i?"Pet Registered Successfully! Photo saved to PawTrack Storage and listed on the adoption board.":"Pet added to your personal roster! Photo saved to PawTrack Storage.",!1,()=>{if(i){const x=document.querySelector('[data-target="pets"]');x?x.click():v.innerHTML=re()}else{const x=document.querySelector('[data-target="home"]');x?x.click():de()}})}catch(b){console.error("Appwrite Pet Registration Error:",b),w("Error","Could not register pet: "+b.message,!0)}})())}),v.addEventListener("reset",e=>{e.target.id==="vetBookingForm"&&(document.querySelectorAll(".pet-select-card").forEach(a=>a.classList.remove("selected")),document.getElementById("apptPetSelectorHidden")&&(document.getElementById("apptPetSelectorHidden").value=""),document.getElementById("apptVetIdHidden")&&(document.getElementById("apptVetIdHidden").value=""),setTimeout(()=>{J()},10))});const je=document.getElementById("editProfileForm");je&&je.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("editFirstName").value,t=document.getElementById("editLastName").value,s=document.getElementById("editContact").value,r=`${a} ${t}`.trim();try{r&&await te.updateName(r),await te.updatePrefs({...R,phone:s}),E=r,ae=s;const i=document.getElementById("userNameDisplay");i&&(i.innerText=E?"Welcome, "+E+"!":"Welcome!"),await $("Updated profile settings & information","","fa-user-pen"),document.getElementById("editProfileModal").style.display="none",w("Profile Saved","Profile updated successfully!",!1,()=>{ve()})}catch(i){w("Error",i.message,!0)}}),v.addEventListener("input",e=>{e.target.id==="shopPriceFilter"&&(document.getElementById("shopPriceDisplay").innerText=`₱${e.target.value}`,Ge())}),v.addEventListener("change",e=>{e.target.id==="shopCategoryFilter"&&Ge()});function Ge(){const e=document.getElementById("shopCategoryFilter").value,a=parseFloat(document.getElementById("shopPriceFilter").value),t=Ve.filter(s=>{const r=e==="All Categories"||s.category===e,i=s.price<=a;return r&&i});document.getElementById("shopGridContainer").innerHTML=Ue(t)}async function ve(){const e=document.getElementById("mainProfilePic");e&&(e.src=R.avatarUrl||"/resources/avatar/Avatar 1.jpg");const a=document.getElementById("btnChangeAvatar")||document.querySelector(".btn-change-photo"),t=document.getElementById("avatarFileInput");a&&t&&(a.onclick=()=>t.click(),t.onchange=async function(){if(this.files&&this.files[0])try{w("Uploading","Uploading avatar to PawTrack Storage...",!1);const y=await Be.createFile(Te,Y.unique(),this.files[0]),A=Be.getFileView(Te,y.$id).toString();await te.updatePrefs({...R,avatarUrl:A}),R.avatarUrl=A,e&&(e.src=A),await $("Updated profile picture","Saved to PawTrack Storage","fa-camera"),w("Success","Profile photo uploaded to PawTrack Storage!")}catch(y){w("Error","Failed to upload photo: "+y.message,!0)}});const s=document.getElementById("profileFullName"),r=document.getElementById("profileUsername"),i=document.getElementById("profileEmail"),n=document.getElementById("profilePhone");s&&(s.innerText=E),r&&(r.innerText=R.username?"@"+R.username:"@"+Pe.split("@")[0]),i&&(i.innerText=Pe),n&&(n.innerText=ae||"None");const[o="",...l]=(E||"").split(" "),u=l.join(" "),p=document.getElementById("editFirstName"),g=document.getElementById("editLastName"),f=document.getElementById("editContact"),b=document.getElementById("editEmail");p&&(p.value=o),g&&(g.value=u),f&&(f.value=ae||""),b&&(b.value=Pe||"");const I=h.filter(y=>y.owner===E),T=B.filter(y=>y.status==="Approved").length,x=document.getElementById("countOwnedPets"),k=document.getElementById("countSuccessfulApps");x&&(x.innerText=I.length),k&&(k.innerText=T);const c=document.getElementById("privateRosterGrid"),d=document.getElementById("adoptionRosterGrid");c&&d&&(c.innerHTML="",d.innerHTML="",I.forEach(y=>{const A=`
                ...
                `;y.status==="Private"?c.innerHTML+=A:d.innerHTML+=A}));const m=document.getElementById("recentActivityLogs");if(m){m.innerHTML='<div class="activity-loading"><i class="fa-solid fa-circle-notch fa-spin"></i> Loading activity logs...</div>';try{const y=await L.listDocuments(P,ct,[Z.equal("user_id",H),Z.orderDesc("$createdAt"),Z.limit(20)]);m.innerHTML="";const A=y.documents;A.length>0?A.forEach(F=>{const kt=parseInt(F.timestamp)||new Date(F.$createdAt).getTime(),Lt=Et(kt),Ee=xt(F.action,F.icon),dt=F.target?M(F.target):"",Ct=M(F.action||"Activity recorded");m.innerHTML+=`
                            <div class="activity-item">
                                <div class="activity-icon-bubble ${Ee.themeClass}">
                                    <i class="fa-solid ${Ee.icon}"></i>
                                </div>
                                <div class="activity-details">
                                    <div class="activity-row-main">
                                        <span class="activity-action-text">${Ct}</span>
                                        <span class="activity-category-pill ${Ee.tagClass}">${Ee.tagLabel}</span>
                                    </div>
                                    ${dt?`
                                    ...
                                    <div class="activity-meta">
                                        <span class="activity-timestamp"><i class="fa-regular fa-clock"></i> ${Lt}</span>
                ...
                `}}}function M(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function xt(e,a){const t=(e||"").toLowerCase();if(t.includes("log")||t.includes("auth")||t.includes("verified")||t.includes("session"))return{themeClass:"theme-emerald",icon:"fa-shield-halved",tagClass:"tag-emerald",tagLabel:"Security"};if(t.includes("bin")||t.includes("delet")||t.includes("dumpster")||t.includes("trash"))return{themeClass:"theme-rose",icon:"fa-trash-can",tagClass:"tag-rose",tagLabel:"Archive"};if(t.includes("pet")||t.includes("adopt")||t.includes("restor")||t.includes("breed"))return{themeClass:"theme-terracotta",icon:"fa-paw",tagClass:"tag-terracotta",tagLabel:"Pet Care"};if(t.includes("photo")||t.includes("avatar")||t.includes("profile")||t.includes("settings"))return{themeClass:"theme-honey",icon:"fa-user-pen",tagClass:"tag-honey",tagLabel:"Profile"};if(t.includes("shop")||t.includes("cart")||t.includes("item")||t.includes("buy"))return{themeClass:"theme-amber",icon:"fa-bag-shopping",tagClass:"tag-amber",tagLabel:"Shop"};let s=a||"fa-bell";return s==="fa-right-to-bracket"&&(s="fa-arrow-right-to-bracket"),{themeClass:"theme-sage",icon:s,tagClass:"tag-sage",tagLabel:"Activity"}}function Et(e){const a=Date.now(),t=Math.max(0,a-e),s=60*1e3,r=60*s,i=24*r,n=new Date(e),o=n.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});return t<2*s?"Just now":t<60*s?`${Math.floor(t/s)}m ago`:t<24*r?`${Math.floor(t/r)}h ago (${o})`:new Date(a-i).toDateString()===n.toDateString()?`Yesterday at ${o}`:n.toLocaleDateString(void 0,{month:"short",day:"numeric"})+` at ${o}`}const Ye=document.getElementById("btnLogout");Ye&&Ye.addEventListener("click",async()=>{try{await te.deleteSession("current")}catch(e){console.warn(e)}window.location.href="/PawTrackLogin.html"});let G=null;Object.keys(C).length===0&&(C.PawTrackCommunity=[{text:"Welcome to PawTrack! Connect with pet lovers, adopters, and arrange playdates right here.",type:"received",time:"10:00 AM"}]);function Xe(){return`
        ...
        `}function Je(e=null){const a=document.getElementById("messengerInboxList"),t=document.getElementById("messengerChatPanel"),s=document.getElementById("btnPageNewChat"),r=document.getElementById("messengerSearchInput");if(!a||!t)return;function i(p=""){a.innerHTML="";const g=Object.keys(C).filter(f=>f.toLowerCase().includes(p.toLowerCase()));if(g.length===0){a.innerHTML=`
                    <div class="inbox-empty-state">
                        <i class="fa-solid fa-comments"></i>
                        <p>No conversations found.<br>Click "New Conversation" to start chatting!</p>
                    </div>
                `;return}g.forEach(f=>{const b=C[f]||[],I=b.length>0?b[b.length-1].text:"No messages yet",T=b.length>0?b[b.length-1].time:"",x=f===G?"active":"",k=document.createElement("div");k.className=`inbox-item ${x}`,k.setAttribute("data-user",f),k.innerHTML=`
                ...
                `,k.onclick=()=>n(f),a.appendChild(k)})}function n(p){G=p,document.querySelectorAll(".inbox-item").forEach(x=>{x.classList.toggle("active",x.getAttribute("data-user")===p)});const g=C[p]||[];t.innerHTML=`
            ...
            `;const f=document.getElementById("activeChatStream");g.forEach(x=>{o(f,x.text,x.type,x.time)}),f.scrollTop=f.scrollHeight;const b=document.getElementById("activeChatInput"),I=document.getElementById("btnActiveSend");function T(){const x=b.value.trim();if(!x||!G)return;const k=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});o(f,x,"sent",k),b.value="",f.scrollTop=f.scrollHeight,C[G]||(C[G]=[]),C[G].push({text:x,type:"sent",time:k}),i(r.value.trim()),G==="PawTrackCommunity"&&setTimeout(()=>{const c=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),d="We're glad to have you! Feel free to connect with pet owners across PawTrack.";C.PawTrackCommunity.push({text:d,type:"received",time:c}),G==="PawTrackCommunity"&&(o(f,d,"received",c),f.scrollTop=f.scrollHeight),i(r.value.trim())},1e3)}I.onclick=T,b.onkeypress=x=>{x.key==="Enter"&&T()},b.focus()}function o(p,g,f,b){const I=document.createElement("div");I.className=`stream-bubble bubble-${f}`,I.innerHTML=`
                <div class="bubble-text">${M(g)}</div>
                <div class="bubble-time">${b}</div>
            `,p.appendChild(I)}r.oninput=p=>{i(p.target.value.trim())},s.onclick=()=>{pt("Start Conversation","Enter the PawTrack username you'd like to message:",p=>{if(p&&p.trim()){const g=p.trim().replace("@","");C[g]||(C[g]=[]),i(),n(g)}})};const l=Object.keys(C),u=e||(l.length>0?l[0]:null);i(),u?n(u):t.innerHTML=`
            ...
            `}v.addEventListener("click",e=>{const a=e.target.closest(".btn-message-owner");if(a){const t=a.getAttribute("data-owner"),s=document.querySelector('.sidebar-nav .nav-btn[data-target="messages"]');s&&(W.forEach(r=>r.classList.remove("active")),s.classList.add("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=Xe(),Je(t)}}),v.addEventListener("input",e=>{if(e.target.id==="radarDistanceRange"){const a=document.getElementById("radarDistanceValue");a&&(a.innerText=`${e.target.value} km`)}}),v.addEventListener("click",async e=>{var a,t,s,r,i;if(e.target.closest("#btnSaveAllSettings")){const n=((a=document.getElementById("notifVet"))==null?void 0:a.checked)??!0,o=((t=document.getElementById("notifMatch"))==null?void 0:t.checked)??!0,l=((s=document.getElementById("notifAdoption"))==null?void 0:s.checked)??!0,u=((r=document.getElementById("prefIncognito"))==null?void 0:r.checked)??!1,p=((i=document.getElementById("radarDistanceRange"))==null?void 0:i.value)||"25";try{await te.updatePrefs({...R,notifVet:n,notifMatch:o,notifAdoption:l,incognito:u,radarDistance:p}),R.radarDistance=p,R.incognito=u,await $("Updated notification & discovery preferences","","fa-sliders"),w("Settings Saved","Your notification, privacy, and discovery preferences have been updated successfully!")}catch(g){w("Error",g.message,!0)}}});const D=[{title:"Your Care Dashboard 🏠",desc:"Welcome to PawTrack! From your Home dashboard, track registered pet counts, see active adoption statuses, review upcoming vet reminders, and get daily outdoor walk weather tips.",icon:"fa-house",targetSelector:".sidebar-nav [data-target='home']",tab:"home",placement:"right"},{title:"Live Adoption Board 🐾",desc:"Browse companions looking for a loving home! When other users register a pet for adoption, it shows up here in real-time without reloading. Click 'Details' to view medical notes, or 'Adopt Now' to send a digital application.",icon:"fa-paw",targetSelector:".sidebar-nav [data-target='pets']",tab:"pets",placement:"right"},{title:"Register Your Own Pet 📸",desc:"Add your furry family member with photo uploads to PawTrack Cloud Storage. Choose whether your companion is a private pet for health tracking or listed publicly for adoption.",icon:"fa-plus-circle",targetSelector:"#btnRegisterPet",tab:null,placement:"bottom"},{title:"Pet Matchmaker & Playdates ❤️",desc:"Find compatible playmates or breeding matches based on breed, age, and location. Give a Treat to express interest, or pass to browse more verified neighborhood companions.",icon:"fa-heart",targetSelector:".sidebar-nav [data-target='breeding']",tab:"breeding",placement:"right"},{title:"Verified Vet Appointments 🩺",desc:"Book check-ups and medical visits with licensed veterinarians. Select your preferred clinic and time slot, and manage upcoming appointments in one place.",icon:"fa-user-doctor",targetSelector:".sidebar-nav [data-target='vet']",tab:"vet",placement:"right"},{title:"Real-Time Messages & Alerts 💬",desc:"Chat directly with pet owners, caregivers, and adopters. Real-time notification toasts keep you updated the moment an application is approved or a new pet is posted!",icon:"fa-comment-dots",targetSelector:"#navMessagesBtn",tab:"messages",placement:"right"}];let q=0,ye=!1,me=null;function Me(e){me&&me!==e&&me.classList.remove("tour-target-elevated"),e?(e.classList.add("tour-target-elevated"),me=e):me=null}function he(e){const a=document.getElementById("pawtrackTourOverlay"),t=document.getElementById("tourSpotlightBox"),s=document.getElementById("tourCard");if(!a||!t||!s)return;let r=e.targetSelector?document.querySelector(e.targetSelector):null;if(!r||r.offsetParent===null){t.style.display="none",a.classList.add("no-target"),Me(null),s.style.top="50%",s.style.left="50%",s.style.transform="translate(-50%, -50%)";return}a.classList.remove("no-target"),Me(r),t.style.display="block";const i=r.getBoundingClientRect(),n=6,o=Math.max(0,i.top-n),l=Math.max(0,i.left-n),u=i.width+n*2,p=i.height+n*2;t.style.top=`${o}px`,t.style.left=`${l}px`,t.style.width=`${u}px`,t.style.height=`${p}px`;try{const x=window.getComputedStyle(r),k=parseFloat(x.borderRadius)||12;t.style.borderRadius=`${Math.max(10,k+4)}px`}catch{t.style.borderRadius="14px"}if(window.innerWidth<=768){s.style.top="",s.style.left="",s.style.transform="";return}s.style.transform="none";const g=390,f=s.offsetHeight||220,b=16;let I,T;e.placement==="right"?(T=i.right+b,I=Math.max(20,i.top+i.height/2-f/2),T+g>window.innerWidth-20&&(T=Math.max(20,i.left-g-b))):e.placement==="bottom"?(I=i.bottom+b,T=i.left+i.width/2-g/2,T+g>window.innerWidth-20&&(T=window.innerWidth-g-24),T<20&&(T=20),I+f>window.innerHeight-20&&(I=Math.max(20,i.top-f-b))):(I=Math.max(20,window.innerHeight/2-f/2),T=Math.max(20,window.innerWidth/2-g/2)),I=Math.min(Math.max(20,I),window.innerHeight-f-20),T=Math.min(Math.max(20,T),window.innerWidth-g-20),s.style.top=`${I}px`,s.style.left=`${T}px`}function De(e){if(e<0||e>=D.length){be();return}q=e;const a=D[e];if(a.tab){const u=document.querySelector(`.sidebar-nav [data-target="${a.tab}"]`);u&&!u.classList.contains("active")&&u.click()}const t=document.getElementById("tourStepBadge"),s=document.getElementById("tourStepTitle"),r=document.getElementById("tourStepDesc"),i=document.getElementById("tourIconBubble"),n=document.getElementById("tourDots"),o=document.getElementById("tourBtnPrev"),l=document.getElementById("tourBtnNext");t&&(t.innerText=`STEP ${e+1} OF ${D.length}`),s&&(s.innerText=a.title),r&&(r.innerText=a.desc),i&&(i.innerHTML=`<i class="fa-solid ${a.icon}"></i>`),n&&(n.innerHTML=D.map((u,p)=>`<div class="tour-dot ${p===e?"active":""}"></div>`).join("")),o&&(o.style.display=e>0?"inline-block":"none"),l&&(e===D.length-1?l.innerHTML='Finish Tour <i class="fa-solid fa-check"></i>':l.innerHTML='Next <i class="fa-solid fa-arrow-right"></i>'),setTimeout(()=>{he(a)},60),setTimeout(()=>{he(a)},220)}function Fe(e=0){const a=document.getElementById("tourWelcomeModal");a&&(a.style.display="none");const t=document.getElementById("howItWorksModal");t&&(t.style.display="none");const s=document.getElementById("pawtrackTourOverlay");s&&(s.style.display="block",ye=!0,De(e))}function be(e=!0){const a=document.getElementById("pawtrackTourOverlay");if(a&&(a.style.display="none"),ye=!1,Me(null),e)try{localStorage.setItem("pawtrack_tour_completed","true")}catch{}}function Bt(){q<D.length-1?De(q+1):(be(!0),j("Tour Complete! 🌟","You're all set! Click 'Guide & Tour' anytime at the top of your screen to replay.","fa-circle-check",null,"app"))}function It(){q>0&&De(q-1)}function we(){const e=document.getElementById("howItWorksModal");e&&(e.style.display="none")}function Tt(){try{localStorage.getItem("pawtrack_tour_completed")||setTimeout(()=>{const a=document.getElementById("tourWelcomeModal"),t=document.getElementById("welcomeTourHeading");t&&E&&(t.innerText=`Welcome, ${E}! 👋`),a&&(a.style.display="flex")},1400)}catch{}}const Ze=document.getElementById("btnStartTour");Ze&&Ze.addEventListener("click",()=>{Fe(0)});const Ke=document.getElementById("btnSidebarAiAssistant");Ke&&Ke.addEventListener("click",()=>{j("AI Assistant 🤖","This feature is still under development. Stay tuned for our AI-powered assistant!","fa-robot",null,"info")});const Qe=document.getElementById("btnWelcomeTourStart");Qe&&Qe.addEventListener("click",()=>{Fe(0)});const et=document.getElementById("btnWelcomeTourSkip");et&&et.addEventListener("click",()=>{const e=document.getElementById("tourWelcomeModal");e&&(e.style.display="none");try{localStorage.setItem("pawtrack_tour_completed","true")}catch{}});const tt=document.getElementById("tourBtnClose");tt&&tt.addEventListener("click",()=>be(!0));const at=document.getElementById("tourBtnSkip");at&&at.addEventListener("click",()=>be(!0));const it=document.getElementById("tourBtnNext");it&&it.addEventListener("click",Bt);const st=document.getElementById("tourBtnPrev");st&&st.addEventListener("click",It);const nt=document.getElementById("btnHiwClose");nt&&nt.addEventListener("click",we);const ot=document.getElementById("btnHiwDismiss");ot&&ot.addEventListener("click",we);const rt=document.getElementById("btnHiwStartInteractiveTour");rt&&rt.addEventListener("click",()=>{we(),Fe(0)});const He=document.getElementById("howItWorksModal");He&&He.addEventListener("click",e=>{e.target===He&&we()});const xe=document.getElementById("tourWelcomeModal");xe&&xe.addEventListener("click",e=>{e.target===xe&&(xe.style.display="none")}),document.querySelectorAll(".hiw-tab-btn").forEach(e=>{e.addEventListener("click",function(){document.querySelectorAll(".hiw-tab-btn").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".hiw-tab-content").forEach(s=>s.classList.remove("active")),this.classList.add("active");const a=this.getAttribute("data-tab"),t=document.getElementById(a);t&&t.classList.add("active")})}),window.addEventListener("resize",()=>{ye&&D[q]&&he(D[q])}),window.addEventListener("scroll",()=>{ye&&D[q]&&he(D[q])},!0);function Pt(){const e=document.querySelector(".sidebar"),a=document.getElementById("sidebarTogglePill"),t=document.getElementById("btnHeaderSidebarToggle"),s=document.getElementById("sidebarLogoWrapper");if(!e)return;localStorage.getItem("pawtrack_sidebar_collapsed")==="true"&&window.innerWidth>768&&(e.classList.add("collapsed"),a&&(a.title="Maximize sidebar",a.setAttribute("aria-label","Maximize sidebar")),t&&(t.title="Maximize sidebar"));function i(n=null){if(window.innerWidth<=768)return;const o=n!==null?n:!e.classList.contains("collapsed");e.classList.toggle("collapsed",o);try{localStorage.setItem("pawtrack_sidebar_collapsed",o?"true":"false")}catch{}a&&(a.title=o?"Maximize sidebar":"Minimize sidebar",a.setAttribute("aria-label",o?"Maximize sidebar":"Minimize sidebar")),t&&(t.title=o?"Maximize sidebar":"Minimize sidebar"),setTimeout(()=>{window.dispatchEvent(new Event("resize"))},310)}a&&a.addEventListener("click",n=>{n.stopPropagation(),i()}),t&&t.addEventListener("click",n=>{n.stopPropagation(),i()}),s&&s.addEventListener("click",()=>{e.classList.contains("collapsed")&&i(!1)})}Pt(),Tt()});
