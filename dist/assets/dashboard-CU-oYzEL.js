import{a as ie,d as C,Q,I as X,s as ke,c as Ne}from"./appwrite-BTLpE_3q.js";const P="pawtrack_db",J="pets",ee="applications",qe="vet_appointments",Pe="recycle_bin",ut="activity_logs",Le="pawtrack_storage";let T="",ze="",A="",L={},se="",y=[],x=[],D=[],R=[],Oe=null,ve=[],S={};document.addEventListener("DOMContentLoaded",async()=>{var pt;try{const e=await ie.get();T=e.name,ze=e.email,A=e.$id,L=e.prefs||{},se=((pt=e.prefs)==null?void 0:pt.phone)||"";const a=document.getElementById("userNameDisplay");if(a){const t=L.username||T;a.innerText=t?"Welcome, "+t+"!":"Welcome!"}try{y=(await C.listDocuments(P,J)).documents.map(s=>({...s,id:s.$id})),x=(await C.listDocuments(P,ee,[Q.equal("user_id",A)])).documents.map(s=>({...s,id:s.$id})),D=(await C.listDocuments(P,qe,[Q.equal("user_id",A)])).documents.map(s=>({...s,id:s.$id})),R=(await C.listDocuments(P,Pe,[Q.equal("owner",T)])).documents.map(s=>({...s,id:s.$id}))}catch(t){console.warn("Database collections not fully setup yet. Using empty arrays.",t)}}catch(e){console.error("User not logged in",e),window.location.href="/PawTrackLogin.html";return}async function M(e,a,t){try{await C.createDocument(P,ut,X.unique(),{user_id:A,action:e||"Activity recorded",target:a||"",icon:t||"fa-paw",timestamp:Date.now().toString()})}catch(i){console.warn("Failed to persist activity log to Appwrite:",i)}}function Ve(){document.body.style.zoom="1",document.body.style.width="100%",document.body.style.height="100%",navigator.userAgent.toLowerCase().includes("firefox")&&(document.body.style.transform="none")}let ne;function b(e,a,t=!1,i=null){const r=document.getElementById("customPopupOverlay"),n=document.getElementById("customPopupTitle"),s=document.getElementById("customPopupMessage"),o=document.getElementById("customPopupActions");n.innerText=e,s.innerText=a,n.className=t?"custom-popup-title title-error":"custom-popup-title title-success",o.style.display="none",r.style.display="flex",clearTimeout(ne),ne=setTimeout(()=>{O(i)},5e3),document.getElementById("customPopupClose").onclick=()=>{clearTimeout(ne),O(i)}}function gt(e,a,t){const i=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),s=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-success",n.innerHTML=`<p style="margin-bottom:10px;">${a}</p><input type="text" id="customPromptInput" style="width:100%; padding:12px; border-radius:8px; border:2px solid #cbd5e1; outline:none; font-size:1rem; font-weight:600; color:#0f172a;" autocomplete="off">`,s.style.display="flex",i.style.display="flex",clearTimeout(ne),document.getElementById("btnPopupConfirm").innerText="Submit",document.getElementById("btnPopupConfirm").onclick=()=>{const o=document.getElementById("customPromptInput").value;O(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",o.trim()!==""&&t(o.trim())},document.getElementById("customPopupClose").onclick=()=>{O()},document.getElementById("btnPopupCancel").onclick=()=>{O(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure"}}function oe(e,a,t){const i=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),s=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-error",n.innerHTML=`<p>${a}</p>`,s.style.display="flex",i.style.display="flex",clearTimeout(ne),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",document.getElementById("btnPopupConfirm").onclick=()=>{O(),t&&t()},document.getElementById("customPopupClose").onclick=()=>{O()},document.getElementById("btnPopupCancel").onclick=()=>{O()}}function O(e){document.getElementById("customPopupOverlay").style.display="none",e&&e()}window.addEventListener("resize",Ve),Ve();const re=[{initials:"MD",name:"Dr. Miguel Antonio Dela Cruz",specialty:"General Veterinary Practitioner",phone:"0917-555-0101",email:"mdelacruz@pawtrack.ph",schedule:"Mon-Fri: 8am-4pm",clinic:"Quezon City Main Clinic",exp:"15 yrs exp",rating:"4.8"},{initials:"JS",name:"Dr. Joanna Marie R. Santos",specialty:"Veterinary Surgeon",phone:"0917-555-0102",email:"jmsantos@pawtrack.ph",schedule:"Tue-Sat: 10am-6pm",clinic:"Makati Pet Hospital",exp:"12 yrs exp",rating:"4.9"},{initials:"PV",name:"Dr. Paulo C. Villanueva",specialty:"Veterinary Oncologist",phone:"0917-555-0103",email:"pvillanueva@pawtrack.ph",schedule:"Mon-Thu: 9am-5pm",clinic:"BGC Animal Center",exp:"8 yrs exp",rating:"4.7"}];let V=0,U=[],te=[],W=0,N=null,ae=[],Ce=3;const j=document.querySelectorAll(".sidebar-nav .nav-btn"),v=document.getElementById("mainDisplayPanel"),Ue=document.getElementById("profileTemplate").innerHTML,$e=document.getElementById("cartItemsContainer"),ft=document.getElementById("cartTotalDisplay"),le=(e,a,t)=>`
    ...
    `;function de(){let e="";if(typeof y<"u"&&y.length>0){const a=y.filter(t=>t.status==="Available");a.length>0?a.forEach(t=>{const i=t.gender==="Female"?"gender-female":"gender-male",n=t.owner===T?`
        ...
        `}const Ae=()=>{let e="";return typeof R<"u"&&R.length>0?R.forEach(a=>{const t=a.gender==="Female"?"gender-female":"gender-male";e+=`
                    <div class="pet-item-wrapper">
    ...
    `;function Se(){let e="";typeof D<"u"&&D.length>0?D.forEach(n=>{const s=n.status==="Approved",o=s?"appt-approved":"appt-pending",l=s?"badge-approved":"badge-pending",u=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],p=n.date.split("-"),g=p.length===3?u[parseInt(p[1])-1]:"TBD",f=p.length===3?p[2]:"??";e+=`
    ...
    `;function ye(){const e=y.filter(t=>t.owner===T&&t.status==="Private");let a="";return e.length>0?e.forEach(t=>{const i=t.gender==="Female"?"gender-female":"gender-male",r=t.img||"https://ui-avatars.com/api/?name=Pet&background=e2e8f0&color=64748b";a+=`
        ...
        `;const o=s.querySelector(".toast-close");let l=null;const u=p=>{p&&p.stopPropagation(),l&&clearTimeout(l),s.style.animation="toastSlideOut 0.25s forwards",setTimeout(()=>{s.parentElement&&s.parentElement.removeChild(s)},250)};o.addEventListener("click",u),i&&s.addEventListener("click",()=>{u();const p=document.querySelector(`.sidebar-nav .nav-btn[data-target="${i}"]`);if(p)p.click();else if(i==="pets"){j.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="pets"]');g&&g.classList.add("active"),v.innerHTML=de()}else if(i==="applications"){j.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="applications"]');g&&g.classList.add("active"),v.innerHTML=ce()}}),n.appendChild(s),l=setTimeout(u,7e3)}function bt(){if(document.getElementById("adoptionApplicationForm"))return"adopt-form";if(document.getElementById("petRegistrationForm")||document.getElementById("registerPetForm"))return"register-pet";if(document.getElementById("profileFullName")||document.querySelector(".profile-container"))return"profile";const e=document.querySelector(".sidebar-nav .nav-btn.active");return e?e.getAttribute("data-target"):"home"}function me(){const e=bt();e==="home"?pe():e==="pets"?v.innerHTML=de():e==="applications"?v.innerHTML=ce():e==="profile"?be():e==="breeding"&&typeof ye=="function"&&!document.getElementById("matchDetailsOverlay")&&(v.innerHTML=ye())}function wt(e){if(!e||!e.payload)return;const a=e.events||[],t=e.payload,i=a.some(s=>s.includes(".create")),r=a.some(s=>s.includes(".update")),n=a.some(s=>s.includes(".delete"));if(t.$collectionId===J||a.some(s=>s.includes(`.${J}.`))){const s=t.$id,o={...t,id:t.$id};if(n){const l=y.find(u=>u.id===s);y=y.filter(u=>u.id!==s),l&&l.status==="Available"&&l.owner!==T&&G("Pet Removed",`${l.name} is no longer listed for adoption.`,"fa-paw","pets","info")}else if(i){const l=y.findIndex(u=>u.id===s);l===-1?y.unshift(o):y[l]=o,o.status==="Available"&&o.owner!==T&&G("New Pet for Adoption! 🐾",`@${o.owner||"A caregiver"} just listed ${o.name} (${o.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}else if(r){const l=y.findIndex(p=>p.id===s),u=l!==-1?y[l]:null;l!==-1?y[l]=o:y.unshift(o),o.owner!==T&&(u&&u.status!=="Available"&&o.status==="Available"?G("Pet Available for Adoption! 🐾",`${o.name} (${o.breed}) is now available for adoption! Tap to view.`,"fa-heart","pets","default"):u&&u.status==="Available"&&o.status!=="Available"&&G("Pet Status Updated",`${o.name} is now ${o.status}.`,"fa-paw","pets","info"))}me()}if(t.$collectionId===ee||a.some(s=>s.includes(`.${ee}.`))){const s=t.$id,o={...t,id:t.$id};if(o.user_id===A){if(n)x=x.filter(l=>l.id!==s);else if(i){const l=x.findIndex(u=>u.id===s);l===-1?x.unshift(o):x[l]=o}else if(r){const l=x.findIndex(p=>p.id===s),u=l!==-1?x[l].status:"";if(l!==-1?x[l]=o:x.unshift(o),u!==o.status){const p=o.status==="Approved";G(p?"🎉 Application Approved!":"Application Status Updated",`Your adoption application for ${o.pet_name} is now: ${o.status}!`,p?"fa-circle-check":"fa-clock","applications","app")}}me()}}}let Me=!1;async function xt(){if(!Me){Me=!0;try{const a=(await C.listDocuments(P,J)).documents.map(n=>({...n,id:n.$id})),t=new Set(y.map(n=>n.id)),i=a.filter(n=>!t.has(n.id)&&n.status==="Available"&&n.owner!==T);if(a.length!==y.length||a.some((n,s)=>{var o,l;return((o=y[s])==null?void 0:o.id)!==n.id||((l=y[s])==null?void 0:l.status)!==n.status})){if(y=a,i.length>0){const n=i[0];G("New Pet for Adoption! 🐾",`@${n.owner||"A caregiver"} just listed ${n.name} (${n.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}me()}if(A){const s=(await C.listDocuments(P,ee,[Q.equal("user_id",A)])).documents.map(l=>({...l,id:l.$id}));(s.length!==x.length||s.some((l,u)=>{var p,g;return((p=x[u])==null?void 0:p.id)!==l.id||((g=x[u])==null?void 0:g.status)!==l.status}))&&(x=s,me())}}catch{}finally{Me=!1}}}function Et(){try{if(Ne&&typeof Ne.subscribe=="function"){const e=`databases.${P}.collections.${J}.documents`,a=`databases.${P}.collections.${ee}.documents`;Ne.subscribe([e,a],t=>{wt(t)}),console.log("PawTrack: Appwrite Realtime connected for live adoption sync.")}}catch(e){console.warn("Realtime subscription fallback to polling:",e)}setInterval(xt,8e3)}Et();function Z(){const e=re[V],a=document.getElementById("vetInitials");if(!a)return;a.innerText=e.initials,document.getElementById("vetNameDisplay").innerText=e.name,document.getElementById("vetSpecialtyDisplay").innerText=e.specialty,document.getElementById("vetRatingDisplay").innerText=`⭐ ${e.rating} (${e.exp})`,document.getElementById("vetPhoneDisplay").innerText=e.phone,document.getElementById("vetEmailDisplay").innerText=e.email,document.getElementById("vetScheduleDisplay").innerText=e.schedule,document.getElementById("vetClinicDisplay").innerText=e.clinic;const t=document.getElementById("apptVetIdHidden").value,i=document.getElementById("btnSelectVet");t===V.toString()?(i.innerText="✅ Confirmed",i.classList.add("selected")):(i.innerText="Confirm",i.classList.remove("selected"))}function ue(){const e=document.getElementById("cartBadge");e&&(e.innerText=U.length),$e.innerHTML="";let a=0;U.length===0?$e.innerHTML='<p style="text-align:center; color:#64748b; font-weight:bold; margin-top: 50px;">Your cart is empty.</p>':U.forEach((t,i)=>{a+=parseFloat(t.price),$e.innerHTML+=`
                ...
                `}),ft.innerText=`₱ ${a.toFixed(2)}`}window.handleRewind=e=>{ae.splice(e,1),W=Math.max(0,W-1),De()};function Bt(){const e=document.getElementById("rewindList");e&&(e.innerHTML="",ae.length===0?e.innerHTML=`
                ...
                `}))}function De(){if(te.length===0||W>=te.length){document.getElementById("datingContentLayout").style.display="none";const s=document.getElementById("instructionOverlay");s.style.display="flex",s.innerHTML='<i class="fa-solid fa-face-frown-open" style="color: #cbd5e1; font-size: 4rem; margin-bottom: 15px;"></i><p>No more matches available<br>for this pet at the moment.</p>';return}const e=te[W];currentPhotoIndex=0;const a=document.getElementById("candThumbnails");a.innerHTML="",e.imgs.forEach((s,o)=>{a.innerHTML+=`<img src="${s}" class="cand-thumb ${o===0?"active":""}" data-index="${o}" alt="Photo ${o+1}">`}),Ye(e,0);const t=e.gender==="Male"?'<i class="fa-solid fa-mars" style="color: #60a5fa;"></i>':'<i class="fa-solid fa-venus" style="color: #f472b6;"></i>';document.getElementById("candNameAge").innerHTML=`${e.name}, ${e.age} ${t}`,document.getElementById("candBreedGender").innerHTML=`<i class="fa-solid fa-paw"></i> ${e.breed}`,document.getElementById("candDesc").innerText=e.desc,document.getElementById("candScoreText").innerText=`${e.score}% Match`,document.getElementById("candScoreBar").style.width=`${e.score}%`;const i=document.getElementById("candBadges");i.innerHTML="",e.badges.includes("vet")&&(i.innerHTML+='<span class="v-badge badge-vet"><i class="fa-solid fa-stethoscope"></i> Vet Verified</span>'),e.badges.includes("pedigree")&&(i.innerHTML+='<span class="v-badge badge-pedigree"><i class="fa-solid fa-dna"></i> Purebred</span>'),e.badges.includes("vax")&&(i.innerHTML+='<span class="v-badge badge-vax"><i class="fa-solid fa-syringe"></i> Vaccinated</span>');const r=document.getElementById("candTraits");r.innerHTML="",e.traits.forEach(s=>{r.innerHTML+=`<span class="trait-tag">${s}</span>`}),document.getElementById("candOwnerName").innerText=e.ownerName,document.getElementById("candOwnerInitial").innerText=e.ownerInitial,document.getElementById("candOwnerRating").innerText=`⭐ ${e.ownerRating}`,document.getElementById("candOwnerPairs").innerText=`${e.successPairs} Successful Pairs`,document.getElementById("barSize").style.width=`${e.compSize}%`,document.getElementById("txtSize").innerText=`${e.compSize}%`,document.getElementById("barEnergy").style.width=`${e.compEnergy}%`,document.getElementById("txtEnergy").innerText=`${e.compEnergy}%`,document.getElementById("barTemp").style.width=`${e.compTemp}%`,document.getElementById("txtTemp").innerText=`${e.compTemp}%`;const n=document.getElementById("litterPredictor");n.innerHTML="",e.litter.forEach(s=>{const o=s.c==="#FFFFFF"||s.c==="#FFF8DC"?"#0f172a":"white";n.innerHTML+=`<div class="color-swatch" style="background:${s.c}; color:${o};">${s.p}%</div>`}),document.getElementById("instructionOverlay").style.display="none",document.getElementById("datingContentLayout").style.display="flex",document.getElementById("treatCountDisplay").innerText=`${Ce} Left`,Bt()}function Ye(e,a){const t=document.getElementById("candImg");t.style.opacity=0,setTimeout(()=>{t.src=e.imgs[a],t.style.opacity=1},150),document.querySelectorAll(".cand-thumb").forEach((i,r)=>{r===parseInt(a)?i.classList.add("active"):i.classList.remove("active")})}j.forEach(e=>{e.addEventListener("click",function(){j.forEach(t=>t.classList.remove("active")),this.classList.add("active");const a=this.getAttribute("data-target");v.scrollTo({top:0,behavior:"smooth"}),a==="home"?pe():a==="pets"?v.innerHTML=de():a==="applications"?v.innerHTML=ce():a==="vet"?(v.innerHTML=Se(),Z()):a==="shop"?(v.innerHTML=vt(),ue()):a==="breeding"?(v.innerHTML=ye(),N=null):a==="messages"&&(v.innerHTML=Ke(),Qe())})}),document.getElementById("btnViewProfile").addEventListener("click",()=>{j.forEach(e=>e.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=Ue,be()}),document.getElementById("btnRegisterPet").addEventListener("click",()=>{j.forEach(r=>r.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=yt;const e=document.getElementById("imageDropZone"),a=document.getElementById("petImageInput"),t=document.getElementById("imagePreview"),i=document.getElementById("dropZoneText");e.addEventListener("click",()=>a.click()),Oe=null,a.addEventListener("change",function(){if(this.files&&this.files[0]){Oe=this.files[0];const r=new FileReader;r.onload=function(n){t.src=n.target.result,t.style.display="block",i.style.display="none"},r.readAsDataURL(this.files[0])}}),document.getElementById("btnCancelReg").addEventListener("click",()=>{pe(),j[0].classList.add("active")})}),v.addEventListener("click",async e=>{if(e.target.closest("#btnMatchRegisterPet")){document.getElementById("btnRegisterPet").click();return}if(e.target.classList.contains("profile-tab-btn")){document.querySelectorAll(".profile-tab-btn").forEach(d=>d.classList.remove("active")),document.querySelectorAll(".profile-tab-content").forEach(d=>d.classList.remove("active")),e.target.classList.add("active");const c=e.target.getAttribute("data-tab");document.getElementById(c).classList.add("active")}const t=e.target.closest(".btn-view-app")||e.target.closest(".btn-view-pet");if(t){const c=t.getAttribute("data-target"),d=document.getElementById(c),m=t.closest(".app-card")||t.closest(".pet-card");d.style.display==="block"?(d.style.display="none",m.classList.remove("dropdown-open")):(d.style.display="block",m.classList.add("dropdown-open"))}const i=e.target.closest(".btn-close-pet");if(i){const c=i.getAttribute("data-target"),d=document.getElementById(c),m=d.previousElementSibling;d.style.display="none",m.classList.remove("dropdown-open")}const r=e.target.closest(".btn-adopt");if(r){const c=r.getAttribute("data-petid"),d=r.getAttribute("data-petname");v.innerHTML=We(c,d)}e.target.closest("#btnBackToPets")&&(v.innerHTML=de()),e.target.closest(".btn-edit-profile")&&(document.getElementById("editProfileModal").style.display="flex");const n=e.target.closest(".btn-archive-pet");if(n){const c=n.getAttribute("data-petid");oe("Move to Bin?","Are you sure you want to move this pet to the Recycle Bin?",async()=>{const d=y.find(m=>m.id===c);if(!d){b("Error","Pet record not found.",!0);return}try{const m=await C.createDocument(P,Pe,X.unique(),{name:d.name,breed:d.breed,gender:d.gender,owner:T,img:d.img||""});await C.deleteDocument(P,J,c),y=y.filter(I=>I.id!==c),R.unshift({...m,id:m.$id}),await M("Moved a pet to the Recycle Bin",d.name,"fa-trash-can"),b("Moved to Bin","Pet successfully moved to Recycle Bin.",!1,()=>{me()})}catch(m){console.error("Appwrite Move to Bin Error:",m),b("Error",m.message,!0)}})}e.target.closest("#btnViewBin")&&(v.innerHTML=Ae()),e.target.closest("#btnEmptyBin")&&oe("Empty Bin?","WARNING: Are you sure you want to permanently delete ALL pets in the Recycle Bin? This cannot be undone!",async()=>{try{for(const c of R)await C.deleteDocument(P,Pe,c.id);R=[],await M("Permanently emptied the Recycle Bin","","fa-dumpster-fire"),b("Bin Emptied","Recycle bin emptied successfully.",!1,()=>{v.innerHTML=Ae()})}catch(c){console.error("Appwrite Empty Bin Error:",c),b("Error",c.message,!0)}}),e.target.closest("#btnBackToProfile")&&(v.innerHTML=Ue,be());const s=e.target.closest(".btn-restore-pet");if(s){const c=s.getAttribute("data-petid"),d=R.find(m=>m.id===c);if(d)try{const m=await C.createDocument(P,J,X.unique(),{name:d.name,breed:d.breed,gender:d.gender,age:"1 yr",status:"Available",health_status:"Healthy",owner:T,contact_number:se||"0917-000-0000",personal_traits:"Friendly",reason_for_adoption:"Restored from Recycle Bin",img:d.img||""});await C.deleteDocument(P,Pe,c),R=R.filter(I=>I.id!==c),y.unshift({...m,id:m.$id}),await M("Restored a pet from the Recycle Bin",d.name,"fa-rotate-left"),b("Restored!","Pet has been restored to the active board!",!1,()=>{v.innerHTML=Ae()})}catch(m){console.error("Appwrite Restore Error:",m),b("Error",m.message,!0)}}const o=e.target.closest(".btn-cancel-app");if(o){const c=o.getAttribute("data-appid");oe("Cancel Application?","Are you sure you want to cancel this application?",async()=>{try{await C.deleteDocument(P,ee,c),x=x.filter(d=>d.id!==c),await M("Cancelled adoption application","","fa-file-circle-xmark"),b("Cancelled","Application successfully cancelled.",!1,()=>{v.innerHTML=ce()})}catch(d){console.error("Appwrite Cancel App Error:",d),b("Error",d.message,!0)}})}const l=e.target.closest(".btn-cancel-vet");if(l){const c=l.getAttribute("data-appid");oe("Cancel Appointment?","Are you sure you want to cancel this veterinary appointment?",async()=>{try{await C.deleteDocument(P,qe,c),D=D.filter(d=>d.id!==c),await M("Cancelled veterinary appointment","","fa-calendar-xmark"),b("Cancelled","Appointment successfully cancelled.",!1,()=>{v.innerHTML=Se(),Z()})}catch(d){console.error("Appwrite Cancel Vet Error:",d),b("Error",d.message,!0)}})}const u=e.target.closest(".pet-select-card");if(u&&document.getElementById("apptPetSelectorHidden")){document.querySelectorAll(".pet-select-card").forEach(m=>m.classList.remove("selected")),u.classList.add("selected");const c=u.getAttribute("data-pet");document.getElementById("apptPetSelectorHidden").value=c;const d=myPetsDB[c];d&&(document.getElementById("apptOwner").value=d.owner,document.getElementById("apptPetType").value=d.type,document.getElementById("apptBreed").value=d.breed,document.getElementById("apptGender").value=d.gender,document.getElementById("apptWeight").value=d.weight)}if(e.target.closest("#btnPrevVet")&&(V=(V-1+re.length)%re.length,Z()),e.target.closest("#btnNextVet")&&(V=(V+1)%re.length,Z()),e.target.closest("#btnSelectVet")){const c=re[V];document.getElementById("apptVetIdHidden").value=V,document.getElementById("apptSelectedVetName").value=c.name,Z()}if(e.target.classList.contains("btn-add-cart")){U.push({name:e.target.getAttribute("data-name"),price:e.target.getAttribute("data-price"),img:e.target.getAttribute("data-img")}),ue();const c=e.target.innerText;e.target.innerText="✓ Added",e.target.style.background="#10b981",e.target.style.color="white",e.target.style.borderColor="#10b981",setTimeout(()=>{e.target.innerText=c,e.target.style.background="white",e.target.style.color="#4f46e5",e.target.style.borderColor="#4f46e5"},1e3)}if(e.target.classList.contains("btn-buy-now")){const d=e.target.closest(".shop-card").querySelector(".btn-add-cart");U.push({name:d.getAttribute("data-name"),price:d.getAttribute("data-price"),img:d.getAttribute("data-img")}),ue(),document.getElementById("cartModalOverlay").style.display="flex"}e.target.closest("#btnOpenCart")&&(document.getElementById("cartModalOverlay").style.display="flex"),e.target.closest(".mode-btn")&&(document.querySelectorAll(".mode-btn").forEach(c=>c.classList.remove("active")),e.target.closest(".mode-btn").classList.add("active"),N&&document.querySelector(`[data-pet="${N}"]`).click());const p=e.target.closest(".pet-select-card");if(p&&document.getElementById("vetBookingForm")){document.querySelectorAll(".pet-select-card").forEach(m=>m.classList.remove("selected")),p.classList.add("selected");const c=p.getAttribute("data-petid");document.getElementById("apptPetSelectorHidden").value=c;const d=y.find(m=>m.id.toString()===c);d&&(document.getElementById("apptOwner").value=T,document.getElementById("apptPetType").value=d.breed&&d.breed.toLowerCase().includes("cat")?"Cat":"Dog",document.getElementById("apptBreed").value=d.breed||"Unknown",document.getElementById("apptGender").value=d.gender||"Unknown")}const g=e.target.closest(".my-pet-card");if(g&&!document.getElementById("apptPetSelectorHidden")){document.querySelectorAll(".my-pet-card").forEach(d=>d.classList.remove("active")),g.classList.add("active"),N=g.getAttribute("data-petid"),y.find(d=>d.id.toString()===N),document.getElementById("datingContentLayout").style.display="none";const c=document.getElementById("instructionOverlay");c.style.display="flex",c.innerHTML='<div class="radar-container"><div class="radar-ring ring1"></div><div class="radar-ring ring2"></div><div class="radar-ring ring3"></div><div class="radar-center"><i class="fa-solid fa-satellite-dish"></i></div></div><p>Scanning database for verified matches...</p>',Promise.resolve().then(()=>({status:"success",candidates:y.filter(m=>m.owner!==T).map(m=>({id:m.id,name:m.name,breed:m.breed,gender:m.gender,age:m.age,photos:[m.img],imgs:[m.img],owner:m.owner||"PawUser",ownerName:m.owner||"PawUser",score:94,traits:m.personal_traits?m.personal_traits.split(",").map(I=>I.trim()):["Playful","Friendly"],desc:m.reason_for_adoption||"Looking for a friend!",verified:!0}))})).then(d=>{d.status==="success"&&d.candidates.length>0?(te=d.candidates,W=0,setTimeout(()=>{De()},800)):c.innerHTML='<p style="color:#64748b; font-weight:bold; font-size:1.1rem;">No match candidates available right now. Invite friends or register more pets!</p>'})}const f=e.target.closest(".cand-thumb");if(f){const c=te[W];Ye(c,f.getAttribute("data-index"))}const h=e.target.closest("#btnPassCandidate"),E=e.target.closest("#btnMatchCandidate"),B=e.target.closest("#btnTreatCandidate");if(h||E||B){const c=te[W],d=y.find(I=>I.id.toString()===N);let m="pass";if(E&&(m="like"),B&&(m="super_like"),m==="super_like"&&Ce<=0){b("Out of Treats","You have used all your Super Treats for today! Come back tomorrow.",!0);return}Promise.resolve({status:"success",isMatch:!0}).then(I=>{if(I.status==="success"){if(m==="super_like"&&Ce--,ae.unshift(c),ae.length>5&&ae.pop(),m!=="pass"){M(m==="super_like"?"Sent a Super Treat to":"Sent a match request to",c.name,"fa-heart"),document.getElementById("matchImgLeft").src=d.img,document.getElementById("matchImgRight").src=c.imgs[0],document.getElementById("matchNameLeft").innerText=d.name,document.getElementById("matchNameRight").innerText=c.name;const _=I.isMatch?"✨ IT'S A MATCH! ✨":m==="super_like"?"Super Liked! 🦴 (Pending)":"Pending Owner Approval";document.getElementById("matchDateSpot").innerText="PawTrack Verified";const k=document.getElementById("matchIcebreaker");k.value=I.isMatch?"You matched! Let's plan a playdate!":"",k.placeholder=I.isMatch?"Type a message...":`Say hi to ${c.ownerName} while you wait...`,document.getElementById("btnContinueMatch").setAttribute("data-targetuser",c.ownerName),document.getElementById("matchOverlay").style.display="flex",activePairs.unshift({id:"p"+Date.now(),maleName:d.gender==="Male"?d.name:c.name,femaleName:d.gender==="Female"?d.name:c.name,maleImg:d.gender==="Male"?d.img:c.imgs[0],femaleImg:d.gender==="Female"?d.img:c.imgs[0],status:_,isApproved:I.isMatch,date:"Just Now",ownerUsername:c.ownerName})}W++,De()}else b("Error",I.message,!0)}).catch(I=>console.error("Swipe Error:",I))}e.target.closest("#btnOpenPrefs")&&(document.getElementById("prefModal").style.display="flex"),e.target.closest("#btnViewActivePairs")&&he(),e.target.closest("#btnBackToMatch")&&(v.innerHTML=ye(),N=null);const w=e.target.closest(".btn-accept-match"),$=e.target.closest(".btn-delete-pair");if(w||$){const c=w?"accept":"delete",d=(w||$).getAttribute("data-matchid");if(c==="delete"){const m=$.getAttribute("data-title")||"Cancel Request",I=$.getAttribute("data-msg")||"Are you sure you want to remove this match?";oe(m,I,()=>{ve=ve.filter(_=>_.id!==d),he()})}else{const m=ve.find(I=>I.id===d);m&&(m.status="approved"),he()}}}),document.getElementById("btnCloseCart").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnContinueShopping").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnCheckout").addEventListener("click",()=>{if(U.length===0){b("Empty Cart","Your cart is empty! Please add some items first.",!0);return}M("Completed a Pet Shop checkout","","fa-bag-shopping"),U=[],ue(),document.getElementById("cartModalOverlay").style.display="none",b("Order Placed!","Order placed successfully! Thank you for shopping.",!1)}),document.getElementById("cartItemsContainer").addEventListener("click",e=>{const a=e.target.closest(".btn-remove-item");a&&(U.splice(parseInt(a.getAttribute("data-index")),1),ue())}),document.getElementById("btnContinueMatch").addEventListener("click",e=>{const a=e.target.getAttribute("data-targetuser"),t=document.getElementById("matchIcebreaker").value.trim();t&&a&&(S[a]||(S[a]=[]),S[a].push({text:t,type:"sent",time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}),M("Sent an Icebreaker message to",a,"fa-comment-dots")),document.getElementById("matchOverlay").style.display="none",he()}),document.getElementById("btnSavePrefs").addEventListener("click",()=>{document.getElementById("prefModal").style.display="none",N&&document.querySelector(`[data-pet="${N}"]`).click()}),document.getElementById("prefModal").addEventListener("click",e=>{e.target===document.getElementById("prefModal")&&(document.getElementById("prefModal").style.display="none")}),v.addEventListener("submit",e=>{if(e.target.id==="vetBookingForm"){e.preventDefault();const a=document.getElementById("apptPetSelectorHidden").value,t=document.getElementById("apptVetIdHidden").value;if(!a){b("Error","Please select a pet for the appointment by clicking their picture!",!0);return}if(!t){b("Error","Please choose a veterinarian by clicking 'Confirm' on the profile card!",!0);return}const i=document.querySelector(`.pet-select-card[data-petid="${a}"] span`).innerText,r=document.getElementById("apptSelectedVetName").value,n=y.find(s=>{var o;return((o=s.id)==null?void 0:o.toString())===(a==null?void 0:a.toString())});(async()=>{try{const s=await C.createDocument(P,qe,X.unique(),{pet_name:i,vet_name:r,status:"Upcoming",time:document.getElementById("apptTime").value||"",date:document.getElementById("apptDate").value||"",user_id:A,img:(n==null?void 0:n.img)||""});D.unshift({...s,id:s.$id}),await M("Booked vet visit for",i,"fa-user-doctor"),b("Request Sent!","Appointment scheduled successfully! It is now Upcoming.",!1,()=>{const o=document.querySelector('[data-target="vet"]');o?o.click():(v.innerHTML=Se(),Z())})}catch(s){console.error("Appwrite Vet Booking Error:",s),b("Error",s.message,!0)}})()}if(e.target.id==="adoptionApplicationForm"){if(e.preventDefault(),!document.getElementById("adoptTerms").checked){b("Missing Requirement","You must agree to the terms and conditions.",!0);return}const a=document.getElementById("adoptPetId").value,t=document.getElementById("adoptPetName").value,i=y.find(r=>{var n;return((n=r.id)==null?void 0:n.toString())===(a==null?void 0:a.toString())});(async()=>{try{const r=await C.createDocument(P,ee,X.unique(),{pet_name:t,date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),status:"Pending Review",user_id:A,img:(i==null?void 0:i.img)||""});x.unshift({...r,id:r.$id}),await M("Applied to adopt",t,"fa-house-chimney-user"),b("Application Sent!","Successfully submitted! Status: PENDING REVIEW.",!1,()=>{const n=document.querySelector('[data-target="applications"]');n?n.click():v.innerHTML=ce()})}catch(r){console.error("Appwrite Adoption Application Error:",r),b("Error",r.message,!0)}})()}(e.target.id==="petRegistrationForm"||e.target.id==="registerPetForm")&&(e.preventDefault(),(async()=>{var l,u,p,g,f;const a=document.getElementById("petImageInput"),t=Oe||a&&a.files&&a.files[0];let i="";if(t)try{b("Uploading Photo","Saving pet photo to PawTrack Storage...",!1);const h=await ke.createFile(Le,X.unique(),t);i=ke.getFileView(Le,h.$id).toString()}catch(h){console.error("Storage upload failed:",h)}i||(i="https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=600&q=80");const r=document.getElementById("regForAdoption"),n=r?r.checked:!0,s=new FormData(e.target),o={name:s.get("name")||((l=document.getElementById("regPetName"))==null?void 0:l.value)||"Unnamed Pet",breed:s.get("breed")||((u=document.getElementById("regBreed"))==null?void 0:u.value)||"Mixed Breed",gender:s.get("gender")||((p=document.getElementById("regGender"))==null?void 0:p.value)||"Male",age:s.get("age")||((g=document.getElementById("regAge"))==null?void 0:g.value)||"1 yr",status:n?"Available":"Private",health_status:s.get("health_status")||"Healthy / Vaccinated",owner:T,contact_number:s.get("contact_number")||se||"0917-000-0000",personal_traits:s.get("personal_traits")||((f=document.getElementById("regDesc"))==null?void 0:f.value)||"Friendly",reason_for_adoption:s.get("reason_for_adoption")||(n?"Looking for a home":"Personal pet"),img:i};try{const h=await C.createDocument(P,J,X.unique(),o),E={...h,id:h.$id};y.unshift(E),await M(n?"Listed pet for adoption":"Registered private pet",o.name,"fa-shield-cat"),b("Success!",n?"Pet Registered Successfully! Photo saved to PawTrack Storage and listed on the adoption board.":"Pet added to your personal roster! Photo saved to PawTrack Storage.",!1,()=>{if(n){const w=document.querySelector('[data-target="pets"]');w?w.click():v.innerHTML=de()}else{const w=document.querySelector('[data-target="home"]');w?w.click():pe()}})}catch(h){console.error("Appwrite Pet Registration Error:",h),b("Error","Could not register pet: "+h.message,!0)}})())}),v.addEventListener("reset",e=>{e.target.id==="vetBookingForm"&&(document.querySelectorAll(".pet-select-card").forEach(a=>a.classList.remove("selected")),document.getElementById("apptPetSelectorHidden")&&(document.getElementById("apptPetSelectorHidden").value=""),document.getElementById("apptVetIdHidden")&&(document.getElementById("apptVetIdHidden").value=""),setTimeout(()=>{Z()},10))});const Xe=document.getElementById("editProfileForm");Xe&&Xe.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("editFirstName").value,t=document.getElementById("editLastName").value,i=document.getElementById("editContact").value,r=`${a} ${t}`.trim();try{await ie.updatePrefs({...L,fullName:r,phone:i}),L.fullName=r,L.phone=i,se=i;const n=document.getElementById("userNameDisplay");if(n){const s=L.username||r;n.innerText=s?"Welcome, "+s+"!":"Welcome!"}await M("Updated profile settings & information","","fa-user-pen"),document.getElementById("editProfileModal").style.display="none",b("Profile Saved","Profile updated successfully!",!1,()=>{be()})}catch(n){b("Error",n.message,!0)}}),v.addEventListener("input",e=>{e.target.id==="shopPriceFilter"&&(document.getElementById("shopPriceDisplay").innerText=`₱${e.target.value}`,Je())}),v.addEventListener("change",e=>{e.target.id==="shopCategoryFilter"&&Je()});function Je(){const e=document.getElementById("shopCategoryFilter").value,a=parseFloat(document.getElementById("shopPriceFilter").value),t=je.filter(i=>{const r=e==="All Categories"||i.category===e,n=i.price<=a;return r&&n});document.getElementById("shopGridContainer").innerHTML=Ge(t)}async function be(){const e=document.getElementById("mainProfilePic");e&&(e.src=L.avatarUrl||"/resources/avatar/Avatar 1.jpg");const a=document.getElementById("btnChangeAvatar")||document.querySelector(".btn-change-photo"),t=document.getElementById("avatarFileInput");a&&t&&(a.onclick=()=>t.click(),t.onchange=async function(){if(this.files&&this.files[0])try{b("Uploading","Uploading avatar to PawTrack Storage...",!1);const k=await ke.createFile(Le,X.unique(),this.files[0]),z=ke.getFileView(Le,k.$id).toString();await ie.updatePrefs({...L,avatarUrl:z}),L.avatarUrl=z,e&&(e.src=z),await M("Updated profile picture","Saved to PawTrack Storage","fa-camera"),b("Success","Profile photo uploaded to PawTrack Storage!")}catch(k){b("Error","Failed to upload photo: "+k.message,!0)}});const i=document.getElementById("profileFullName"),r=document.getElementById("profileUsername"),n=document.getElementById("profileEmail"),s=document.getElementById("profilePhone"),o=L.fullName||T,l=L.username||T;i&&(i.innerText=o),r&&(r.innerText="@"+l),n&&(n.innerText=ze),s&&(s.innerText=se||"None");const[u="",...p]=(o||"").split(" "),g=p.join(" "),f=document.getElementById("editFirstName"),h=document.getElementById("editLastName"),E=document.getElementById("editContact"),B=document.getElementById("editEmail");f&&(f.value=u),h&&(h.value=g),E&&(E.value=se||""),B&&(B.value=ze||"");const w=y.filter(k=>k.owner===T),$=x.filter(k=>k.status==="Approved").length,c=document.getElementById("countOwnedPets"),d=document.getElementById("countSuccessfulApps");c&&(c.innerText=w.length),d&&(d.innerText=$);const m=document.getElementById("privateRosterGrid"),I=document.getElementById("adoptionRosterGrid");m&&I&&(m.innerHTML="",I.innerHTML="",w.forEach(k=>{const z=`
                    <div class="roster-card">
                        <img src="${k.img}" class="roster-img">
                        <div class="roster-info"><h4>${k.name}</h4><p>${k.breed}</p></div>
                        <button class="btn-archive-pet" data-petid="${k.id}" title="Move to Bin" 
                ...
                `;k.status==="Private"?m.innerHTML+=z:I.innerHTML+=z}));const _=document.getElementById("recentActivityLogs");if(_){_.innerHTML='<div class="activity-loading"><i class="fa-solid fa-circle-notch fa-spin"></i> Loading activity logs...</div>';try{const k=await C.listDocuments(P,ut,[Q.equal("user_id",A),Q.orderDesc("$createdAt"),Q.limit(20)]);_.innerHTML="";const z=k.documents;z.length>0?z.forEach(K=>{const $t=parseInt(K.timestamp)||new Date(K.$createdAt).getTime(),At=Tt($t),Te=It(K.action,K.icon),mt=K.target?F(K.target):"",St=F(K.action||"Activity recorded");_.innerHTML+=`
                            <div class="activity-item">
                                <div class="activity-icon-bubble ${Te.themeClass}">
                                    <i class="fa-solid ${Te.icon}"></i>
                                </div>
                                <div class="activity-details">
                                    <div class="activity-row-main">
                                        <span class="activity-action-text">${St}</span>
                                        <span class="activity-category-pill ${Te.tagClass}">${Te.tagLabel}</span>
                                    </div>
                                    ${mt?`
                                    ...
                                    <div class="activity-meta">
                                        <span class="activity-timestamp"><i class="fa-regular fa-clock"></i> ${At}</span>
                    ...
                    `}catch(k){console.warn("Could not load activity logs:",k),_.innerHTML=`
                    <div class="activity-empty-state">
                ...
                `}}}function F(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function It(e,a){const t=(e||"").toLowerCase();if(t.includes("log")||t.includes("auth")||t.includes("verified")||t.includes("session"))return{themeClass:"theme-emerald",icon:"fa-shield-halved",tagClass:"tag-emerald",tagLabel:"Security"};if(t.includes("bin")||t.includes("delet")||t.includes("dumpster")||t.includes("trash"))return{themeClass:"theme-rose",icon:"fa-trash-can",tagClass:"tag-rose",tagLabel:"Archive"};if(t.includes("pet")||t.includes("adopt")||t.includes("restor")||t.includes("breed"))return{themeClass:"theme-terracotta",icon:"fa-paw",tagClass:"tag-terracotta",tagLabel:"Pet Care"};if(t.includes("photo")||t.includes("avatar")||t.includes("profile")||t.includes("settings"))return{themeClass:"theme-honey",icon:"fa-user-pen",tagClass:"tag-honey",tagLabel:"Profile"};if(t.includes("shop")||t.includes("cart")||t.includes("item")||t.includes("buy"))return{themeClass:"theme-amber",icon:"fa-bag-shopping",tagClass:"tag-amber",tagLabel:"Shop"};let i=a||"fa-bell";return i==="fa-right-to-bracket"&&(i="fa-arrow-right-to-bracket"),{themeClass:"theme-sage",icon:i,tagClass:"tag-sage",tagLabel:"Activity"}}function Tt(e){const a=Date.now(),t=Math.max(0,a-e),i=60*1e3,r=60*i,n=24*r,s=new Date(e),o=s.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});return t<2*i?"Just now":t<60*i?`${Math.floor(t/i)}m ago`:t<24*r?`${Math.floor(t/r)}h ago (${o})`:new Date(a-n).toDateString()===s.toDateString()?`Yesterday at ${o}`:s.toLocaleDateString(void 0,{month:"short",day:"numeric"})+` at ${o}`}const Ze=document.getElementById("btnLogout");Ze&&Ze.addEventListener("click",async()=>{try{await ie.deleteSession("current")}catch(e){console.warn(e)}window.location.href="/PawTrackLogin.html"});let Y=null;Object.keys(S).length===0&&(S.PawTrackCommunity=[{text:"Welcome to PawTrack! Connect with pet lovers, adopters, and arrange playdates right here.",type:"received",time:"10:00 AM"}]);function Ke(){return`
        ...
        `}function Qe(e=null){const a=document.getElementById("messengerInboxList"),t=document.getElementById("messengerChatPanel"),i=document.getElementById("btnPageNewChat"),r=document.getElementById("messengerSearchInput");if(!a||!t)return;function n(p=""){a.innerHTML="";const g=Object.keys(S).filter(f=>f.toLowerCase().includes(p.toLowerCase()));if(g.length===0){a.innerHTML=`
                    <div class="inbox-empty-state">
                        <i class="fa-solid fa-comments"></i>
                        <p>No conversations found.<br>Click "New Conversation" to start chatting!</p>
                    </div>
                `;return}g.forEach(f=>{const h=S[f]||[],E=h.length>0?h[h.length-1].text:"No messages yet",B=h.length>0?h[h.length-1].time:"",w=f===Y?"active":"",$=document.createElement("div");$.className=`inbox-item ${w}`,$.setAttribute("data-user",f),$.innerHTML=`
                ...
                `,$.onclick=()=>s(f),a.appendChild($)})}function s(p){Y=p,document.querySelectorAll(".inbox-item").forEach(w=>{w.classList.toggle("active",w.getAttribute("data-user")===p)});const g=S[p]||[];t.innerHTML=`
            ...
            `;const f=document.getElementById("activeChatStream");g.forEach(w=>{o(f,w.text,w.type,w.time)}),f.scrollTop=f.scrollHeight;const h=document.getElementById("activeChatInput"),E=document.getElementById("btnActiveSend");function B(){const w=h.value.trim();if(!w||!Y)return;const $=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});o(f,w,"sent",$),h.value="",f.scrollTop=f.scrollHeight,S[Y]||(S[Y]=[]),S[Y].push({text:w,type:"sent",time:$}),n(r.value.trim()),Y==="PawTrackCommunity"&&setTimeout(()=>{const c=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),d="We're glad to have you! Feel free to connect with pet owners across PawTrack.";S.PawTrackCommunity.push({text:d,type:"received",time:c}),Y==="PawTrackCommunity"&&(o(f,d,"received",c),f.scrollTop=f.scrollHeight),n(r.value.trim())},1e3)}E.onclick=B,h.onkeypress=w=>{w.key==="Enter"&&B()},h.focus()}function o(p,g,f,h){const E=document.createElement("div");E.className=`stream-bubble bubble-${f}`,E.innerHTML=`
                <div class="bubble-text">${F(g)}</div>
                <div class="bubble-time">${h}</div>
            `,p.appendChild(E)}r.oninput=p=>{n(p.target.value.trim())},i.onclick=()=>{gt("Start Conversation","Enter the PawTrack username you'd like to message:",p=>{if(p&&p.trim()){const g=p.trim().replace("@","");S[g]||(S[g]=[]),n(),s(g)}})};const l=Object.keys(S),u=e||(l.length>0?l[0]:null);n(),u?s(u):t.innerHTML=`
