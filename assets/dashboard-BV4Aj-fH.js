import{a as me,d as D,Q as re,I as ae,s as He,c as Ke}from"./appwrite-CRWyoAJa.js";const M="pawtrack_db",ie="pets",le="applications",Ze="vet_appointments",De="recycle_bin",Bt="activity_logs",Fe="pawtrack_storage";let A="",Qe="",F="",S={},de="",x=[],k=[],O=[],Y=[],et=null,Ee=[],P={};async function It(I,X=1600,ue=.85){return!I||!I.type||!I.type.startsWith("image/")||I.type==="image/svg+xml"||I.type==="image/gif"||I.size<=250*1024?I:new Promise(ne=>{const W=new Image,Te=new FileReader;Te.onload=_=>{W.onload=()=>{try{let N=W.naturalWidth||W.width,R=W.naturalHeight||W.height;(N>X||R>X)&&(N>R?(R=Math.round(R*X/N),N=X):(N=Math.round(N*X/R),R=X));const h=document.createElement("canvas");h.width=N,h.height=R,h.getContext("2d").drawImage(W,0,0,N,R);const J=I.type==="image/png"&&I.size<1024*1024?"image/png":"image/jpeg";h.toBlob(q=>{if(q&&q.size<I.size){const se=J==="image/png"?".png":".jpg",U=(I.name||"image").replace(/\.[^/.]+$/,""),V=new File([q],`${U}${se}`,{type:J,lastModified:Date.now()});console.log(`[PawTrack Storage] Compressed ${I.name} (${(I.size/1024).toFixed(1)} KB -> ${(V.size/1024).toFixed(1)} KB)`),ne(V)}else ne(I)},J,ue)}catch(N){console.warn("[PawTrack Storage] Canvas compression bypassed:",N),ne(I)}},W.onerror=()=>ne(I),W.src=_.target.result},Te.onerror=()=>ne(I),Te.readAsDataURL(I)})}document.addEventListener("DOMContentLoaded",async()=>{var xt;const I=document.getElementById("btnThemeToggle"),X=document.querySelector('meta[name="color-scheme"]');function ue(e){document.documentElement.setAttribute("data-theme",e),X&&(X.content=e),I&&(I.setAttribute("title",e==="dark"?"Switch to Light Mode":"Switch to Dark Mode"),I.setAttribute("aria-label",e==="dark"?"Switch to Light Mode":"Switch to Dark Mode"))}const ne=localStorage.getItem("pawtrack_theme"),W=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches;ue(ne||(W?"dark":"light")),I&&I.addEventListener("click",()=>{const a=(document.documentElement.getAttribute("data-theme")||"light")==="dark"?"light":"dark";localStorage.setItem("pawtrack_theme",a),ue(a)}),window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",e=>{localStorage.getItem("pawtrack_theme")||ue(e.matches?"dark":"light")});try{const e=await me.get();A=e.name,Qe=e.email,F=e.$id,S=e.prefs||{},de=((xt=e.prefs)==null?void 0:xt.phone)||"";const a=document.getElementById("userNameDisplay");if(a){const t=S.username||A;a.innerText=t?"Welcome, "+t+"!":"Welcome!"}try{x=(await D.listDocuments(M,ie)).documents.map(s=>({...s,id:s.$id})),k=(await D.listDocuments(M,le,[re.equal("user_id",F)])).documents.map(s=>({...s,id:s.$id})),O=(await D.listDocuments(M,Ze,[re.equal("user_id",F)])).documents.map(s=>({...s,id:s.$id})),Y=(await D.listDocuments(M,De,[re.equal("owner",A)])).documents.map(s=>({...s,id:s.$id}))}catch(t){console.warn("Database collections not fully setup yet. Using empty arrays.",t)}}catch(e){console.error("User not logged in",e),window.location.href="/PawTrackLogin.html";return}async function _(e,a,t){try{await D.createDocument(M,Bt,ae.unique(),{user_id:F,action:e||"Activity recorded",target:a||"",icon:t||"fa-paw",timestamp:Date.now().toString()})}catch(i){console.warn("Failed to persist activity log to Appwrite:",i)}}function N(){document.body.style.zoom="1",document.body.style.width="100%",document.body.style.height="100%",navigator.userAgent.toLowerCase().includes("firefox")&&(document.body.style.transform="none")}let R;function h(e,a,t=!1,i=null){const r=document.getElementById("customPopupOverlay"),n=document.getElementById("customPopupTitle"),s=document.getElementById("customPopupMessage"),o=document.getElementById("customPopupActions");n.innerText=e,s.innerText=a,n.className=t?"custom-popup-title title-error":"custom-popup-title title-success",o.style.display="flex";const l=document.getElementById("btnPopupCancel"),p=document.getElementById("btnPopupConfirm");l.style.display="none",p.innerText="Okay",p.style.background=t?"#ef4444":"var(--sage, #2D6A4F)",p.style.boxShadow=t?"0 4px 10px rgba(239,68,68,0.3)":"0 4px 10px rgba(45,106,79,0.3)",p.onclick=()=>{clearTimeout(R),l.style.display="",p.innerText="Yes, I'm sure",p.style.background="#ef4444",p.style.boxShadow="0 4px 10px rgba(239,68,68,0.3)",q(i)},r.style.display="flex",clearTimeout(R),R=setTimeout(()=>{q(i)},8e3),document.getElementById("customPopupClose").onclick=()=>{clearTimeout(R),l.style.display="",p.innerText="Yes, I'm sure",p.style.background="#ef4444",p.style.boxShadow="0 4px 10px rgba(239,68,68,0.3)",q(i)}}function _e(e,a,t){const i=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),s=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-success",n.innerHTML=`<p style="margin-bottom:10px;">${a}</p><input type="text" id="customPromptInput" style="width:100%; padding:12px; border-radius:8px; border:2px solid #cbd5e1; outline:none; font-size:1rem; font-weight:600; color:#0f172a;" autocomplete="off">`,s.style.display="flex",i.style.display="flex",clearTimeout(R),document.getElementById("btnPopupConfirm").innerText="Submit",document.getElementById("btnPopupConfirm").onclick=()=>{const o=document.getElementById("customPromptInput").value;q(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",o.trim()!==""&&t(o.trim())},document.getElementById("customPopupClose").onclick=()=>{q()},document.getElementById("btnPopupCancel").onclick=()=>{q(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure"}}function J(e,a,t){const i=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),s=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-error",n.innerHTML=`<p>${a}</p>`,s.style.display="flex",i.style.display="flex",clearTimeout(R),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",document.getElementById("btnPopupConfirm").onclick=()=>{q(),t&&t()},document.getElementById("customPopupClose").onclick=()=>{q()},document.getElementById("btnPopupCancel").onclick=()=>{q()}}function q(e){document.getElementById("customPopupOverlay").style.display="none",e&&e()}window.addEventListener("resize",N),N();const se=[{initials:"MD",name:"Dr. Miguel Antonio Dela Cruz",specialty:"General Veterinary Practitioner",phone:"0917-555-0101",email:"mdelacruz@pawtrack.ph",schedule:"Mon-Fri: 8am-4pm",clinic:"Quezon City Main Clinic",exp:"15 yrs exp",rating:"4.8"},{initials:"JS",name:"Dr. Joanna Marie R. Santos",specialty:"Veterinary Surgeon",phone:"0917-555-0102",email:"jmsantos@pawtrack.ph",schedule:"Tue-Sat: 10am-6pm",clinic:"Makati Pet Hospital",exp:"12 yrs exp",rating:"4.9"},{initials:"PV",name:"Dr. Paulo C. Villanueva",specialty:"Veterinary Oncologist",phone:"0917-555-0103",email:"pvillanueva@pawtrack.ph",schedule:"Mon-Thu: 9am-5pm",clinic:"BGC Animal Center",exp:"8 yrs exp",rating:"4.7"}];let U=0,V=[],ce=[],Q=0,K=null,pe=[],Re=3;const ee=document.querySelectorAll(".sidebar-nav .nav-btn"),v=document.getElementById("mainDisplayPanel"),tt=document.getElementById("profileTemplate").innerHTML,Ne=document.getElementById("cartItemsContainer"),kt=document.getElementById("cartTotalDisplay"),ge=(e,a,t)=>`
    ...
    `;function ze(){let e="";typeof O<"u"&&O.length>0?O.forEach(n=>{const s=n.status==="Approved",o=s?"appt-approved":"appt-pending",l=s?"badge-approved":"badge-pending",p=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],m=n.date.split("-"),g=m.length===3?p[parseInt(m[1])-1]:"TBD",f=m.length===3?m[2]:"??";e+=`
    ...
    `;function Be(){const e=x.filter(t=>t.owner===A&&t.status==="Private");let a="";return e.length>0?e.forEach(t=>{const i=t.gender==="Female"?"gender-female":"gender-male",r=t.img||"https://ui-avatars.com/api/?name=Pet&background=e2e8f0&color=64748b";a+=`
        ...
        `,Promise.resolve({status:"success",matches:Ee}).then(o=>{const l=document.getElementById("homeMatchAlertsContainer");if(l)if(o.status==="success"&&o.matches.length>0){let p="";o.matches.slice(0,3).forEach(m=>{let g=m.status==="approved"?"Approved Match!":"Pending Request",f=m.status==="approved"?"💌":"⏳";p+=`
                        <div class="match-alert-item" style="cursor:pointer;" onclick="document.querySelector('[data-target=\\'breeding\\']').click()">
                            ...
                            <div class="alert-text"><strong>${m.their_pet_name} ${f}</strong><span>${g}</span></div>
                            <button class="btn-small-view">View</button>
                        </div>
                    `}),l.innerHTML=p}else l.innerHTML='<p style="text-align:center; color:#64748b; font-size:0.85rem; margin-top:20px;">No recent match activity.</p>'}),fetch("https://api.open-meteo.com/v1/forecast?latitude=14.6488&longitude=121.0509&current_weather=true").then(o=>o.json()).then(o=>{const l=Math.round(o.current_weather.temperature),p=o.current_weather.weathercode,m=document.getElementById("weatherTemp"),g=document.getElementById("weatherIcon"),f=document.getElementById("weatherText");m&&g&&f&&(m.innerText=`${l}°C`,p<=3?(g.className="fa-solid fa-sun weather-icon",g.style.color="#fef08a",f.innerHTML="<strong>Perfect time for a walk!</strong><br>The weather is great today. Take your pets out before it gets too hot."):p>=51&&p<=67||p>=80?(g.className="fa-solid fa-cloud-rain weather-icon",g.style.color="#bae6fd",f.innerHTML="<strong>It's a bit rainy!</strong><br>Keep your pets dry indoors today. A good time for some indoor training!"):(g.className="fa-solid fa-cloud weather-icon",g.style.color="#e2e8f0",f.innerHTML="<strong>Nice and cool!</strong><br>A great day for some indoor play or a quick stroll around the neighborhood."))}).catch(o=>console.error("Weather fetch error:",o)),setTimeout(()=>{const o=document.querySelector(".btn-adopt-potd");o&&o.addEventListener("click",l=>{const p=l.target.getAttribute("data-petid"),m=l.target.getAttribute("data-petname");v.innerHTML=at(p,m),document.querySelectorAll(".sidebar-nav .nav-btn").forEach(g=>g.classList.remove("active")),document.querySelector('[data-target="pets"]').classList.add("active")})},100)}he();function Lt(){let e=document.getElementById("pawtrackToastContainer");return e||(e=document.createElement("div"),e.id="pawtrackToastContainer",e.className="pawtrack-toast-container",document.body.appendChild(e)),e}function te(e,a,t="fa-paw",i=null,r="default"){const n=Lt(),s=document.createElement("div");s.className=`pawtrack-toast-card ${r==="app"?"toast-app":r==="info"?"toast-info":""}`,s.innerHTML=`
        ...
        `;const o=s.querySelector(".toast-close");let l=null;const p=m=>{m&&m.stopPropagation(),l&&clearTimeout(l),s.style.animation="toastSlideOut 0.25s forwards",setTimeout(()=>{s.parentElement&&s.parentElement.removeChild(s)},250)};o.addEventListener("click",p),i&&s.addEventListener("click",()=>{p();const m=document.querySelector(`.sidebar-nav .nav-btn[data-target="${i}"]`);if(m)m.click();else if(i==="pets"){ee.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="pets"]');g&&g.classList.add("active"),v.innerHTML=fe()}else if(i==="applications"){ee.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="applications"]');g&&g.classList.add("active"),v.innerHTML=ve()}}),n.appendChild(s),l=setTimeout(p,7e3)}function St(){if(document.getElementById("adoptionApplicationForm"))return"adopt-form";if(document.getElementById("petRegistrationForm")||document.getElementById("registerPetForm"))return"register-pet";if(document.getElementById("profileFullName")||document.querySelector(".profile-container"))return"profile";const e=document.querySelector(".sidebar-nav .nav-btn.active");return e?e.getAttribute("data-target"):"home"}function ye(){const e=St();e==="home"?he():e==="pets"?v.innerHTML=fe():e==="applications"?v.innerHTML=ve():e==="profile"?Pe():e==="breeding"&&typeof Be=="function"&&!document.getElementById("matchDetailsOverlay")&&(v.innerHTML=Be())}function $t(e){if(!e||!e.payload)return;const a=e.events||[],t=e.payload,i=a.some(s=>s.includes(".create")),r=a.some(s=>s.includes(".update")),n=a.some(s=>s.includes(".delete"));if(t.$collectionId===ie||a.some(s=>s.includes(`.${ie}.`))){const s=t.$id,o={...t,id:t.$id};if(n){const l=x.find(p=>p.id===s);x=x.filter(p=>p.id!==s),l&&l.status==="Available"&&l.owner!==A&&te("Pet Removed",`${l.name} is no longer listed for adoption.`,"fa-paw","pets","info")}else if(i){const l=x.findIndex(p=>p.id===s);l===-1?x.unshift(o):x[l]=o,o.status==="Available"&&o.owner!==A&&te("New Pet for Adoption! 🐾",`@${o.owner||"A caregiver"} just listed ${o.name} (${o.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}else if(r){const l=x.findIndex(m=>m.id===s),p=l!==-1?x[l]:null;l!==-1?x[l]=o:x.unshift(o),o.owner!==A&&(p&&p.status!=="Available"&&o.status==="Available"?te("Pet Available for Adoption! 🐾",`${o.name} (${o.breed}) is now available for adoption! Tap to view.`,"fa-heart","pets","default"):p&&p.status==="Available"&&o.status!=="Available"&&te("Pet Status Updated",`${o.name} is now ${o.status}.`,"fa-paw","pets","info"))}ye()}if(t.$collectionId===le||a.some(s=>s.includes(`.${le}.`))){const s=t.$id,o={...t,id:t.$id};if(o.user_id===F){if(n)k=k.filter(l=>l.id!==s);else if(i){const l=k.findIndex(p=>p.id===s);l===-1?k.unshift(o):k[l]=o}else if(r){const l=k.findIndex(m=>m.id===s),p=l!==-1?k[l].status:"";if(l!==-1?k[l]=o:k.unshift(o),p!==o.status){const m=o.status==="Approved";te(m?"🎉 Application Approved!":"Application Status Updated",`Your adoption application for ${o.pet_name} is now: ${o.status}!`,m?"fa-circle-check":"fa-clock","applications","app")}}ye()}}}let Oe=!1;async function At(){if(!Oe){Oe=!0;try{const a=(await D.listDocuments(M,ie)).documents.map(n=>({...n,id:n.$id})),t=new Set(x.map(n=>n.id)),i=a.filter(n=>!t.has(n.id)&&n.status==="Available"&&n.owner!==A);if(a.length!==x.length||a.some((n,s)=>{var o,l;return((o=x[s])==null?void 0:o.id)!==n.id||((l=x[s])==null?void 0:l.status)!==n.status})){if(x=a,i.length>0){const n=i[0];te("New Pet for Adoption! 🐾",`@${n.owner||"A caregiver"} just listed ${n.name} (${n.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}ye()}if(F){const s=(await D.listDocuments(M,le,[re.equal("user_id",F)])).documents.map(l=>({...l,id:l.$id}));(s.length!==k.length||s.some((l,p)=>{var m,g;return((m=k[p])==null?void 0:m.id)!==l.id||((g=k[p])==null?void 0:g.status)!==l.status}))&&(k=s,ye())}}catch{}finally{Oe=!1}}}function Mt(){try{if(Ke&&typeof Ke.subscribe=="function"){const e=`databases.${M}.collections.${ie}.documents`,a=`databases.${M}.collections.${le}.documents`;Ke.subscribe([e,a],t=>{$t(t)}),console.log("PawTrack: Appwrite Realtime connected for live adoption sync.")}}catch(e){console.warn("Realtime subscription fallback to polling:",e)}setInterval(At,8e3)}Mt();function oe(){const e=se[U],a=document.getElementById("vetInitials");if(!a)return;a.innerText=e.initials,document.getElementById("vetNameDisplay").innerText=e.name,document.getElementById("vetSpecialtyDisplay").innerText=e.specialty,document.getElementById("vetRatingDisplay").innerText=`⭐ ${e.rating} (${e.exp})`,document.getElementById("vetPhoneDisplay").innerText=e.phone,document.getElementById("vetEmailDisplay").innerText=e.email,document.getElementById("vetScheduleDisplay").innerText=e.schedule,document.getElementById("vetClinicDisplay").innerText=e.clinic;const t=document.getElementById("apptVetIdHidden").value,i=document.getElementById("btnSelectVet");t===U.toString()?(i.innerText="✅ Confirmed",i.classList.add("selected")):(i.innerText="Confirm",i.classList.remove("selected"))}function ke(e){if(!e)return;const a=document.querySelector(`.pet-select-card[data-petid="${e}"]`)||document.querySelector(`.pet-select-card[data-pet="${e}"]`);document.querySelectorAll(".pet-select-card").forEach(r=>r.classList.remove("selected")),a&&a.classList.add("selected");const t=document.getElementById("apptPetSelectorHidden");t&&(t.value=e);const i=x.find(r=>{var n;return((n=r.id)==null?void 0:n.toString())===(e==null?void 0:e.toString())});if(i){const r=document.getElementById("apptOwner");r&&(r.value=i.owner||A||"");const n=document.getElementById("apptContact");n&&(n.value=i.contact_number||de||S&&S.phone||"");const s=document.getElementById("apptPetType");if(s){const m=(i.breed||"").toLowerCase(),g=(i.type||"").toLowerCase(),f=m.includes("cat")||m.includes("kitten")||m.includes("feline")||m.includes("persian")||m.includes("siamese")||g.includes("cat");s.value=i.type||(f?"Cat":"Dog")}const o=document.getElementById("apptBreed");o&&(o.value=i.breed||"Unknown");const l=document.getElementById("apptGender");l&&(l.value=i.gender||"Unknown");const p=document.getElementById("apptWeight");p&&(p.value=i.weight||"")}}function be(){const e=document.getElementById("cartBadge");e&&(e.innerText=V.length),Ne.innerHTML="";let a=0;V.length===0?Ne.innerHTML='<p style="text-align:center; color:#64748b; font-weight:bold; margin-top: 50px;">Your cart is empty.</p>':V.forEach((t,i)=>{a+=parseFloat(t.price),Ne.innerHTML+=`
                ...
                `}),kt.innerText=`₱ ${a.toFixed(2)}`}window.handleRewind=e=>{pe.splice(e,1),Q=Math.max(0,Q-1),je()};function Ht(){const e=document.getElementById("rewindList");e&&(e.innerHTML="",pe.length===0?e.innerHTML=`
                ...
                `}))}function je(){if(ce.length===0||Q>=ce.length){document.getElementById("datingContentLayout").style.display="none";const s=document.getElementById("instructionOverlay");s.style.display="flex",s.innerHTML='<i class="fa-solid fa-face-frown-open" style="color: #cbd5e1; font-size: 4rem; margin-bottom: 15px;"></i><p>No more matches available<br>for this pet at the moment.</p>';return}const e=ce[Q];currentPhotoIndex=0;const a=document.getElementById("candThumbnails");a.innerHTML="",e.imgs.forEach((s,o)=>{a.innerHTML+=`<img src="${s}" class="cand-thumb ${o===0?"active":""}" data-index="${o}" alt="Photo ${o+1}">`}),st(e,0);const t=e.gender==="Male"?'<i class="fa-solid fa-mars" style="color: #60a5fa;"></i>':'<i class="fa-solid fa-venus" style="color: #f472b6;"></i>';document.getElementById("candNameAge").innerHTML=`${e.name}, ${e.age} ${t}`,document.getElementById("candBreedGender").innerHTML=`<i class="fa-solid fa-paw"></i> ${e.breed}`,document.getElementById("candDesc").innerText=e.desc,document.getElementById("candScoreText").innerText=`${e.score}% Match`,document.getElementById("candScoreBar").style.width=`${e.score}%`;const i=document.getElementById("candBadges");i.innerHTML="",e.badges.includes("vet")&&(i.innerHTML+='<span class="v-badge badge-vet"><i class="fa-solid fa-stethoscope"></i> Vet Verified</span>'),e.badges.includes("pedigree")&&(i.innerHTML+='<span class="v-badge badge-pedigree"><i class="fa-solid fa-dna"></i> Purebred</span>'),e.badges.includes("vax")&&(i.innerHTML+='<span class="v-badge badge-vax"><i class="fa-solid fa-syringe"></i> Vaccinated</span>');const r=document.getElementById("candTraits");r.innerHTML="",e.traits.forEach(s=>{r.innerHTML+=`<span class="trait-tag">${s}</span>`}),document.getElementById("candOwnerName").innerText=e.ownerName,document.getElementById("candOwnerInitial").innerText=e.ownerInitial,document.getElementById("candOwnerRating").innerText=`⭐ ${e.ownerRating}`,document.getElementById("candOwnerPairs").innerText=`${e.successPairs} Successful Pairs`,document.getElementById("barSize").style.width=`${e.compSize}%`,document.getElementById("txtSize").innerText=`${e.compSize}%`,document.getElementById("barEnergy").style.width=`${e.compEnergy}%`,document.getElementById("txtEnergy").innerText=`${e.compEnergy}%`,document.getElementById("barTemp").style.width=`${e.compTemp}%`,document.getElementById("txtTemp").innerText=`${e.compTemp}%`;const n=document.getElementById("litterPredictor");n.innerHTML="",e.litter.forEach(s=>{const o=s.c==="#FFFFFF"||s.c==="#FFF8DC"?"#0f172a":"white";n.innerHTML+=`<div class="color-swatch" style="background:${s.c}; color:${o};">${s.p}%</div>`}),document.getElementById("instructionOverlay").style.display="none",document.getElementById("datingContentLayout").style.display="grid",document.getElementById("treatCountDisplay").innerText=`${Re} Left`,Ht()}function st(e,a){const t=document.getElementById("candImg");t.style.opacity=0,setTimeout(()=>{t.src=e.imgs[a],t.style.opacity=1},150),document.querySelectorAll(".cand-thumb").forEach((i,r)=>{r===parseInt(a)?i.classList.add("active"):i.classList.remove("active")})}ee.forEach(e=>{e.addEventListener("click",function(){ee.forEach(t=>t.classList.remove("active")),this.classList.add("active");const a=this.getAttribute("data-target");if(v.scrollTo({top:0,behavior:"smooth"}),a==="home")he();else if(a==="pets")v.innerHTML=fe();else if(a==="applications")v.innerHTML=ve();else if(a==="vet"){v.innerHTML=ze(),oe();const t=document.querySelector(".pet-select-card");t&&ke(t.getAttribute("data-petid")||t.getAttribute("data-pet"))}else a==="shop"?(v.innerHTML=Pt(),be()):a==="breeding"?(v.innerHTML=Be(),K=null):a==="messages"&&(v.innerHTML=Ve(),We())})}),document.getElementById("btnViewProfile").addEventListener("click",()=>{ee.forEach(e=>e.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=tt,Pe()}),document.getElementById("btnRegisterPet").addEventListener("click",()=>{ee.forEach(r=>r.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=Ct;const e=document.getElementById("imageDropZone"),a=document.getElementById("petImageInput"),t=document.getElementById("imagePreview"),i=document.getElementById("dropZoneText");if(e&&a){e.addEventListener("click",()=>a.click());const r=n=>{if(!n)return;if(!n.type||!n.type.startsWith("image/")){h("Invalid File","Please choose a photo image file (PNG, JPG, WebP, etc.).",!0);return}et=n;const s=new FileReader;s.onload=function(o){t&&(t.src=o.target.result,t.style.display="block"),i&&(i.style.display="none")},s.readAsDataURL(n)};["dragenter","dragover"].forEach(n=>{e.addEventListener(n,s=>{s.preventDefault(),s.stopPropagation(),e.classList.add("drag-over")})}),["dragleave","drop"].forEach(n=>{e.addEventListener(n,s=>{s.preventDefault(),s.stopPropagation(),e.classList.remove("drag-over")})}),e.addEventListener("drop",n=>{n.dataTransfer&&n.dataTransfer.files&&n.dataTransfer.files.length>0&&r(n.dataTransfer.files[0])}),et=null,a.addEventListener("change",function(){this.files&&this.files[0]&&r(this.files[0])})}document.getElementById("btnCancelReg").addEventListener("click",()=>{he(),ee[0].classList.add("active")})}),v.addEventListener("click",async e=>{if(e.target.closest("#btnMatchRegisterPet")){document.getElementById("btnRegisterPet").click();return}if(e.target.classList.contains("profile-tab-btn")){document.querySelectorAll(".profile-tab-btn").forEach(c=>c.classList.remove("active")),document.querySelectorAll(".profile-tab-content").forEach(c=>c.classList.remove("active")),e.target.classList.add("active");const d=e.target.getAttribute("data-tab");document.getElementById(d).classList.add("active")}const t=e.target.closest(".btn-view-app")||e.target.closest(".btn-view-pet");if(t){const d=t.getAttribute("data-target"),c=document.getElementById(d),u=t.closest(".app-card")||t.closest(".pet-card");c.style.display==="block"?(c.style.display="none",u.classList.remove("dropdown-open")):(c.style.display="block",u.classList.add("dropdown-open"))}const i=e.target.closest(".btn-close-pet");if(i){const d=i.getAttribute("data-target"),c=document.getElementById(d),u=c.previousElementSibling;c.style.display="none",u.classList.remove("dropdown-open")}const r=e.target.closest(".btn-adopt");if(r){const d=r.getAttribute("data-petid"),c=r.getAttribute("data-petname");v.innerHTML=at(d,c)}e.target.closest("#btnBackToPets")&&(v.innerHTML=fe()),e.target.closest(".btn-edit-profile")&&(document.getElementById("editProfileModal").style.display="flex");const n=e.target.closest(".btn-archive-pet");if(n){const d=n.getAttribute("data-petid");J("Move to Bin?","Are you sure you want to move this pet to the Recycle Bin?",async()=>{const c=x.find(u=>u.id===d);if(!c){h("Error","Pet record not found.",!0);return}try{const u=await D.createDocument(M,De,ae.unique(),{name:c.name,breed:c.breed,gender:c.gender,owner:A,img:c.img||""});await D.deleteDocument(M,ie,d),x=x.filter(E=>E.id!==d),Y.unshift({...u,id:u.$id}),await _("Moved a pet to the Recycle Bin",c.name,"fa-trash-can"),h("Moved to Bin","Pet successfully moved to Recycle Bin.",!1,()=>{ye()})}catch(u){console.error("Appwrite Move to Bin Error:",u),h("Error",u.message,!0)}})}e.target.closest("#btnViewBin")&&(v.innerHTML=qe()),e.target.closest("#btnEmptyBin")&&J("Empty Bin?","WARNING: Are you sure you want to permanently delete ALL pets in the Recycle Bin? This cannot be undone!",async()=>{try{for(const d of Y)await D.deleteDocument(M,De,d.id);Y=[],await _("Permanently emptied the Recycle Bin","","fa-dumpster-fire"),h("Bin Emptied","Recycle bin emptied successfully.",!1,()=>{v.innerHTML=qe()})}catch(d){console.error("Appwrite Empty Bin Error:",d),h("Error",d.message,!0)}}),e.target.closest("#btnBackToProfile")&&(v.innerHTML=tt,Pe());const s=e.target.closest(".btn-restore-pet");if(s){const d=s.getAttribute("data-petid"),c=Y.find(u=>u.id===d);if(c)try{const u=await D.createDocument(M,ie,ae.unique(),{name:c.name,breed:c.breed,gender:c.gender,age:"1 yr",status:"Available",health_status:"Healthy",owner:A,contact_number:de||"0917-000-0000",personal_traits:"Friendly",reason_for_adoption:"Restored from Recycle Bin",img:c.img||""});await D.deleteDocument(M,De,d),Y=Y.filter(E=>E.id!==d),x.unshift({...u,id:u.$id}),await _("Restored a pet from the Recycle Bin",c.name,"fa-rotate-left"),h("Restored!","Pet has been restored to the active board!",!1,()=>{v.innerHTML=qe()})}catch(u){console.error("Appwrite Restore Error:",u),h("Error",u.message,!0)}}const o=e.target.closest(".btn-cancel-app");if(o){const d=o.getAttribute("data-appid");J("Cancel Application?","Are you sure you want to cancel this application?",async()=>{try{await D.deleteDocument(M,le,d),k=k.filter(c=>c.id!==d),await _("Cancelled adoption application","","fa-file-circle-xmark"),h("Cancelled","Application successfully cancelled.",!1,()=>{v.innerHTML=ve()})}catch(c){console.error("Appwrite Cancel App Error:",c),h("Error",c.message,!0)}})}const l=e.target.closest(".btn-cancel-vet");if(l){const d=l.getAttribute("data-appid");J("Cancel Appointment?","Are you sure you want to cancel this veterinary appointment?",async()=>{try{await D.deleteDocument(M,Ze,d),O=O.filter(c=>c.id!==d),await _("Cancelled veterinary appointment","","fa-calendar-xmark"),h("Cancelled","Appointment successfully cancelled.",!1,()=>{v.innerHTML=ze(),oe();const c=document.querySelector(".pet-select-card");c&&ke(c.getAttribute("data-petid")||c.getAttribute("data-pet"))})}catch(c){console.error("Appwrite Cancel Vet Error:",c),h("Error",c.message,!0)}})}const p=e.target.closest(".pet-select-card");if(p&&(document.getElementById("apptPetSelectorHidden")||document.getElementById("vetBookingForm"))){const d=p.getAttribute("data-petid")||p.getAttribute("data-pet");ke(d)}if(e.target.closest("#btnPrevVet")&&(U=(U-1+se.length)%se.length,oe()),e.target.closest("#btnNextVet")&&(U=(U+1)%se.length,oe()),e.target.closest("#btnSelectVet")){const d=se[U];document.getElementById("apptVetIdHidden").value=U,document.getElementById("apptSelectedVetName").value=d.name,oe()}if(e.target.classList.contains("btn-add-cart")){V.push({name:e.target.getAttribute("data-name"),price:e.target.getAttribute("data-price"),img:e.target.getAttribute("data-img")}),be();const d=e.target.innerText;e.target.innerText="✓ Added",e.target.style.background="#10b981",e.target.style.color="white",e.target.style.borderColor="#10b981",setTimeout(()=>{e.target.innerText=d,e.target.style.background="white",e.target.style.color="#4f46e5",e.target.style.borderColor="#4f46e5"},1e3)}if(e.target.classList.contains("btn-buy-now")){const c=e.target.closest(".shop-card").querySelector(".btn-add-cart");V.push({name:c.getAttribute("data-name"),price:c.getAttribute("data-price"),img:c.getAttribute("data-img")}),be(),document.getElementById("cartModalOverlay").style.display="flex"}e.target.closest("#btnOpenCart")&&(document.getElementById("cartModalOverlay").style.display="flex"),e.target.closest(".mode-btn")&&(document.querySelectorAll(".mode-btn").forEach(d=>d.classList.remove("active")),e.target.closest(".mode-btn").classList.add("active"),K&&document.querySelector(`[data-pet="${K}"]`).click());const m=e.target.closest(".my-pet-card");if(m&&!document.getElementById("apptPetSelectorHidden")){document.querySelectorAll(".my-pet-card").forEach(c=>c.classList.remove("active")),m.classList.add("active"),K=m.getAttribute("data-petid"),x.find(c=>c.id.toString()===K),document.getElementById("datingContentLayout").style.display="none";const d=document.getElementById("instructionOverlay");d.style.display="flex",d.innerHTML='<div class="radar-container"><div class="radar-ring ring1"></div><div class="radar-ring ring2"></div><div class="radar-ring ring3"></div><div class="radar-center"><i class="fa-solid fa-satellite-dish"></i></div></div><p>Scanning database for verified matches...</p>',Promise.resolve().then(()=>({status:"success",candidates:x.filter(u=>u.owner!==A).map(u=>({id:u.id,name:u.name,breed:u.breed,gender:u.gender,age:u.age,photos:[u.img],imgs:[u.img],owner:u.owner||"PawUser",ownerName:u.owner||"PawUser",score:94,traits:u.personal_traits?u.personal_traits.split(",").map(E=>E.trim()):["Playful","Friendly"],desc:u.reason_for_adoption||"Looking for a friend!",verified:!0}))})).then(c=>{c.status==="success"&&c.candidates.length>0?(ce=c.candidates,Q=0,setTimeout(()=>{je()},800)):d.innerHTML='<p style="color:#64748b; font-weight:bold; font-size:1.1rem;">No match candidates available right now. Invite friends or register more pets!</p>'})}const g=e.target.closest(".cand-thumb");if(g){const d=ce[Q];st(d,g.getAttribute("data-index"))}const f=e.target.closest("#btnPassCandidate"),y=e.target.closest("#btnMatchCandidate"),w=e.target.closest("#btnTreatCandidate");if(f||y||w){const d=ce[Q],c=x.find(E=>E.id.toString()===K);let u="pass";if(y&&(u="like"),w&&(u="super_like"),u==="super_like"&&Re<=0){h("Out of Treats","You have used all your Super Treats for today! Come back tomorrow.",!0);return}Promise.resolve({status:"success",isMatch:!0}).then(E=>{if(E.status==="success"){if(u==="super_like"&&Re--,pe.unshift(d),pe.length>5&&pe.pop(),u!=="pass"){_(u==="super_like"?"Sent a Super Treat to":"Sent a match request to",d.name,"fa-heart"),document.getElementById("matchImgLeft").src=c.img,document.getElementById("matchImgRight").src=d.imgs[0],document.getElementById("matchNameLeft").innerText=c.name,document.getElementById("matchNameRight").innerText=d.name;const H=E.isMatch?"✨ IT'S A MATCH! ✨":u==="super_like"?"Super Liked! 🦴 (Pending)":"Pending Owner Approval";document.getElementById("matchDateSpot").innerText="PawTrack Verified";const C=document.getElementById("matchIcebreaker");C.value=E.isMatch?"You matched! Let's plan a playdate!":"",C.placeholder=E.isMatch?"Type a message...":`Say hi to ${d.ownerName} while you wait...`,document.getElementById("btnContinueMatch").setAttribute("data-targetuser",d.ownerName),document.getElementById("matchOverlay").style.display="flex",activePairs.unshift({id:"p"+Date.now(),maleName:c.gender==="Male"?c.name:d.name,femaleName:c.gender==="Female"?c.name:d.name,maleImg:c.gender==="Male"?c.img:d.imgs[0],femaleImg:c.gender==="Female"?c.img:d.imgs[0],status:H,isApproved:E.isMatch,date:"Just Now",ownerUsername:d.ownerName})}Q++,je()}else h("Error",E.message,!0)}).catch(E=>console.error("Swipe Error:",E))}e.target.closest("#btnOpenPrefs")&&(document.getElementById("prefModal").style.display="flex"),e.target.closest("#btnViewActivePairs")&&Ie(),e.target.closest("#btnBackToMatch")&&(v.innerHTML=Be(),K=null);const T=e.target.closest(".btn-accept-match"),b=e.target.closest(".btn-delete-pair");if(T||b){const d=T?"accept":"delete",c=(T||b).getAttribute("data-matchid");if(d==="delete"){const u=b.getAttribute("data-title")||"Cancel Request",E=b.getAttribute("data-msg")||"Are you sure you want to remove this match?";J(u,E,()=>{Ee=Ee.filter(H=>H.id!==c),Ie()})}else{const u=Ee.find(E=>E.id===c);u&&(u.status="approved"),Ie()}}}),document.getElementById("btnCloseCart").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnContinueShopping").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnCheckout").addEventListener("click",()=>{if(V.length===0){h("Empty Cart","Your cart is empty! Please add some items first.",!0);return}_("Completed a Pet Shop checkout","","fa-bag-shopping"),V=[],be(),document.getElementById("cartModalOverlay").style.display="none",h("Order Placed!","Order placed successfully! Thank you for shopping.",!1)}),document.getElementById("cartItemsContainer").addEventListener("click",e=>{const a=e.target.closest(".btn-remove-item");a&&(V.splice(parseInt(a.getAttribute("data-index")),1),be())}),document.getElementById("btnContinueMatch").addEventListener("click",e=>{const a=e.target.getAttribute("data-targetuser"),t=document.getElementById("matchIcebreaker").value.trim();t&&a&&(P[a]||(P[a]=[]),P[a].push({text:t,type:"sent",time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}),_("Sent an Icebreaker message to",a,"fa-comment-dots")),document.getElementById("matchOverlay").style.display="none",Ie()}),document.getElementById("btnSavePrefs").addEventListener("click",()=>{document.getElementById("prefModal").style.display="none",K&&document.querySelector(`[data-pet="${K}"]`).click()}),document.getElementById("prefModal").addEventListener("click",e=>{e.target===document.getElementById("prefModal")&&(document.getElementById("prefModal").style.display="none")}),v.addEventListener("submit",e=>{if(e.target.id==="vetBookingForm"){e.preventDefault();const a=document.getElementById("apptPetSelectorHidden").value,t=document.getElementById("apptVetIdHidden").value;if(!a){h("Error","Please select a pet for the appointment by clicking their picture!",!0);return}if(!t){h("Error","Please choose a veterinarian by clicking 'Confirm' on the profile card!",!0);return}const i=x.find(o=>{var l;return((l=o.id)==null?void 0:l.toString())===(a==null?void 0:a.toString())}),r=document.querySelector(`.pet-select-card[data-petid="${a}"] span`)||document.querySelector(`.pet-select-card[data-pet="${a}"] span`),n=(i==null?void 0:i.name)||(r?r.innerText:"My Pet"),s=document.getElementById("apptSelectedVetName").value;(async()=>{try{const o=await D.createDocument(M,Ze,ae.unique(),{pet_name:n,vet_name:s,status:"Upcoming",time:document.getElementById("apptTime").value||"",date:document.getElementById("apptDate").value||"",user_id:F,img:(i==null?void 0:i.img)||""});O.unshift({...o,id:o.$id}),await _("Booked vet visit for",n,"fa-user-doctor"),h("Request Sent!","Appointment scheduled successfully! It is now Upcoming.",!1,()=>{const l=document.querySelector('[data-target="vet"]');if(l)l.click();else{v.innerHTML=ze(),oe();const p=document.querySelector(".pet-select-card");p&&ke(p.getAttribute("data-petid")||p.getAttribute("data-pet"))}})}catch(o){console.error("Appwrite Vet Booking Error:",o),h("Error",o.message,!0)}})()}if(e.target.id==="adoptionApplicationForm"){if(e.preventDefault(),!document.getElementById("adoptTerms").checked){h("Missing Requirement","You must agree to the terms and conditions.",!0);return}const a=document.getElementById("adoptPetId").value,t=document.getElementById("adoptPetName").value,i=x.find(r=>{var n;return((n=r.id)==null?void 0:n.toString())===(a==null?void 0:a.toString())});(async()=>{try{const r=await D.createDocument(M,le,ae.unique(),{pet_name:t,date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),status:"Pending Review",user_id:F,img:(i==null?void 0:i.img)||""});k.unshift({...r,id:r.$id}),await _("Applied to adopt",t,"fa-house-chimney-user"),h("Application Sent!","Successfully submitted! Status: PENDING REVIEW.",!1,()=>{const n=document.querySelector('[data-target="applications"]');n?n.click():v.innerHTML=ve()})}catch(r){console.error("Appwrite Adoption Application Error:",r),h("Error",r.message,!0)}})()}(e.target.id==="petRegistrationForm"||e.target.id==="registerPetForm")&&(e.preventDefault(),(async()=>{var l,p,m,g,f;const a=document.getElementById("petImageInput"),t=et||a&&a.files&&a.files[0];let i="";if(t)try{h("Uploading Photo","Optimizing and saving pet photo to PawTrack Storage...",!1);const y=await It(t,1600,.85),w=await He.createFile(Fe,ae.unique(),y);i=He.getFileView(Fe,w.$id).toString()}catch(y){console.error("Storage upload failed:",y)}i||(i="https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=600&q=80");const r=document.getElementById("regForAdoption"),n=r?r.checked:!0,s=new FormData(e.target),o={name:s.get("name")||((l=document.getElementById("regPetName"))==null?void 0:l.value)||"Unnamed Pet",breed:s.get("breed")||((p=document.getElementById("regBreed"))==null?void 0:p.value)||"Mixed Breed",gender:s.get("gender")||((m=document.getElementById("regGender"))==null?void 0:m.value)||"Male",age:s.get("age")||((g=document.getElementById("regAge"))==null?void 0:g.value)||"1 yr",status:n?"Available":"Private",health_status:s.get("health_status")||"Healthy / Vaccinated",owner:A,contact_number:s.get("contact_number")||de||"0917-000-0000",personal_traits:s.get("personal_traits")||((f=document.getElementById("regDesc"))==null?void 0:f.value)||"Friendly",reason_for_adoption:s.get("reason_for_adoption")||(n?"Looking for a home":"Personal pet"),img:i};try{const y=await D.createDocument(M,ie,ae.unique(),o),w={...y,id:y.$id};x.unshift(w),await _(n?"Listed pet for adoption":"Registered private pet",o.name,"fa-shield-cat"),h("Success!",n?"Pet Registered Successfully! Photo saved to PawTrack Storage and listed on the adoption board.":"Pet added to your personal roster! Photo saved to PawTrack Storage.",!1,()=>{if(n){const b=document.querySelector('[data-target="pets"]');b?b.click():v.innerHTML=fe()}else{const b=document.querySelector('[data-target="home"]');b?b.click():he()}})}catch(y){console.error("Appwrite Pet Registration Error:",y),h("Error","Could not register pet: "+y.message,!0)}})())}),v.addEventListener("reset",e=>{e.target.id==="vetBookingForm"&&(document.querySelectorAll(".pet-select-card").forEach(a=>a.classList.remove("selected")),document.getElementById("apptPetSelectorHidden")&&(document.getElementById("apptPetSelectorHidden").value=""),document.getElementById("apptVetIdHidden")&&(document.getElementById("apptVetIdHidden").value=""),setTimeout(()=>{oe()},10))});const ot=document.getElementById("editProfileForm");ot&&ot.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("editFirstName").value,t=document.getElementById("editLastName").value,i=document.getElementById("editContact").value,r=`${a} ${t}`.trim();try{await me.updatePrefs({...S,fullName:r,phone:i}),S.fullName=r,S.phone=i,de=i;const n=document.getElementById("userNameDisplay");if(n){const s=S.username||r;n.innerText=s?"Welcome, "+s+"!":"Welcome!"}await _("Updated profile settings & information","","fa-user-pen"),document.getElementById("editProfileModal").style.display="none",h("Profile Saved","Profile updated successfully!",!1,()=>{Pe()})}catch(n){h("Error",n.message,!0)}}),v.addEventListener("input",e=>{e.target.id==="shopPriceFilter"&&(document.getElementById("shopPriceDisplay").innerText=`₱${e.target.value}`,rt())}),v.addEventListener("change",e=>{e.target.id==="shopCategoryFilter"&&rt()});function rt(){const e=document.getElementById("shopCategoryFilter").value,a=parseFloat(document.getElementById("shopPriceFilter").value),t=it.filter(i=>{const r=e==="All Categories"||i.category===e,n=i.price<=a;return r&&n});document.getElementById("shopGridContainer").innerHTML=nt(t)}async function Pe(){const e=document.getElementById("mainProfilePic");e&&(e.src=S.avatarUrl||"/resources/avatar/Avatar 1.jpg",e.onerror=()=>{e.src="/resources/avatar/Avatar 1.jpg"});const a=document.getElementById("btnChangeAvatar")||document.querySelector(".btn-change-photo"),t=document.getElementById("avatarFileInput");a&&t&&(a.onclick=()=>t.click(),t.onchange=async function(){if(this.files&&this.files[0])try{h("Uploading Avatar","Optimizing and saving avatar to PawTrack Storage...",!1);const B=await It(this.files[0],800,.85),z=await He.createFile(Fe,ae.unique(),B),j=He.getFileView(Fe,z.$id).toString();await me.updatePrefs({...S,avatarUrl:j}),S.avatarUrl=j,e&&(e.src=j),await _("Updated profile picture","Saved to PawTrack Storage","fa-camera"),h("Success","Profile photo uploaded to PawTrack Storage!")}catch(B){console.error("Avatar storage upload failed:",B),h("Error","Failed to upload photo: "+(B.message||B),!0)}});const i=document.getElementById("profileFullName"),r=document.getElementById("profileUsername"),n=document.getElementById("profileEmail"),s=document.getElementById("profilePhone"),o=S.fullName||A,l=S.username||A;i&&(i.innerText=o),r&&(r.innerText="@"+l),n&&(n.innerText=Qe),s&&(s.innerText=de||"None");const[p="",...m]=(o||"").split(" "),g=m.join(" "),f=document.getElementById("editFirstName"),y=document.getElementById("editLastName"),w=document.getElementById("editContact"),T=document.getElementById("editEmail");f&&(f.value=p),y&&(y.value=g),w&&(w.value=de||""),T&&(T.value=Qe||"");const b=x.filter(B=>B.owner===A),d=k.filter(B=>B.status==="Approved").length,c=document.getElementById("countOwnedPets"),u=document.getElementById("countSuccessfulApps");c&&(c.innerText=b.length),u&&(u.innerText=d);const E=document.getElementById("privateRosterGrid"),H=document.getElementById("adoptionRosterGrid");E&&H&&(E.innerHTML="",H.innerHTML="",b.forEach(B=>{const z=`
                    <div class="roster-card">
                        <img src="${B.img||"https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=600&q=80"}" class="roster-img" onerror="this.onerror=null; this.src='https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=600&q=80';">
                ...
                `;B.status==="Private"?E.innerHTML+=z:H.innerHTML+=z})),await lt();const C=document.getElementById("btnRefreshActivityLogs");C&&(C.onclick=async()=>{C.classList.add("spinning"),C.disabled=!0,await lt(),setTimeout(()=>{C.classList.remove("spinning"),C.disabled=!1},450)})}async function lt(){const e=document.getElementById("recentActivityLogs");if(e){e.innerHTML='<div class="activity-loading"><i class="fa-solid fa-circle-notch fa-spin"></i> Loading activity logs...</div>';try{const a=await D.listDocuments(M,Bt,[re.equal("user_id",F),re.orderDesc("$createdAt"),re.limit(20)]);e.innerHTML="";const t=a.documents;t&&t.length>0?t.forEach(i=>{const r=Dt(i.timestamp,i.$createdAt),n=_t(r),s=Ft(i.action,i.icon),o=i.target?L(i.target):"",l=L(i.action||"Activity recorded");e.innerHTML+=`
        ...
        `}function We(e=null){const a=document.getElementById("messengerInboxList"),t=document.getElementById("messengerChatPanel"),i=document.getElementById("btnPageNewChat"),r=document.getElementById("messengerSearchInput");if(!a||!t)return;function n(m=""){a.innerHTML="";const g=Object.keys(P).filter(f=>f.toLowerCase().includes(m.toLowerCase()));if(g.length===0){a.innerHTML=`
                    <div class="inbox-empty-state">
                        <i class="fa-solid fa-comments"></i>
                        <p>No conversations found.<br>Click "New Conversation" to start chatting!</p>
                    </div>
                `;return}g.forEach(f=>{const y=P[f]||[],w=y.length>0?y[y.length-1].text:"No messages yet",T=y.length>0?y[y.length-1].time:"",b=f===$?"active":"",d=document.createElement("div");d.className=`inbox-item ${b}`,d.setAttribute("data-user",f),d.innerHTML=`
                ...
                `,d.onclick=()=>s(f),a.appendChild(d)})}function s(m){$=m,document.querySelectorAll(".inbox-item").forEach(b=>{b.classList.toggle("active",b.getAttribute("data-user")===m)});const g=P[m]||[];t.innerHTML=`
                ...
                <div class="chat-pane-messages" id="activeChatStream">
                    ${g.length===0?'<div class="empty-stream"><p>This is the start of your message history with @'+L(m)+"</p></div>":""}
                </div>

                <div class="chat-pane-input-bar">
                    <div class="chat-input-pill">
                        <input type="text" id="activeChatInput" placeholder="Write a message to @${L(m)}...">
                    </div>
                    <button class="btn-chat-send" id="btnActiveSend">
                        <i class="fa-solid fa-paper-plane"></i>
                    </button>
                </div>
            `;const f=document.getElementById("activeChatStream");g.forEach(b=>{o(f,b.text,b.type,b.time)}),f.scrollTop=f.scrollHeight;const y=document.getElementById("activeChatInput"),w=document.getElementById("btnActiveSend");function T(){const b=y.value.trim();if(!b||!$)return;const d=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});o(f,b,"sent",d),y.value="",f.scrollTop=f.scrollHeight,P[$]||(P[$]=[]),P[$].push({text:b,type:"sent",time:d}),n(r.value.trim()),$==="PawTrackCommunity"&&setTimeout(()=>{const c=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),u="We're glad to have you! Feel free to connect with pet owners across PawTrack.";P.PawTrackCommunity.push({text:u,type:"received",time:c}),$==="PawTrackCommunity"&&(o(f,u,"received",c),f.scrollTop=f.scrollHeight),n(r.value.trim())},1e3)}w.onclick=T,y.onkeypress=b=>{b.key==="Enter"&&T()},y.focus()}function o(m,g,f,y){const w=document.createElement("div");w.className=`stream-bubble-row row-${f}`,w.innerHTML=`
                <div class="stream-bubble bubble-${f}">
                    <div class="bubble-text">${L(g)}</div>
                    <div class="bubble-time">${L(y||"")}</div>
                </div>
            `,m.appendChild(w)}r.oninput=m=>{n(m.target.value.trim())},i.onclick=()=>{_e("Start Conversation","Enter the PawTrack username you'd like to message:",m=>{if(m&&m.trim()){const g=m.trim().replace("@","");P[g]||(P[g]=[]),n(),s(g)}})};const l=Object.keys(P),p=e||(l.length>0?l[0]:null);n(),p?s(p):t.innerHTML=`
            ...
            `}function Rt(){const e=document.getElementById("floatingChatWidget"),a=document.getElementById("floatingChatBubble"),t=document.getElementById("floatingChatWindow"),i=document.getElementById("floatingChatBody"),r=document.getElementById("btnFloatingChatBack"),n=document.getElementById("btnFloatingChatNew"),s=document.getElementById("btnFloatingChatExpand"),o=document.getElementById("btnFloatingChatClose"),l=document.getElementById("floatingChatHeaderTitle"),p=document.getElementById("floatingChatHeaderStatus"),m=document.getElementById("floatingChatHeaderAvatar"),g=document.getElementById("chatGlobalBadge");if(!e||!a||!t||!i)return;let f=!1;function y(d=0){g&&(d>0?(g.innerText=d>99?"99+":d,g.style.display="flex"):(g.innerText="0",g.style.display="none"))}function w(d=null){f=d!==null?d:!f,f?(e.classList.add("open"),t.setAttribute("aria-hidden","false"),y(0),$?b($):T()):(e.classList.remove("open"),t.setAttribute("aria-hidden","true"))}function T(d=""){r&&(r.style.display="none"),l&&(l.innerText="PawTrack Messenger"),p&&(p.innerHTML='<span class="status-dot-green"></span> Online & Active'),m&&(m.innerHTML='<i class="fa-solid fa-comments"></i>'),i.innerHTML=`
                <div class="floating-inbox-wrapper">
                    <div class="floating-inbox-search">
                        <i class="fa-solid fa-magnifying-glass"></i>
                        <input type="text" id="floatingInboxSearchInput" placeholder="Search conversations..." value="${L(d)}">
                    </div>
                    <div class="floating-inbox-list" id="floatingInboxList"></div>
                </div>
            `;const c=document.getElementById("floatingInboxList"),u=document.getElementById("floatingInboxSearchInput"),E=Object.keys(P).filter(H=>H.toLowerCase().includes(d.toLowerCase()));E.length===0?c.innerHTML=`
                    <div class="inbox-empty-state">
                        <i class="fa-solid fa-comments"></i>
                        <p>No conversations found.<br>Click <i class="fa-solid fa-pen-to-square"></i> above to start chatting!</p>
                    </div>
                `:E.forEach(H=>{const C=P[H]||[],B=C.length>0?C[C.length-1].text:"No messages yet",z=C.length>0?C[C.length-1].time:"",j=document.createElement("div");j.className="inbox-item",j.innerHTML=`
                        <div class="inbox-avatar">
                            <i class="fa-solid fa-user"></i>
                        </div>
                        <div class="inbox-info">
                            <div class="inbox-info-header">
                                <h4>@${L(H)}</h4>
                                <span class="inbox-time">${z}</span>
                            </div>
                            <p class="inbox-snippet">${L(B)}</p>
                        </div>
                    `,j.onclick=()=>b(H),c.appendChild(j)}),u&&(u.oninput=H=>{T(H.target.value.trim());const C=document.getElementById("floatingInboxSearchInput");C&&(C.focus(),C.selectionStart=C.selectionEnd=C.value.length)})}function b(d){$=d,r&&(r.style.display="inline-flex"),l&&(l.innerText=`@${d}`),p&&(p.innerHTML='<span class="status-dot-green"></span> Active Conversation'),m&&(m.innerHTML='<i class="fa-solid fa-user"></i>');const c=P[d]||[];i.innerHTML=`
                <div class="floating-chat-stream-wrapper">
                    <div class="floating-chat-stream" id="floatingActiveStream">
                        ${c.length===0?'<div class="empty-stream"><p>This is the start of your message history with @'+L(d)+"</p></div>":""}
                    </div>
                    <div class="floating-chat-input-bar">
                        <input type="text" id="floatingChatInput" placeholder="Message @${L(d)}...">
                        <button class="btn-floating-chat-send" id="btnFloatingSend" title="Send Message" aria-label="Send Message">
                            <i class="fa-solid fa-paper-plane"></i>
                        </button>
                    </div>
                </div>
            `;const u=document.getElementById("floatingActiveStream");c.forEach(B=>{const z=document.createElement("div");z.className=`stream-bubble-row row-${B.type}`,z.innerHTML=`
                    <div class="stream-bubble bubble-${B.type}">
                        <div class="bubble-text">${L(B.text)}</div>
                        <div class="bubble-time">${L(B.time||"")}</div>
                    </div>
                `,u.appendChild(z)}),u.scrollTop=u.scrollHeight;const E=document.getElementById("floatingChatInput"),H=document.getElementById("btnFloatingSend");function C(){const B=E.value.trim();if(!B||!$)return;const z=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),j=document.createElement("div");j.className="stream-bubble-row row-sent",j.innerHTML=`
                    <div class="stream-bubble bubble-sent">
                        <div class="bubble-text">${L(B)}</div>
                        <div class="bubble-time">${L(z)}</div>
                    </div>
                `,u.appendChild(j),E.value="",u.scrollTop=u.scrollHeight,P[$]||(P[$]=[]),P[$].push({text:B,type:"sent",time:z}),$==="PawTrackCommunity"&&setTimeout(()=>{const Et=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),Tt="We're glad to have you! Feel free to connect with pet owners across PawTrack.";if(P.PawTrackCommunity.push({text:Tt,type:"received",time:Et}),$==="PawTrackCommunity"&&document.getElementById("floatingActiveStream")){const Je=document.createElement("div");Je.className="stream-bubble-row row-received",Je.innerHTML=`
                                <div class="stream-bubble bubble-received">
                                    <div class="bubble-text">${L(Tt)}</div>
                                    <div class="bubble-time">${L(Et)}</div>
                                </div>
                            `;const Me=document.getElementById("floatingActiveStream");Me&&(Me.appendChild(Je),Me.scrollTop=Me.scrollHeight)}},800)}H&&(H.onclick=C),E&&(E.onkeypress=B=>{B.key==="Enter"&&C()},setTimeout(()=>E.focus(),150))}a.onclick=d=>{d.stopPropagation(),w()},o&&(o.onclick=d=>{d.stopPropagation(),w(!1)}),r&&(r.onclick=d=>{d.stopPropagation(),$=null,T()}),n&&(n.onclick=d=>{d.stopPropagation(),_e("Start Conversation","Enter the PawTrack username you'd like to message:",c=>{if(c&&c.trim()){const u=c.trim().replace("@","");P[u]||(P[u]=[]),b(u)}})}),s&&(s.onclick=d=>{d.stopPropagation(),w(!1),ee.forEach(c=>c.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=Ve(),We($)}),document.addEventListener("click",d=>{f&&!e.contains(d.target)&&w(!1)}),window.pawtrackOpenFloatingChat=function(d=null){w(!0),d?(P[d]||(P[d]=[]),b(d)):$?b($):T()}}v.addEventListener("click",e=>{const a=e.target.closest(".btn-message-owner");if(a){const t=a.getAttribute("data-owner");window.pawtrackOpenFloatingChat?window.pawtrackOpenFloatingChat(t):(v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=Ve(),We(t))}}),v.addEventListener("input",e=>{if(e.target.id==="radarDistanceRange"){const a=document.getElementById("radarDistanceValue");a&&(a.innerText=`${e.target.value} km`)}}),v.addEventListener("click",async e=>{var a,t,i,r,n;if(e.target.closest("#btnSaveAllSettings")){const s=((a=document.getElementById("notifVet"))==null?void 0:a.checked)??!0,o=((t=document.getElementById("notifMatch"))==null?void 0:t.checked)??!0,l=((i=document.getElementById("notifAdoption"))==null?void 0:i.checked)??!0,p=((r=document.getElementById("prefIncognito"))==null?void 0:r.checked)??!1,m=((n=document.getElementById("radarDistanceRange"))==null?void 0:n.value)||"25";try{await me.updatePrefs({...S,notifVet:s,notifMatch:o,notifAdoption:l,incognito:p,radarDistance:m}),S.radarDistance=m,S.incognito=p,await _("Updated notification & discovery preferences","","fa-sliders"),h("Settings Saved","Your notification, privacy, and discovery preferences have been updated successfully!")}catch(g){h("Error",g.message,!0)}}});const G=[{title:"Your Care Dashboard 🏠",desc:"Welcome to PawTrack! From your Home dashboard, track registered pet counts, see active adoption statuses, review upcoming vet reminders, and get daily outdoor walk weather tips.",icon:"fa-house",targetSelector:".sidebar-nav [data-target='home']",tab:"home",placement:"right"},{title:"Live Adoption Board 🐾",desc:"Browse companions looking for a loving home! When other users register a pet for adoption, it shows up here in real-time without reloading. Click 'Details' to view medical notes, or 'Adopt Now' to send a digital application.",icon:"fa-paw",targetSelector:".sidebar-nav [data-target='pets']",tab:"pets",placement:"right"},{title:"Register Your Own Pet 📸",desc:"Add your furry family member with photo uploads to PawTrack Cloud Storage. Choose whether your companion is a private pet for health tracking or listed publicly for adoption.",icon:"fa-plus-circle",targetSelector:"#btnRegisterPet",tab:null,placement:"bottom"},{title:"Pet Matchmaker & Playdates ❤️",desc:"Find compatible playmates or breeding matches based on breed, age, and location. Give a Treat to express interest, or pass to browse more verified neighborhood companions.",icon:"fa-heart",targetSelector:".sidebar-nav [data-target='breeding']",tab:"breeding",placement:"right"},{title:"Verified Vet Appointments 🩺",desc:"Book check-ups and medical visits with licensed veterinarians. Select your preferred clinic and time slot, and manage upcoming appointments in one place.",icon:"fa-user-doctor",targetSelector:".sidebar-nav [data-target='vet']",tab:"vet",placement:"right"},{title:"Real-Time Messages & Alerts 💬",desc:"Chat directly with pet owners, caregivers, and adopters anytime via your floating messenger bubble! Real-time notification toasts keep you updated the moment an application is approved or a new pet is posted!",icon:"fa-comment-dots",targetSelector:"#floatingChatBubble",tab:null,placement:"left"}];let Z=0,Ce=!1,we=null;function Ue(e){we&&we!==e&&we.classList.remove("tour-target-elevated"),e?(e.classList.add("tour-target-elevated"),we=e):we=null}function Le(e){const a=document.getElementById("pawtrackTourOverlay"),t=document.getElementById("tourSpotlightBox"),i=document.getElementById("tourCard");if(!a||!t||!i)return;let r=e.targetSelector?document.querySelector(e.targetSelector):null;if(!r||r.offsetParent===null){t.style.display="none",a.classList.add("no-target"),Ue(null),i.style.top="50%",i.style.left="50%",i.style.transform="translate(-50%, -50%)";return}a.classList.remove("no-target"),Ue(r),t.style.display="block";const n=r.getBoundingClientRect(),s=6,o=Math.max(0,n.top-s),l=Math.max(0,n.left-s),p=n.width+s*2,m=n.height+s*2;t.style.top=`${o}px`,t.style.left=`${l}px`,t.style.width=`${p}px`,t.style.height=`${m}px`;try{const b=window.getComputedStyle(r),d=parseFloat(b.borderRadius)||12;t.style.borderRadius=`${Math.max(10,d+4)}px`}catch{t.style.borderRadius="14px"}if(window.innerWidth<=768){i.style.top="",i.style.left="",i.style.transform="";return}i.style.transform="none";const g=390,f=i.offsetHeight||220,y=16;let w,T;e.placement==="right"?(T=n.right+y,w=Math.max(20,n.top+n.height/2-f/2),T+g>window.innerWidth-20&&(T=Math.max(20,n.left-g-y))):e.placement==="bottom"?(w=n.bottom+y,T=n.left+n.width/2-g/2,T+g>window.innerWidth-20&&(T=window.innerWidth-g-24),T<20&&(T=20),w+f>window.innerHeight-20&&(w=Math.max(20,n.top-f-y))):e.placement==="left"?(T=Math.max(20,n.left-g-y),w=Math.max(20,n.top+n.height/2-f/2),w+f>window.innerHeight-20&&(w=window.innerHeight-f-20)):(w=Math.max(20,window.innerHeight/2-f/2),T=Math.max(20,window.innerWidth/2-g/2)),w=Math.min(Math.max(20,w),window.innerHeight-f-20),T=Math.min(Math.max(20,T),window.innerWidth-g-20),i.style.top=`${w}px`,i.style.left=`${T}px`}function Ge(e){if(e<0||e>=G.length){Se();return}Z=e;const a=G[e];if(a.tab){const p=document.querySelector(`.sidebar-nav [data-target="${a.tab}"]`);p&&!p.classList.contains("active")&&p.click()}const t=document.getElementById("tourStepBadge"),i=document.getElementById("tourStepTitle"),r=document.getElementById("tourStepDesc"),n=document.getElementById("tourIconBubble"),s=document.getElementById("tourDots"),o=document.getElementById("tourBtnPrev"),l=document.getElementById("tourBtnNext");t&&(t.innerText=`STEP ${e+1} OF ${G.length}`),i&&(i.innerText=a.title),r&&(r.innerText=a.desc),n&&(n.innerHTML=`<i class="fa-solid ${a.icon}"></i>`),s&&(s.innerHTML=G.map((p,m)=>`<div class="tour-dot ${m===e?"active":""}"></div>`).join("")),o&&(o.style.display=e>0?"inline-block":"none"),l&&(e===G.length-1?l.innerHTML='Finish Tour <i class="fa-solid fa-check"></i>':l.innerHTML='Next <i class="fa-solid fa-arrow-right"></i>'),setTimeout(()=>{Le(a)},60),setTimeout(()=>{Le(a)},220)}function Ye(e=0){const a=document.getElementById("tourWelcomeModal");a&&(a.style.display="none");const t=document.getElementById("howItWorksModal");t&&(t.style.display="none");const i=document.getElementById("pawtrackTourOverlay");i&&(i.style.display="block",Ce=!0,Ge(e))}async function xe(){if(F)try{localStorage.setItem("pawtrack_tour_completed_"+F,"true"),localStorage.removeItem("pawtrack_is_new_user_"+F),sessionStorage.removeItem("pawtrack_just_registered_user"),sessionStorage.removeItem("pawtrack_is_new_registration"),S.tour_completed=!0,S.is_new_user=!1,await me.updatePrefs({...S,tour_completed:!0,is_new_user:!1})}catch(e){console.warn("Could not persist tour completion to Appwrite:",e)}}function Se(e=!0){const a=document.getElementById("pawtrackTourOverlay");a&&(a.style.display="none"),Ce=!1,Ue(null),e&&xe()}function Nt(){Z<G.length-1?Ge(Z+1):(Se(!0),te("Tour Complete! 🌟","You're all set! Click 'Guide & Tour' anytime at the top of your screen to replay.","fa-circle-check",null,"app"))}function qt(){Z>0&&Ge(Z-1)}function $e(){const e=document.getElementById("howItWorksModal");e&&(e.style.display="none")}function zt(){try{if(!F)return;const e=S.is_new_user===!0||localStorage.getItem("pawtrack_is_new_user_"+F)==="true"||sessionStorage.getItem("pawtrack_just_registered_user")===F||sessionStorage.getItem("pawtrack_is_new_registration")==="true",a=S.tour_completed===!0||localStorage.getItem("pawtrack_tour_completed_"+F)==="true";if(e&&!a)setTimeout(()=>{const t=document.getElementById("tourWelcomeModal"),i=document.getElementById("welcomeTourHeading"),r=S.username||A;i&&r&&(i.innerText=`Welcome, ${r}! 👋`),t&&(t.style.display="flex"),xe()},1200);else{const t=document.getElementById("tourWelcomeModal");t&&(t.style.display="none")}}catch(e){console.warn("First-time tour check warning:",e)}}const ct=document.getElementById("btnStartTour");ct&&ct.addEventListener("click",()=>{Ye(0)});const pt=document.getElementById("btnSidebarAiAssistant");pt&&pt.addEventListener("click",()=>{te("AI Assistant 🤖","This feature is still under development. Stay tuned for our AI-powered assistant!","fa-robot",null,"info")});const mt=document.getElementById("btnWelcomeTourStart");mt&&mt.addEventListener("click",()=>{xe(),Ye(0)});const ut=document.getElementById("btnWelcomeTourSkip");ut&&ut.addEventListener("click",()=>{const e=document.getElementById("tourWelcomeModal");e&&(e.style.display="none"),xe()});const gt=document.getElementById("tourBtnClose");gt&&gt.addEventListener("click",()=>Se(!0));const ft=document.getElementById("tourBtnSkip");ft&&ft.addEventListener("click",()=>Se(!0));const vt=document.getElementById("tourBtnNext");vt&&vt.addEventListener("click",Nt);const ht=document.getElementById("tourBtnPrev");ht&&ht.addEventListener("click",qt);const yt=document.getElementById("btnHiwClose");yt&&yt.addEventListener("click",$e);const bt=document.getElementById("btnHiwDismiss");bt&&bt.addEventListener("click",$e);const wt=document.getElementById("btnHiwStartInteractiveTour");wt&&wt.addEventListener("click",()=>{$e(),Ye(0)});const Xe=document.getElementById("howItWorksModal");Xe&&Xe.addEventListener("click",e=>{e.target===Xe&&$e()});const Ae=document.getElementById("tourWelcomeModal");Ae&&Ae.addEventListener("click",e=>{e.target===Ae&&(Ae.style.display="none",xe())}),document.querySelectorAll(".hiw-tab-btn").forEach(e=>{e.addEventListener("click",function(){document.querySelectorAll(".hiw-tab-btn").forEach(i=>i.classList.remove("active")),document.querySelectorAll(".hiw-tab-content").forEach(i=>i.classList.remove("active")),this.classList.add("active");const a=this.getAttribute("data-tab"),t=document.getElementById(a);t&&t.classList.add("active")})}),window.addEventListener("resize",()=>{Ce&&G[Z]&&Le(G[Z])}),window.addEventListener("scroll",()=>{Ce&&G[Z]&&Le(G[Z])},!0);function Ot(){const e=document.querySelector(".sidebar"),a=document.getElementById("sidebarTogglePill"),t=document.getElementById("btnHeaderSidebarToggle"),i=document.getElementById("sidebarLogoWrapper");if(!e)return;localStorage.getItem("pawtrack_sidebar_collapsed")==="true"&&window.innerWidth>768&&(e.classList.add("collapsed"),a&&(a.title="Maximize sidebar",a.setAttribute("aria-label","Maximize sidebar")),t&&(t.title="Maximize sidebar"));function n(s=null){if(window.innerWidth<=768)return;const o=s!==null?s:!e.classList.contains("collapsed");e.classList.toggle("collapsed",o);try{localStorage.setItem("pawtrack_sidebar_collapsed",o?"true":"false")}catch{}a&&(a.title=o?"Maximize sidebar":"Minimize sidebar",a.setAttribute("aria-label",o?"Maximize sidebar":"Minimize sidebar")),t&&(t.title=o?"Maximize sidebar":"Minimize sidebar"),setTimeout(()=>{window.dispatchEvent(new Event("resize"))},310)}a&&a.addEventListener("click",s=>{s.stopPropagation(),n()}),t&&t.addEventListener("click",s=>{s.stopPropagation(),n()}),i&&i.addEventListener("click",()=>{e.classList.contains("collapsed")&&n(!1)})}Ot(),Rt(),zt()});
