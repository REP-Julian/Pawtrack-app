import{a as te,d as L,Q as Z,I as G,s as Be,c as Re}from"./appwrite-BTLpE_3q.js";const P="pawtrack_db",Y="pets",K="applications",Ne="vet_appointments",Ie="recycle_bin",ct="activity_logs",Te="pawtrack_storage";let E="",Pe="",H="",R={},ae="",h=[],B=[],S=[],N=[],_e=null,ue=[],C={};document.addEventListener("DOMContentLoaded",async()=>{var lt;try{const e=await te.get();E=e.name,Pe=e.email,H=e.$id,R=e.prefs||{},ae=((lt=e.prefs)==null?void 0:lt.phone)||"";const a=document.getElementById("userNameDisplay");a&&(a.innerText=E?"Welcome, "+E+"!":"Welcome!");try{h=(await L.listDocuments(P,Y)).documents.map(n=>({...n,id:n.$id})),B=(await L.listDocuments(P,K,[Z.equal("user_id",H)])).documents.map(n=>({...n,id:n.$id})),S=(await L.listDocuments(P,Ne,[Z.equal("user_id",H)])).documents.map(n=>({...n,id:n.$id})),N=(await L.listDocuments(P,Ie,[Z.equal("owner",E)])).documents.map(n=>({...n,id:n.$id}))}catch(t){console.warn("Database collections not fully setup yet. Using empty arrays.",t)}}catch(e){console.error("User not logged in",e),window.location.href="/PawTrackLogin.html";return}async function $(e,a,t){try{await L.createDocument(P,ct,G.unique(),{user_id:H,action:e||"Activity recorded",target:a||"",icon:t||"fa-paw",timestamp:Date.now().toString()})}catch(s){console.warn("Failed to persist activity log to Appwrite:",s)}}function qe(){document.body.style.zoom="1",document.body.style.width="100%",document.body.style.height="100%",navigator.userAgent.toLowerCase().includes("firefox")&&(document.body.style.transform="none")}let ie;function w(e,a,t=!1,s=null){const r=document.getElementById("customPopupOverlay"),i=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),o=document.getElementById("customPopupActions");i.innerText=e,n.innerText=a,i.className=t?"custom-popup-title title-error":"custom-popup-title title-success",o.style.display="none",r.style.display="flex",clearTimeout(ie),ie=setTimeout(()=>{O(s)},5e3),document.getElementById("customPopupClose").onclick=()=>{clearTimeout(ie),O(s)}}function pt(e,a,t){const s=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),i=document.getElementById("customPopupMessage"),n=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-success",i.innerHTML=`<p style="margin-bottom:10px;">${a}</p><input type="text" id="customPromptInput" style="width:100%; padding:12px; border-radius:8px; border:2px solid #cbd5e1; outline:none; font-size:1rem; font-weight:600; color:#0f172a;" autocomplete="off">`,n.style.display="flex",s.style.display="flex",clearTimeout(ie),document.getElementById("btnPopupConfirm").innerText="Submit",document.getElementById("btnPopupConfirm").onclick=()=>{const o=document.getElementById("customPromptInput").value;O(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",o.trim()!==""&&t(o.trim())},document.getElementById("customPopupClose").onclick=()=>{O()},document.getElementById("btnPopupCancel").onclick=()=>{O(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure"}}function se(e,a,t){const s=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),i=document.getElementById("customPopupMessage"),n=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-error",i.innerHTML=`<p>${a}</p>`,n.style.display="flex",s.style.display="flex",clearTimeout(ie),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",document.getElementById("btnPopupConfirm").onclick=()=>{O(),t&&t()},document.getElementById("customPopupClose").onclick=()=>{O()},document.getElementById("btnPopupCancel").onclick=()=>{O()}}function O(e){document.getElementById("customPopupOverlay").style.display="none",e&&e()}window.addEventListener("resize",qe),qe();const ne=[{initials:"MD",name:"Dr. Miguel Antonio Dela Cruz",specialty:"General Veterinary Practitioner",phone:"0917-555-0101",email:"mdelacruz@pawtrack.ph",schedule:"Mon-Fri: 8am-4pm",clinic:"Quezon City Main Clinic",exp:"15 yrs exp",rating:"4.8"},{initials:"JS",name:"Dr. Joanna Marie R. Santos",specialty:"Veterinary Surgeon",phone:"0917-555-0102",email:"jmsantos@pawtrack.ph",schedule:"Tue-Sat: 10am-6pm",clinic:"Makati Pet Hospital",exp:"12 yrs exp",rating:"4.9"},{initials:"PV",name:"Dr. Paulo C. Villanueva",specialty:"Veterinary Oncologist",phone:"0917-555-0103",email:"pvillanueva@pawtrack.ph",schedule:"Mon-Thu: 9am-5pm",clinic:"BGC Animal Center",exp:"8 yrs exp",rating:"4.7"}];let z=0,V=[],Q=[],U=0,_=null,ee=[],ke=3;const j=document.querySelectorAll(".sidebar-nav .nav-btn"),v=document.getElementById("mainDisplayPanel"),Oe=document.getElementById("profileTemplate").innerHTML,Le=document.getElementById("cartItemsContainer"),mt=document.getElementById("cartTotalDisplay"),oe=(e,a,t)=>`
    ...
    `;function re(){let e="";if(typeof h<"u"&&h.length>0){const a=h.filter(t=>t.status==="Available");a.length>0?a.forEach(t=>{const s=t.gender==="Female"?"gender-female":"gender-male",i=t.owner===E?`
        ...
        `}const Ce=()=>{let e="";return typeof N<"u"&&N.length>0?N.forEach(a=>{const t=a.gender==="Female"?"gender-female":"gender-male";e+=`
                    <div class="pet-item-wrapper">
    ...
    `;function $e(){let e="";typeof S<"u"&&S.length>0?S.forEach(i=>{const n=i.status==="Approved",o=n?"appt-approved":"appt-pending",d=n?"badge-approved":"badge-pending",u=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],p=i.date.split("-"),g=p.length===3?u[parseInt(p[1])-1]:"TBD",f=p.length===3?p[2]:"??";e+=`
    ...
    `;function ge(){const e=h.filter(t=>t.owner===E&&t.status==="Private");let a="";return e.length>0?e.forEach(t=>{const s=t.gender==="Female"?"gender-female":"gender-male",r=t.img||"https://ui-avatars.com/api/?name=Pet&background=e2e8f0&color=64748b";a+=`
        ...
        `}function Je(e=null){const a=document.getElementById("messengerInboxList"),t=document.getElementById("messengerChatPanel"),s=document.getElementById("btnPageNewChat"),r=document.getElementById("messengerSearchInput");if(!a||!t)return;function i(p=""){a.innerHTML="";const g=Object.keys(C).filter(f=>f.toLowerCase().includes(p.toLowerCase()));if(g.length===0){a.innerHTML=`
                ...
                `;return}g.forEach(f=>{const b=C[f]||[],I=b.length>0?b[b.length-1].text:"No messages yet",T=b.length>0?b[b.length-1].time:"",x=f===W?"active":"",k=document.createElement("div");k.className=`inbox-item ${x}`,k.setAttribute("data-user",f),k.innerHTML=`
                ...
                `,k.onclick=()=>n(f),a.appendChild(k)})}function n(p){W=p,document.querySelectorAll(".inbox-item").forEach(x=>{x.classList.toggle("active",x.getAttribute("data-user")===p)});const g=C[p]||[];t.innerHTML=`
            ...
            `;const f=document.getElementById("activeChatStream");g.forEach(x=>{o(f,x.text,x.type,x.time)}),f.scrollTop=f.scrollHeight;const b=document.getElementById("activeChatInput"),I=document.getElementById("btnActiveSend");function T(){const x=b.value.trim();if(!x||!W)return;const k=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});o(f,x,"sent",k),b.value="",f.scrollTop=f.scrollHeight,C[W]||(C[W]=[]),C[W].push({text:x,type:"sent",time:k}),i(r.value.trim()),W==="PawTrackCommunity"&&setTimeout(()=>{const c=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),l="We're glad to have you! Feel free to connect with pet owners across PawTrack.";C.PawTrackCommunity.push({text:l,type:"received",time:c}),W==="PawTrackCommunity"&&(o(f,l,"received",c),f.scrollTop=f.scrollHeight),i(r.value.trim())},1e3)}I.onclick=T,b.onkeypress=x=>{x.key==="Enter"&&T()},b.focus()}function o(p,g,f,b){const I=document.createElement("div");I.className=`stream-bubble bubble-${f}`,I.innerHTML=`
                <div class="bubble-text">${M(g)}</div>
                <div class="bubble-time">${b}</div>
            `,p.appendChild(I)}r.oninput=p=>{i(p.target.value.trim())},s.onclick=()=>{pt("Start Conversation","Enter the PawTrack username you'd like to message:",p=>{if(p&&p.trim()){const g=p.trim().replace("@","");C[g]||(C[g]=[]),i(),n(g)}})};const d=Object.keys(C),u=e||(d.length>0?d[0]:null);i(),u?n(u):t.innerHTML=`
