import{a as ie,d as w,Q as j,I as H,s as le}from"./appwrite-BfOVdxfC.js";const E="pawtrack_db",de="pets",we="applications",Ee="vet_appointments",ce="recycle_bin",Ue="activity_logs",pe="pawtrack_storage";let h="",me="",N="",z={},G="",B=[],L=[],k=[],Y=[],Be=null,se=[],A={};document.addEventListener("DOMContentLoaded",async()=>{var Oe;try{const e=await ie.get();h=e.name,me=e.email,N=e.$id,z=e.prefs||{},G=((Oe=e.prefs)==null?void 0:Oe.phone)||"";const t=document.getElementById("userNameDisplay");t&&(t.innerText=h?"Welcome, "+h+"!":"Welcome!");try{B=(await w.listDocuments(E,de)).documents.map(o=>({...o,id:o.$id})),L=(await w.listDocuments(E,we,[j.equal("user_id",N)])).documents.map(o=>({...o,id:o.$id})),k=(await w.listDocuments(E,Ee,[j.equal("user_id",N)])).documents.map(o=>({...o,id:o.$id})),Y=(await w.listDocuments(E,ce,[j.equal("owner",h)])).documents.map(o=>({...o,id:o.$id}))}catch(a){console.warn("Database collections not fully setup yet. Using empty arrays.",a)}}catch(e){console.error("User not logged in",e),window.location.href="/PawTrackLogin.html";return}async function I(e,t,a){try{await w.createDocument(E,Ue,H.unique(),{user_id:N,action:e||"Activity recorded",target:t||"",icon:a||"fa-paw",timestamp:Date.now().toString()})}catch(i){console.warn("Failed to persist activity log to Appwrite:",i)}}function Ie(){document.body.style.zoom="1",document.body.style.width="100%",document.body.style.height="100%",navigator.userAgent.toLowerCase().includes("firefox")&&(document.body.style.transform="none")}let W;function m(e,t,a=!1,i=null){const l=document.getElementById("customPopupOverlay"),n=document.getElementById("customPopupTitle"),o=document.getElementById("customPopupMessage"),r=document.getElementById("customPopupActions");n.innerText=e,o.innerText=t,n.className=a?"custom-popup-title title-error":"custom-popup-title title-success",r.style.display="none",l.style.display="flex",clearTimeout(W),W=setTimeout(()=>{M(i)},5e3),document.getElementById("customPopupClose").onclick=()=>{clearTimeout(W),M(i)}}function je(e,t,a){const i=document.getElementById("customPopupOverlay"),l=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),o=document.getElementById("customPopupActions");l.innerText=e,l.className="custom-popup-title title-success",n.innerHTML=`<p style="margin-bottom:10px;">${t}</p><input type="text" id="customPromptInput" style="width:100%; padding:12px; border-radius:8px; border:2px solid #cbd5e1; outline:none; font-size:1rem; font-weight:600; color:#0f172a;" autocomplete="off">`,o.style.display="flex",i.style.display="flex",clearTimeout(W),document.getElementById("btnPopupConfirm").innerText="Submit",document.getElementById("btnPopupConfirm").onclick=()=>{const r=document.getElementById("customPromptInput").value;M(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",r.trim()!==""&&a(r.trim())},document.getElementById("customPopupClose").onclick=()=>{M()},document.getElementById("btnPopupCancel").onclick=()=>{M(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure"}}function X(e,t,a){const i=document.getElementById("customPopupOverlay"),l=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),o=document.getElementById("customPopupActions");l.innerText=e,l.className="custom-popup-title title-error",n.innerHTML=`<p>${t}</p>`,o.style.display="flex",i.style.display="flex",clearTimeout(W),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",document.getElementById("btnPopupConfirm").onclick=()=>{M(),a&&a()},document.getElementById("customPopupClose").onclick=()=>{M()},document.getElementById("btnPopupCancel").onclick=()=>{M()}}function M(e){document.getElementById("customPopupOverlay").style.display="none",e&&e()}window.addEventListener("resize",Ie),Ie();const J=[{initials:"MD",name:"Dr. Miguel Antonio Dela Cruz",specialty:"General Veterinary Practitioner",phone:"0917-555-0101",email:"mdelacruz@pawtrack.ph",schedule:"Mon-Fri: 8am-4pm",clinic:"Quezon City Main Clinic",exp:"15 yrs exp",rating:"4.8"},{initials:"JS",name:"Dr. Joanna Marie R. Santos",specialty:"Veterinary Surgeon",phone:"0917-555-0102",email:"jmsantos@pawtrack.ph",schedule:"Tue-Sat: 10am-6pm",clinic:"Makati Pet Hospital",exp:"12 yrs exp",rating:"4.9"},{initials:"PV",name:"Dr. Paulo C. Villanueva",specialty:"Veterinary Oncologist",phone:"0917-555-0103",email:"pvillanueva@pawtrack.ph",schedule:"Mon-Thu: 9am-5pm",clinic:"BGC Animal Center",exp:"8 yrs exp",rating:"4.7"}];let S=0,F=[],q=[],D=0,$=null,O=[],ge=3;const Z=document.querySelectorAll(".sidebar-nav .nav-btn"),y=document.getElementById("mainDisplayPanel"),Te=document.getElementById("profileTemplate").innerHTML,ue=document.getElementById("cartItemsContainer"),Ge=document.getElementById("cartTotalDisplay"),K=(e,t,a)=>`
    ...
    `;function Ye(){let e="",t="";return typeof L<"u"&&L.length>0&&L.forEach(a=>{const i=a.status==="Approved",l=i?"var(--emerald-light, #D1FAE5)":"var(--amber-light, #FEF3C7)",n=i?"#065F46":"#92400E",r=`
    ...
    `;function Xe(){let e="";typeof k<"u"&&k.length>0?k.forEach(n=>{const o=n.status==="Approved",r=o?"appt-approved":"appt-pending",p=o?"badge-approved":"badge-pending",f=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],g=n.date.split("-"),v=g.length===3?f[parseInt(g[1])-1]:"TBD",x=g.length===3?g[2]:"??";e+=`
                ...
                `}):e='<p style="color: #64748b; margin-top: 10px; font-weight: 500;">No upcoming appointments.</p>';let t="";const a=B.filter(n=>n.owner===h);let i=[];typeof k<"u"&&(i=k.filter(n=>n.status!=="Cancelled"&&n.status!=="Completed").map(n=>n.pet_name));const l=a.filter(n=>!i.includes(n.name));return l.length>0?l.forEach(n=>{const o=n.gender&&n.gender.toLowerCase()==="female"?"gender-female":"gender-male";t+=`
    ...
    `;function Ce(){const e=B.filter(a=>a.owner===h&&a.status==="Private");let t="";return e.length>0?e.forEach(a=>{const i=a.gender==="Female"?"gender-female":"gender-male",l=a.img||"https://ui-avatars.com/api/?name=Pet&background=e2e8f0&color=64748b";t+=`
        ...
        `,Promise.resolve({status:"success",matches:se}).then(e=>{if(e.status==="success"){let t='<div class="pairs-grid">';e.matches.length===0?t+='<p style="grid-column: 1/-1; text-align: center; color: #64748b; font-weight: bold; margin-top: 30px;">You have no active matches or requests yet.</p>':e.matches.forEach(a=>{let i="",l="",n="";if(a.status==="pending"){const o=!a.is_sender,r=o?"Reject Match":"Cancel Request",p=o?"Are you sure you want to reject this match request?":"Are you sure you want to cancel your match request?";n=`
        ...
        `,Promise.resolve({status:"success",matches:se}).then(r=>{const p=document.getElementById("homeMatchAlertsContainer");if(p)if(r.status==="success"&&r.matches.length>0){let f="";r.matches.slice(0,3).forEach(g=>{let v=g.status==="approved"?"Approved Match!":"Pending Request",x=g.status==="approved"?"💌":"⏳";f+=`
                ...
                `;b.status==="Private"?s.innerHTML+=P:c.innerHTML+=P}));const u=document.getElementById("recentActivityLogs");if(u){u.innerHTML='<p style="text-align:center; color:#94a3b8; padding:15px;">Loading activity logs...</p>';try{const b=await w.listDocuments(E,Ue,[j.equal("user_id",N),j.orderDesc("$createdAt"),j.limit(20)]);u.innerHTML="";const P=b.documents;P.length>0?P.forEach(U=>{const tt=parseInt(U.timestamp)||new Date(U.$createdAt).getTime(),Ve=new Date(tt),at=Ve.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});u.innerHTML+=`
                    ...
                    `}catch(b){console.warn("Could not load activity logs:",b),u.innerHTML='<p style="text-align:center; color:#94a3b8; padding:15px;">No activity recorded yet.</p>'}}}const De=document.getElementById("chatFab"),re=document.getElementById("chatWindow"),Qe=document.getElementById("chatCloseBtn"),ye=document.getElementById("chatBackBtn"),be=document.getElementById("btnNewChat"),He=document.getElementById("chatTitle"),R=document.getElementById("chatListView"),Re=document.getElementById("chatConvoView"),C=document.getElementById("chatMessagesBox"),he=document.getElementById("chatInputField"),et=document.getElementById("btnSendMsg");let V=null;De.addEventListener("click",()=>{re.classList.toggle("active"),re.classList.contains("active")&&Ne()}),Qe.addEventListener("click",()=>re.classList.remove("active"));const _e=document.getElementById("btnLogout");_e&&_e.addEventListener("click",async()=>{try{await ie.deleteSession("current")}catch(e){console.warn(e)}window.location.href="/PawTrackLogin.html"});function Ne(){Re.style.display="none",R.style.display="flex",ye.style.display="none",be.style.display="flex",He.innerHTML='<i class="fa-solid fa-messages"></i> Messages',V=null,R.innerHTML='<p style="text-align:center; margin-top:20px; color:#64748b;">Loading...</p>',Promise.resolve().then(()=>({status:"success",inbox:Object.keys(A).map(t=>{const a=A[t],i=a[a.length-1];return{contact:t,last_message:i.text,time:i.time,unread:0}})})).then(e=>{if(e.status==="success"){if(R.innerHTML="",e.inbox.length===0){R.innerHTML='<p style="text-align:center; margin-top:20px; color:#64748b; font-weight:bold;">No messages yet.<br><br>Click the + icon to start a chat with a username!</p>';return}e.inbox.forEach(t=>{const a=t.unread>0?`<span class="chat-unread">${t.unread}</span>`:"";R.innerHTML+=`
