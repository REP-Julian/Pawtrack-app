import{a as ae,d as L,Q as Z,I as Y,s as Te,c as Ne}from"./appwrite-BTLpE_3q.js";const k="pawtrack_db",X="pets",K="applications",qe="vet_appointments",ke="recycle_bin",gt="activity_logs",Pe="pawtrack_storage";let T="",ze="",$="",P={},ie="",y=[],x=[],D=[],R=[],Oe=null,ve=[],A={};document.addEventListener("DOMContentLoaded",async()=>{var ut;const se=document.getElementById("btnThemeToggle"),Ve=document.querySelector('meta[name="color-scheme"]');function Le(e){document.documentElement.setAttribute("data-theme",e),Ve&&(Ve.content=e),se&&(se.setAttribute("title",e==="dark"?"Switch to Light Mode":"Switch to Dark Mode"),se.setAttribute("aria-label",e==="dark"?"Switch to Light Mode":"Switch to Dark Mode"))}const ft=localStorage.getItem("pawtrack_theme"),vt=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches;Le(ft||(vt?"dark":"light")),se&&se.addEventListener("click",()=>{const a=(document.documentElement.getAttribute("data-theme")||"light")==="dark"?"light":"dark";localStorage.setItem("pawtrack_theme",a),Le(a)}),window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",e=>{localStorage.getItem("pawtrack_theme")||Le(e.matches?"dark":"light")});try{const e=await ae.get();T=e.name,ze=e.email,$=e.$id,P=e.prefs||{},ie=((ut=e.prefs)==null?void 0:ut.phone)||"";const a=document.getElementById("userNameDisplay");if(a){const t=P.username||T;a.innerText=t?"Welcome, "+t+"!":"Welcome!"}try{y=(await L.listDocuments(k,X)).documents.map(s=>({...s,id:s.$id})),x=(await L.listDocuments(k,K,[Z.equal("user_id",$)])).documents.map(s=>({...s,id:s.$id})),D=(await L.listDocuments(k,qe,[Z.equal("user_id",$)])).documents.map(s=>({...s,id:s.$id})),R=(await L.listDocuments(k,ke,[Z.equal("owner",T)])).documents.map(s=>({...s,id:s.$id}))}catch(t){console.warn("Database collections not fully setup yet. Using empty arrays.",t)}}catch(e){console.error("User not logged in",e),window.location.href="/PawTrackLogin.html";return}async function M(e,a,t){try{await L.createDocument(k,gt,Y.unique(),{user_id:$,action:e||"Activity recorded",target:a||"",icon:t||"fa-paw",timestamp:Date.now().toString()})}catch(i){console.warn("Failed to persist activity log to Appwrite:",i)}}function Ue(){document.body.style.zoom="1",document.body.style.width="100%",document.body.style.height="100%",navigator.userAgent.toLowerCase().includes("firefox")&&(document.body.style.transform="none")}let ne;function b(e,a,t=!1,i=null){const r=document.getElementById("customPopupOverlay"),n=document.getElementById("customPopupTitle"),s=document.getElementById("customPopupMessage"),o=document.getElementById("customPopupActions");n.innerText=e,s.innerText=a,n.className=t?"custom-popup-title title-error":"custom-popup-title title-success",o.style.display="none",r.style.display="flex",clearTimeout(ne),ne=setTimeout(()=>{z(i)},5e3),document.getElementById("customPopupClose").onclick=()=>{clearTimeout(ne),z(i)}}function yt(e,a,t){const i=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),s=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-success",n.innerHTML=`<p style="margin-bottom:10px;">${a}</p><input type="text" id="customPromptInput" style="width:100%; padding:12px; border-radius:8px; border:2px solid #cbd5e1; outline:none; font-size:1rem; font-weight:600; color:#0f172a;" autocomplete="off">`,s.style.display="flex",i.style.display="flex",clearTimeout(ne),document.getElementById("btnPopupConfirm").innerText="Submit",document.getElementById("btnPopupConfirm").onclick=()=>{const o=document.getElementById("customPromptInput").value;z(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",o.trim()!==""&&t(o.trim())},document.getElementById("customPopupClose").onclick=()=>{z()},document.getElementById("btnPopupCancel").onclick=()=>{z(),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure"}}function oe(e,a,t){const i=document.getElementById("customPopupOverlay"),r=document.getElementById("customPopupTitle"),n=document.getElementById("customPopupMessage"),s=document.getElementById("customPopupActions");r.innerText=e,r.className="custom-popup-title title-error",n.innerHTML=`<p>${a}</p>`,s.style.display="flex",i.style.display="flex",clearTimeout(ne),document.getElementById("btnPopupConfirm").innerText="Yes, I'm sure",document.getElementById("btnPopupConfirm").onclick=()=>{z(),t&&t()},document.getElementById("customPopupClose").onclick=()=>{z()},document.getElementById("btnPopupCancel").onclick=()=>{z()}}function z(e){document.getElementById("customPopupOverlay").style.display="none",e&&e()}window.addEventListener("resize",Ue),Ue();const re=[{initials:"MD",name:"Dr. Miguel Antonio Dela Cruz",specialty:"General Veterinary Practitioner",phone:"0917-555-0101",email:"mdelacruz@pawtrack.ph",schedule:"Mon-Fri: 8am-4pm",clinic:"Quezon City Main Clinic",exp:"15 yrs exp",rating:"4.8"},{initials:"JS",name:"Dr. Joanna Marie R. Santos",specialty:"Veterinary Surgeon",phone:"0917-555-0102",email:"jmsantos@pawtrack.ph",schedule:"Tue-Sat: 10am-6pm",clinic:"Makati Pet Hospital",exp:"12 yrs exp",rating:"4.9"},{initials:"PV",name:"Dr. Paulo C. Villanueva",specialty:"Veterinary Oncologist",phone:"0917-555-0103",email:"pvillanueva@pawtrack.ph",schedule:"Mon-Thu: 9am-5pm",clinic:"BGC Animal Center",exp:"8 yrs exp",rating:"4.7"}];let O=0,V=[],Q=[],U=0,N=null,ee=[],Ce=3;const W=document.querySelectorAll(".sidebar-nav .nav-btn"),v=document.getElementById("mainDisplayPanel"),We=document.getElementById("profileTemplate").innerHTML,$e=document.getElementById("cartItemsContainer"),ht=document.getElementById("cartTotalDisplay"),le=(e,a,t)=>`
    ...
    `;function de(){let e="";if(typeof y<"u"&&y.length>0){const a=y.filter(t=>t.status==="Available");a.length>0?a.forEach(t=>{const i=t.gender==="Female"?"gender-female":"gender-male",n=t.owner===T?`
        ...
        `}function ce(){let e="",a="";return typeof x<"u"&&x.length>0&&x.forEach(t=>{const i=t.status==="Approved",r=i?"var(--emerald-light, #D1FAE5)":"var(--amber-light, #FEF3C7)",n=i?"#065F46":"#92400E",o=`
        ...
        `}const Se=()=>{let e="";return typeof R<"u"&&R.length>0?R.forEach(a=>{const t=a.gender==="Female"?"gender-female":"gender-male";e+=`
                    <div class="pet-item-wrapper">
    ...
    `;function Ae(){let e="";typeof D<"u"&&D.length>0?D.forEach(n=>{const s=n.status==="Approved",o=s?"appt-approved":"appt-pending",l=s?"badge-approved":"badge-pending",u=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],p=n.date.split("-"),g=p.length===3?u[parseInt(p[1])-1]:"TBD",f=p.length===3?p[2]:"??";e+=`
                    <div class="appt-mini-card ${o}">
                        ...
                        <div class="appt-details">
                            ...
                            <p>${n.vet_name}</p>
                            <span class="badge ${l}">${n.status} • ${n.time}</span>
                        </div>
                ...
                `}):e='<p style="color: #64748b; margin-top: 10px; font-weight: 500;">No upcoming appointments.</p>';let a="";const t=y.filter(n=>n.owner===T);let i=[];typeof D<"u"&&(i=D.filter(n=>n.status!=="Cancelled"&&n.status!=="Completed").map(n=>n.pet_name));const r=t.filter(n=>!i.includes(n.name));return r.length>0?r.forEach(n=>{const s=n.gender&&n.gender.toLowerCase()==="female"?"gender-female":"gender-male";a+=`
                    <div class="pet-select-card ${s}" data-petid="${n.id}">
    ...
    `;function ye(){const e=y.filter(t=>t.owner===T&&t.status==="Private");let a="";return e.length>0?e.forEach(t=>{const i=t.gender==="Female"?"gender-female":"gender-male",r=t.img||"https://ui-avatars.com/api/?name=Pet&background=e2e8f0&color=64748b";a+=`
        ...
        `,Promise.resolve({status:"success",matches:ve}).then(e=>{if(e.status==="success"){let a='<div class="pairs-grid">';e.matches.length===0?a+='<p style="grid-column: 1/-1; text-align: center; color: #64748b; font-weight: bold; margin-top: 30px;">You have no active matches or requests yet.</p>':e.matches.forEach(t=>{let i="",r="",n="";if(t.status==="pending"){const s=!t.is_sender,o=s?"Reject Match":"Cancel Request",l=s?"Are you sure you want to reject this match request?":"Are you sure you want to cancel your match request?";n=`
                                <button class="btn-delete-pair" data-matchid="${t.id}" data-title="${o}" data-msg="${l}" title="${s?"Reject":"Cancel Request"}" 
        ...
        `,Promise.resolve({status:"success",matches:ve}).then(o=>{const l=document.getElementById("homeMatchAlertsContainer");if(l)if(o.status==="success"&&o.matches.length>0){let u="";o.matches.slice(0,3).forEach(p=>{let g=p.status==="approved"?"Approved Match!":"Pending Request",f=p.status==="approved"?"💌":"⏳";u+=`
            ...
            <div class="toast-body">
                ...
                <p class="toast-msg">${F(a)}</p>
            </div>
            <button class="toast-close" title="Dismiss">&times;</button>
        `;const o=s.querySelector(".toast-close");let l=null;const u=p=>{p&&p.stopPropagation(),l&&clearTimeout(l),s.style.animation="toastSlideOut 0.25s forwards",setTimeout(()=>{s.parentElement&&s.parentElement.removeChild(s)},250)};o.addEventListener("click",u),i&&s.addEventListener("click",()=>{u();const p=document.querySelector(`.sidebar-nav .nav-btn[data-target="${i}"]`);if(p)p.click();else if(i==="pets"){W.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="pets"]');g&&g.classList.add("active"),v.innerHTML=de()}else if(i==="applications"){W.forEach(f=>f.classList.remove("active"));const g=document.querySelector('[data-target="applications"]');g&&g.classList.add("active"),v.innerHTML=ce()}}),n.appendChild(s),l=setTimeout(u,7e3)}function Et(){if(document.getElementById("adoptionApplicationForm"))return"adopt-form";if(document.getElementById("petRegistrationForm")||document.getElementById("registerPetForm"))return"register-pet";if(document.getElementById("profileFullName")||document.querySelector(".profile-container"))return"profile";const e=document.querySelector(".sidebar-nav .nav-btn.active");return e?e.getAttribute("data-target"):"home"}function me(){const e=Et();e==="home"?pe():e==="pets"?v.innerHTML=de():e==="applications"?v.innerHTML=ce():e==="profile"?be():e==="breeding"&&typeof ye=="function"&&!document.getElementById("matchDetailsOverlay")&&(v.innerHTML=ye())}function Bt(e){if(!e||!e.payload)return;const a=e.events||[],t=e.payload,i=a.some(s=>s.includes(".create")),r=a.some(s=>s.includes(".update")),n=a.some(s=>s.includes(".delete"));if(t.$collectionId===X||a.some(s=>s.includes(`.${X}.`))){const s=t.$id,o={...t,id:t.$id};if(n){const l=y.find(u=>u.id===s);y=y.filter(u=>u.id!==s),l&&l.status==="Available"&&l.owner!==T&&j("Pet Removed",`${l.name} is no longer listed for adoption.`,"fa-paw","pets","info")}else if(i){const l=y.findIndex(u=>u.id===s);l===-1?y.unshift(o):y[l]=o,o.status==="Available"&&o.owner!==T&&j("New Pet for Adoption! 🐾",`@${o.owner||"A caregiver"} just listed ${o.name} (${o.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}else if(r){const l=y.findIndex(p=>p.id===s),u=l!==-1?y[l]:null;l!==-1?y[l]=o:y.unshift(o),o.owner!==T&&(u&&u.status!=="Available"&&o.status==="Available"?j("Pet Available for Adoption! 🐾",`${o.name} (${o.breed}) is now available for adoption! Tap to view.`,"fa-heart","pets","default"):u&&u.status==="Available"&&o.status!=="Available"&&j("Pet Status Updated",`${o.name} is now ${o.status}.`,"fa-paw","pets","info"))}me()}if(t.$collectionId===K||a.some(s=>s.includes(`.${K}.`))){const s=t.$id,o={...t,id:t.$id};if(o.user_id===$){if(n)x=x.filter(l=>l.id!==s);else if(i){const l=x.findIndex(u=>u.id===s);l===-1?x.unshift(o):x[l]=o}else if(r){const l=x.findIndex(p=>p.id===s),u=l!==-1?x[l].status:"";if(l!==-1?x[l]=o:x.unshift(o),u!==o.status){const p=o.status==="Approved";j(p?"🎉 Application Approved!":"Application Status Updated",`Your adoption application for ${o.pet_name} is now: ${o.status}!`,p?"fa-circle-check":"fa-clock","applications","app")}}me()}}}let Me=!1;async function It(){if(!Me){Me=!0;try{const a=(await L.listDocuments(k,X)).documents.map(n=>({...n,id:n.$id})),t=new Set(y.map(n=>n.id)),i=a.filter(n=>!t.has(n.id)&&n.status==="Available"&&n.owner!==T);if(a.length!==y.length||a.some((n,s)=>{var o,l;return((o=y[s])==null?void 0:o.id)!==n.id||((l=y[s])==null?void 0:l.status)!==n.status})){if(y=a,i.length>0){const n=i[0];j("New Pet for Adoption! 🐾",`@${n.owner||"A caregiver"} just listed ${n.name} (${n.breed||"Pet"}). Tap to meet them!`,"fa-heart","pets","default")}me()}if($){const s=(await L.listDocuments(k,K,[Z.equal("user_id",$)])).documents.map(l=>({...l,id:l.$id}));(s.length!==x.length||s.some((l,u)=>{var p,g;return((p=x[u])==null?void 0:p.id)!==l.id||((g=x[u])==null?void 0:g.status)!==l.status}))&&(x=s,me())}}catch{}finally{Me=!1}}}function Tt(){try{if(Ne&&typeof Ne.subscribe=="function"){const e=`databases.${k}.collections.${X}.documents`,a=`databases.${k}.collections.${K}.documents`;Ne.subscribe([e,a],t=>{Bt(t)}),console.log("PawTrack: Appwrite Realtime connected for live adoption sync.")}}catch(e){console.warn("Realtime subscription fallback to polling:",e)}setInterval(It,8e3)}Tt();function J(){const e=re[O],a=document.getElementById("vetInitials");if(!a)return;a.innerText=e.initials,document.getElementById("vetNameDisplay").innerText=e.name,document.getElementById("vetSpecialtyDisplay").innerText=e.specialty,document.getElementById("vetRatingDisplay").innerText=`⭐ ${e.rating} (${e.exp})`,document.getElementById("vetPhoneDisplay").innerText=e.phone,document.getElementById("vetEmailDisplay").innerText=e.email,document.getElementById("vetScheduleDisplay").innerText=e.schedule,document.getElementById("vetClinicDisplay").innerText=e.clinic;const t=document.getElementById("apptVetIdHidden").value,i=document.getElementById("btnSelectVet");t===O.toString()?(i.innerText="✅ Confirmed",i.classList.add("selected")):(i.innerText="Confirm",i.classList.remove("selected"))}function ue(){const e=document.getElementById("cartBadge");e&&(e.innerText=V.length),$e.innerHTML="";let a=0;V.length===0?$e.innerHTML='<p style="text-align:center; color:#64748b; font-weight:bold; margin-top: 50px;">Your cart is empty.</p>':V.forEach((t,i)=>{a+=parseFloat(t.price),$e.innerHTML+=`
                ...
                `}),ht.innerText=`₱ ${a.toFixed(2)}`}window.handleRewind=e=>{ee.splice(e,1),U=Math.max(0,U-1),De()};function kt(){const e=document.getElementById("rewindList");e&&(e.innerHTML="",ee.length===0?e.innerHTML=`
                ...
                `}))}function De(){if(Q.length===0||U>=Q.length){document.getElementById("datingContentLayout").style.display="none";const s=document.getElementById("instructionOverlay");s.style.display="flex",s.innerHTML='<i class="fa-solid fa-face-frown-open" style="color: #cbd5e1; font-size: 4rem; margin-bottom: 15px;"></i><p>No more matches available<br>for this pet at the moment.</p>';return}const e=Q[U];currentPhotoIndex=0;const a=document.getElementById("candThumbnails");a.innerHTML="",e.imgs.forEach((s,o)=>{a.innerHTML+=`<img src="${s}" class="cand-thumb ${o===0?"active":""}" data-index="${o}" alt="Photo ${o+1}">`}),Xe(e,0);const t=e.gender==="Male"?'<i class="fa-solid fa-mars" style="color: #60a5fa;"></i>':'<i class="fa-solid fa-venus" style="color: #f472b6;"></i>';document.getElementById("candNameAge").innerHTML=`${e.name}, ${e.age} ${t}`,document.getElementById("candBreedGender").innerHTML=`<i class="fa-solid fa-paw"></i> ${e.breed}`,document.getElementById("candDesc").innerText=e.desc,document.getElementById("candScoreText").innerText=`${e.score}% Match`,document.getElementById("candScoreBar").style.width=`${e.score}%`;const i=document.getElementById("candBadges");i.innerHTML="",e.badges.includes("vet")&&(i.innerHTML+='<span class="v-badge badge-vet"><i class="fa-solid fa-stethoscope"></i> Vet Verified</span>'),e.badges.includes("pedigree")&&(i.innerHTML+='<span class="v-badge badge-pedigree"><i class="fa-solid fa-dna"></i> Purebred</span>'),e.badges.includes("vax")&&(i.innerHTML+='<span class="v-badge badge-vax"><i class="fa-solid fa-syringe"></i> Vaccinated</span>');const r=document.getElementById("candTraits");r.innerHTML="",e.traits.forEach(s=>{r.innerHTML+=`<span class="trait-tag">${s}</span>`}),document.getElementById("candOwnerName").innerText=e.ownerName,document.getElementById("candOwnerInitial").innerText=e.ownerInitial,document.getElementById("candOwnerRating").innerText=`⭐ ${e.ownerRating}`,document.getElementById("candOwnerPairs").innerText=`${e.successPairs} Successful Pairs`,document.getElementById("barSize").style.width=`${e.compSize}%`,document.getElementById("txtSize").innerText=`${e.compSize}%`,document.getElementById("barEnergy").style.width=`${e.compEnergy}%`,document.getElementById("txtEnergy").innerText=`${e.compEnergy}%`,document.getElementById("barTemp").style.width=`${e.compTemp}%`,document.getElementById("txtTemp").innerText=`${e.compTemp}%`;const n=document.getElementById("litterPredictor");n.innerHTML="",e.litter.forEach(s=>{const o=s.c==="#FFFFFF"||s.c==="#FFF8DC"?"#0f172a":"white";n.innerHTML+=`<div class="color-swatch" style="background:${s.c}; color:${o};">${s.p}%</div>`}),document.getElementById("instructionOverlay").style.display="none",document.getElementById("datingContentLayout").style.display="grid",document.getElementById("treatCountDisplay").innerText=`${Ce} Left`,kt()}function Xe(e,a){const t=document.getElementById("candImg");t.style.opacity=0,setTimeout(()=>{t.src=e.imgs[a],t.style.opacity=1},150),document.querySelectorAll(".cand-thumb").forEach((i,r)=>{r===parseInt(a)?i.classList.add("active"):i.classList.remove("active")})}W.forEach(e=>{e.addEventListener("click",function(){W.forEach(t=>t.classList.remove("active")),this.classList.add("active");const a=this.getAttribute("data-target");v.scrollTo({top:0,behavior:"smooth"}),a==="home"?pe():a==="pets"?v.innerHTML=de():a==="applications"?v.innerHTML=ce():a==="vet"?(v.innerHTML=Ae(),J()):a==="shop"?(v.innerHTML=bt(),ue()):a==="breeding"?(v.innerHTML=ye(),N=null):a==="messages"&&(v.innerHTML=et(),tt())})}),document.getElementById("btnViewProfile").addEventListener("click",()=>{W.forEach(e=>e.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=We,be()}),document.getElementById("btnRegisterPet").addEventListener("click",()=>{W.forEach(r=>r.classList.remove("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=wt;const e=document.getElementById("imageDropZone"),a=document.getElementById("petImageInput"),t=document.getElementById("imagePreview"),i=document.getElementById("dropZoneText");e.addEventListener("click",()=>a.click()),Oe=null,a.addEventListener("change",function(){if(this.files&&this.files[0]){Oe=this.files[0];const r=new FileReader;r.onload=function(n){t.src=n.target.result,t.style.display="block",i.style.display="none"},r.readAsDataURL(this.files[0])}}),document.getElementById("btnCancelReg").addEventListener("click",()=>{pe(),W[0].classList.add("active")})}),v.addEventListener("click",async e=>{if(e.target.closest("#btnMatchRegisterPet")){document.getElementById("btnRegisterPet").click();return}if(e.target.classList.contains("profile-tab-btn")){document.querySelectorAll(".profile-tab-btn").forEach(d=>d.classList.remove("active")),document.querySelectorAll(".profile-tab-content").forEach(d=>d.classList.remove("active")),e.target.classList.add("active");const c=e.target.getAttribute("data-tab");document.getElementById(c).classList.add("active")}const t=e.target.closest(".btn-view-app")||e.target.closest(".btn-view-pet");if(t){const c=t.getAttribute("data-target"),d=document.getElementById(c),m=t.closest(".app-card")||t.closest(".pet-card");d.style.display==="block"?(d.style.display="none",m.classList.remove("dropdown-open")):(d.style.display="block",m.classList.add("dropdown-open"))}const i=e.target.closest(".btn-close-pet");if(i){const c=i.getAttribute("data-target"),d=document.getElementById(c),m=d.previousElementSibling;d.style.display="none",m.classList.remove("dropdown-open")}const r=e.target.closest(".btn-adopt");if(r){const c=r.getAttribute("data-petid"),d=r.getAttribute("data-petname");v.innerHTML=je(c,d)}e.target.closest("#btnBackToPets")&&(v.innerHTML=de()),e.target.closest(".btn-edit-profile")&&(document.getElementById("editProfileModal").style.display="flex");const n=e.target.closest(".btn-archive-pet");if(n){const c=n.getAttribute("data-petid");oe("Move to Bin?","Are you sure you want to move this pet to the Recycle Bin?",async()=>{const d=y.find(m=>m.id===c);if(!d){b("Error","Pet record not found.",!0);return}try{const m=await L.createDocument(k,ke,Y.unique(),{name:d.name,breed:d.breed,gender:d.gender,owner:T,img:d.img||""});await L.deleteDocument(k,X,c),y=y.filter(I=>I.id!==c),R.unshift({...m,id:m.$id}),await M("Moved a pet to the Recycle Bin",d.name,"fa-trash-can"),b("Moved to Bin","Pet successfully moved to Recycle Bin.",!1,()=>{me()})}catch(m){console.error("Appwrite Move to Bin Error:",m),b("Error",m.message,!0)}})}e.target.closest("#btnViewBin")&&(v.innerHTML=Se()),e.target.closest("#btnEmptyBin")&&oe("Empty Bin?","WARNING: Are you sure you want to permanently delete ALL pets in the Recycle Bin? This cannot be undone!",async()=>{try{for(const c of R)await L.deleteDocument(k,ke,c.id);R=[],await M("Permanently emptied the Recycle Bin","","fa-dumpster-fire"),b("Bin Emptied","Recycle bin emptied successfully.",!1,()=>{v.innerHTML=Se()})}catch(c){console.error("Appwrite Empty Bin Error:",c),b("Error",c.message,!0)}}),e.target.closest("#btnBackToProfile")&&(v.innerHTML=We,be());const s=e.target.closest(".btn-restore-pet");if(s){const c=s.getAttribute("data-petid"),d=R.find(m=>m.id===c);if(d)try{const m=await L.createDocument(k,X,Y.unique(),{name:d.name,breed:d.breed,gender:d.gender,age:"1 yr",status:"Available",health_status:"Healthy",owner:T,contact_number:ie||"0917-000-0000",personal_traits:"Friendly",reason_for_adoption:"Restored from Recycle Bin",img:d.img||""});await L.deleteDocument(k,ke,c),R=R.filter(I=>I.id!==c),y.unshift({...m,id:m.$id}),await M("Restored a pet from the Recycle Bin",d.name,"fa-rotate-left"),b("Restored!","Pet has been restored to the active board!",!1,()=>{v.innerHTML=Se()})}catch(m){console.error("Appwrite Restore Error:",m),b("Error",m.message,!0)}}const o=e.target.closest(".btn-cancel-app");if(o){const c=o.getAttribute("data-appid");oe("Cancel Application?","Are you sure you want to cancel this application?",async()=>{try{await L.deleteDocument(k,K,c),x=x.filter(d=>d.id!==c),await M("Cancelled adoption application","","fa-file-circle-xmark"),b("Cancelled","Application successfully cancelled.",!1,()=>{v.innerHTML=ce()})}catch(d){console.error("Appwrite Cancel App Error:",d),b("Error",d.message,!0)}})}const l=e.target.closest(".btn-cancel-vet");if(l){const c=l.getAttribute("data-appid");oe("Cancel Appointment?","Are you sure you want to cancel this veterinary appointment?",async()=>{try{await L.deleteDocument(k,qe,c),D=D.filter(d=>d.id!==c),await M("Cancelled veterinary appointment","","fa-calendar-xmark"),b("Cancelled","Appointment successfully cancelled.",!1,()=>{v.innerHTML=Ae(),J()})}catch(d){console.error("Appwrite Cancel Vet Error:",d),b("Error",d.message,!0)}})}const u=e.target.closest(".pet-select-card");if(u&&document.getElementById("apptPetSelectorHidden")){document.querySelectorAll(".pet-select-card").forEach(m=>m.classList.remove("selected")),u.classList.add("selected");const c=u.getAttribute("data-pet");document.getElementById("apptPetSelectorHidden").value=c;const d=myPetsDB[c];d&&(document.getElementById("apptOwner").value=d.owner,document.getElementById("apptPetType").value=d.type,document.getElementById("apptBreed").value=d.breed,document.getElementById("apptGender").value=d.gender,document.getElementById("apptWeight").value=d.weight)}if(e.target.closest("#btnPrevVet")&&(O=(O-1+re.length)%re.length,J()),e.target.closest("#btnNextVet")&&(O=(O+1)%re.length,J()),e.target.closest("#btnSelectVet")){const c=re[O];document.getElementById("apptVetIdHidden").value=O,document.getElementById("apptSelectedVetName").value=c.name,J()}if(e.target.classList.contains("btn-add-cart")){V.push({name:e.target.getAttribute("data-name"),price:e.target.getAttribute("data-price"),img:e.target.getAttribute("data-img")}),ue();const c=e.target.innerText;e.target.innerText="✓ Added",e.target.style.background="#10b981",e.target.style.color="white",e.target.style.borderColor="#10b981",setTimeout(()=>{e.target.innerText=c,e.target.style.background="white",e.target.style.color="#4f46e5",e.target.style.borderColor="#4f46e5"},1e3)}if(e.target.classList.contains("btn-buy-now")){const d=e.target.closest(".shop-card").querySelector(".btn-add-cart");V.push({name:d.getAttribute("data-name"),price:d.getAttribute("data-price"),img:d.getAttribute("data-img")}),ue(),document.getElementById("cartModalOverlay").style.display="flex"}e.target.closest("#btnOpenCart")&&(document.getElementById("cartModalOverlay").style.display="flex"),e.target.closest(".mode-btn")&&(document.querySelectorAll(".mode-btn").forEach(c=>c.classList.remove("active")),e.target.closest(".mode-btn").classList.add("active"),N&&document.querySelector(`[data-pet="${N}"]`).click());const p=e.target.closest(".pet-select-card");if(p&&document.getElementById("vetBookingForm")){document.querySelectorAll(".pet-select-card").forEach(m=>m.classList.remove("selected")),p.classList.add("selected");const c=p.getAttribute("data-petid");document.getElementById("apptPetSelectorHidden").value=c;const d=y.find(m=>m.id.toString()===c);d&&(document.getElementById("apptOwner").value=T,document.getElementById("apptPetType").value=d.breed&&d.breed.toLowerCase().includes("cat")?"Cat":"Dog",document.getElementById("apptBreed").value=d.breed||"Unknown",document.getElementById("apptGender").value=d.gender||"Unknown")}const g=e.target.closest(".my-pet-card");if(g&&!document.getElementById("apptPetSelectorHidden")){document.querySelectorAll(".my-pet-card").forEach(d=>d.classList.remove("active")),g.classList.add("active"),N=g.getAttribute("data-petid"),y.find(d=>d.id.toString()===N),document.getElementById("datingContentLayout").style.display="none";const c=document.getElementById("instructionOverlay");c.style.display="flex",c.innerHTML='<div class="radar-container"><div class="radar-ring ring1"></div><div class="radar-ring ring2"></div><div class="radar-ring ring3"></div><div class="radar-center"><i class="fa-solid fa-satellite-dish"></i></div></div><p>Scanning database for verified matches...</p>',Promise.resolve().then(()=>({status:"success",candidates:y.filter(m=>m.owner!==T).map(m=>({id:m.id,name:m.name,breed:m.breed,gender:m.gender,age:m.age,photos:[m.img],imgs:[m.img],owner:m.owner||"PawUser",ownerName:m.owner||"PawUser",score:94,traits:m.personal_traits?m.personal_traits.split(",").map(I=>I.trim()):["Playful","Friendly"],desc:m.reason_for_adoption||"Looking for a friend!",verified:!0}))})).then(d=>{d.status==="success"&&d.candidates.length>0?(Q=d.candidates,U=0,setTimeout(()=>{De()},800)):c.innerHTML='<p style="color:#64748b; font-weight:bold; font-size:1.1rem;">No match candidates available right now. Invite friends or register more pets!</p>'})}const f=e.target.closest(".cand-thumb");if(f){const c=Q[U];Xe(c,f.getAttribute("data-index"))}const h=e.target.closest("#btnPassCandidate"),E=e.target.closest("#btnMatchCandidate"),B=e.target.closest("#btnTreatCandidate");if(h||E||B){const c=Q[U],d=y.find(I=>I.id.toString()===N);let m="pass";if(E&&(m="like"),B&&(m="super_like"),m==="super_like"&&Ce<=0){b("Out of Treats","You have used all your Super Treats for today! Come back tomorrow.",!0);return}Promise.resolve({status:"success",isMatch:!0}).then(I=>{if(I.status==="success"){if(m==="super_like"&&Ce--,ee.unshift(c),ee.length>5&&ee.pop(),m!=="pass"){M(m==="super_like"?"Sent a Super Treat to":"Sent a match request to",c.name,"fa-heart"),document.getElementById("matchImgLeft").src=d.img,document.getElementById("matchImgRight").src=c.imgs[0],document.getElementById("matchNameLeft").innerText=d.name,document.getElementById("matchNameRight").innerText=c.name;const _=I.isMatch?"✨ IT'S A MATCH! ✨":m==="super_like"?"Super Liked! 🦴 (Pending)":"Pending Owner Approval";document.getElementById("matchDateSpot").innerText="PawTrack Verified";const S=document.getElementById("matchIcebreaker");S.value=I.isMatch?"You matched! Let's plan a playdate!":"",S.placeholder=I.isMatch?"Type a message...":`Say hi to ${c.ownerName} while you wait...`,document.getElementById("btnContinueMatch").setAttribute("data-targetuser",c.ownerName),document.getElementById("matchOverlay").style.display="flex",activePairs.unshift({id:"p"+Date.now(),maleName:d.gender==="Male"?d.name:c.name,femaleName:d.gender==="Female"?d.name:c.name,maleImg:d.gender==="Male"?d.img:c.imgs[0],femaleImg:d.gender==="Female"?d.img:c.imgs[0],status:_,isApproved:I.isMatch,date:"Just Now",ownerUsername:c.ownerName})}U++,De()}else b("Error",I.message,!0)}).catch(I=>console.error("Swipe Error:",I))}e.target.closest("#btnOpenPrefs")&&(document.getElementById("prefModal").style.display="flex"),e.target.closest("#btnViewActivePairs")&&he(),e.target.closest("#btnBackToMatch")&&(v.innerHTML=ye(),N=null);const w=e.target.closest(".btn-accept-match"),C=e.target.closest(".btn-delete-pair");if(w||C){const c=w?"accept":"delete",d=(w||C).getAttribute("data-matchid");if(c==="delete"){const m=C.getAttribute("data-title")||"Cancel Request",I=C.getAttribute("data-msg")||"Are you sure you want to remove this match?";oe(m,I,()=>{ve=ve.filter(_=>_.id!==d),he()})}else{const m=ve.find(I=>I.id===d);m&&(m.status="approved"),he()}}}),document.getElementById("btnCloseCart").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnContinueShopping").addEventListener("click",()=>document.getElementById("cartModalOverlay").style.display="none"),document.getElementById("btnCheckout").addEventListener("click",()=>{if(V.length===0){b("Empty Cart","Your cart is empty! Please add some items first.",!0);return}M("Completed a Pet Shop checkout","","fa-bag-shopping"),V=[],ue(),document.getElementById("cartModalOverlay").style.display="none",b("Order Placed!","Order placed successfully! Thank you for shopping.",!1)}),document.getElementById("cartItemsContainer").addEventListener("click",e=>{const a=e.target.closest(".btn-remove-item");a&&(V.splice(parseInt(a.getAttribute("data-index")),1),ue())}),document.getElementById("btnContinueMatch").addEventListener("click",e=>{const a=e.target.getAttribute("data-targetuser"),t=document.getElementById("matchIcebreaker").value.trim();t&&a&&(A[a]||(A[a]=[]),A[a].push({text:t,type:"sent",time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}),M("Sent an Icebreaker message to",a,"fa-comment-dots")),document.getElementById("matchOverlay").style.display="none",he()}),document.getElementById("btnSavePrefs").addEventListener("click",()=>{document.getElementById("prefModal").style.display="none",N&&document.querySelector(`[data-pet="${N}"]`).click()}),document.getElementById("prefModal").addEventListener("click",e=>{e.target===document.getElementById("prefModal")&&(document.getElementById("prefModal").style.display="none")}),v.addEventListener("submit",e=>{if(e.target.id==="vetBookingForm"){e.preventDefault();const a=document.getElementById("apptPetSelectorHidden").value,t=document.getElementById("apptVetIdHidden").value;if(!a){b("Error","Please select a pet for the appointment by clicking their picture!",!0);return}if(!t){b("Error","Please choose a veterinarian by clicking 'Confirm' on the profile card!",!0);return}const i=document.querySelector(`.pet-select-card[data-petid="${a}"] span`).innerText,r=document.getElementById("apptSelectedVetName").value,n=y.find(s=>{var o;return((o=s.id)==null?void 0:o.toString())===(a==null?void 0:a.toString())});(async()=>{try{const s=await L.createDocument(k,qe,Y.unique(),{pet_name:i,vet_name:r,status:"Upcoming",time:document.getElementById("apptTime").value||"",date:document.getElementById("apptDate").value||"",user_id:$,img:(n==null?void 0:n.img)||""});D.unshift({...s,id:s.$id}),await M("Booked vet visit for",i,"fa-user-doctor"),b("Request Sent!","Appointment scheduled successfully! It is now Upcoming.",!1,()=>{const o=document.querySelector('[data-target="vet"]');o?o.click():(v.innerHTML=Ae(),J())})}catch(s){console.error("Appwrite Vet Booking Error:",s),b("Error",s.message,!0)}})()}if(e.target.id==="adoptionApplicationForm"){if(e.preventDefault(),!document.getElementById("adoptTerms").checked){b("Missing Requirement","You must agree to the terms and conditions.",!0);return}const a=document.getElementById("adoptPetId").value,t=document.getElementById("adoptPetName").value,i=y.find(r=>{var n;return((n=r.id)==null?void 0:n.toString())===(a==null?void 0:a.toString())});(async()=>{try{const r=await L.createDocument(k,K,Y.unique(),{pet_name:t,date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),status:"Pending Review",user_id:$,img:(i==null?void 0:i.img)||""});x.unshift({...r,id:r.$id}),await M("Applied to adopt",t,"fa-house-chimney-user"),b("Application Sent!","Successfully submitted! Status: PENDING REVIEW.",!1,()=>{const n=document.querySelector('[data-target="applications"]');n?n.click():v.innerHTML=ce()})}catch(r){console.error("Appwrite Adoption Application Error:",r),b("Error",r.message,!0)}})()}(e.target.id==="petRegistrationForm"||e.target.id==="registerPetForm")&&(e.preventDefault(),(async()=>{var l,u,p,g,f;const a=document.getElementById("petImageInput"),t=Oe||a&&a.files&&a.files[0];let i="";if(t)try{b("Uploading Photo","Saving pet photo to PawTrack Storage...",!1);const h=await Te.createFile(Pe,Y.unique(),t);i=Te.getFileView(Pe,h.$id).toString()}catch(h){console.error("Storage upload failed:",h)}i||(i="https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=600&q=80");const r=document.getElementById("regForAdoption"),n=r?r.checked:!0,s=new FormData(e.target),o={name:s.get("name")||((l=document.getElementById("regPetName"))==null?void 0:l.value)||"Unnamed Pet",breed:s.get("breed")||((u=document.getElementById("regBreed"))==null?void 0:u.value)||"Mixed Breed",gender:s.get("gender")||((p=document.getElementById("regGender"))==null?void 0:p.value)||"Male",age:s.get("age")||((g=document.getElementById("regAge"))==null?void 0:g.value)||"1 yr",status:n?"Available":"Private",health_status:s.get("health_status")||"Healthy / Vaccinated",owner:T,contact_number:s.get("contact_number")||ie||"0917-000-0000",personal_traits:s.get("personal_traits")||((f=document.getElementById("regDesc"))==null?void 0:f.value)||"Friendly",reason_for_adoption:s.get("reason_for_adoption")||(n?"Looking for a home":"Personal pet"),img:i};try{const h=await L.createDocument(k,X,Y.unique(),o),E={...h,id:h.$id};y.unshift(E),await M(n?"Listed pet for adoption":"Registered private pet",o.name,"fa-shield-cat"),b("Success!",n?"Pet Registered Successfully! Photo saved to PawTrack Storage and listed on the adoption board.":"Pet added to your personal roster! Photo saved to PawTrack Storage.",!1,()=>{if(n){const w=document.querySelector('[data-target="pets"]');w?w.click():v.innerHTML=de()}else{const w=document.querySelector('[data-target="home"]');w?w.click():pe()}})}catch(h){console.error("Appwrite Pet Registration Error:",h),b("Error","Could not register pet: "+h.message,!0)}})())}),v.addEventListener("reset",e=>{e.target.id==="vetBookingForm"&&(document.querySelectorAll(".pet-select-card").forEach(a=>a.classList.remove("selected")),document.getElementById("apptPetSelectorHidden")&&(document.getElementById("apptPetSelectorHidden").value=""),document.getElementById("apptVetIdHidden")&&(document.getElementById("apptVetIdHidden").value=""),setTimeout(()=>{J()},10))});const Je=document.getElementById("editProfileForm");Je&&Je.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("editFirstName").value,t=document.getElementById("editLastName").value,i=document.getElementById("editContact").value,r=`${a} ${t}`.trim();try{await ae.updatePrefs({...P,fullName:r,phone:i}),P.fullName=r,P.phone=i,ie=i;const n=document.getElementById("userNameDisplay");if(n){const s=P.username||r;n.innerText=s?"Welcome, "+s+"!":"Welcome!"}await M("Updated profile settings & information","","fa-user-pen"),document.getElementById("editProfileModal").style.display="none",b("Profile Saved","Profile updated successfully!",!1,()=>{be()})}catch(n){b("Error",n.message,!0)}}),v.addEventListener("input",e=>{e.target.id==="shopPriceFilter"&&(document.getElementById("shopPriceDisplay").innerText=`₱${e.target.value}`,Ze())}),v.addEventListener("change",e=>{e.target.id==="shopCategoryFilter"&&Ze()});function Ze(){const e=document.getElementById("shopCategoryFilter").value,a=parseFloat(document.getElementById("shopPriceFilter").value),t=Ge.filter(i=>{const r=e==="All Categories"||i.category===e,n=i.price<=a;return r&&n});document.getElementById("shopGridContainer").innerHTML=Ye(t)}async function be(){const e=document.getElementById("mainProfilePic");e&&(e.src=P.avatarUrl||"/resources/avatar/Avatar 1.jpg");const a=document.getElementById("btnChangeAvatar")||document.querySelector(".btn-change-photo"),t=document.getElementById("avatarFileInput");a&&t&&(a.onclick=()=>t.click(),t.onchange=async function(){if(this.files&&this.files[0])try{b("Uploading","Uploading avatar to PawTrack Storage...",!1);const S=await Te.createFile(Pe,Y.unique(),this.files[0]),te=Te.getFileView(Pe,S.$id).toString();await ae.updatePrefs({...P,avatarUrl:te}),P.avatarUrl=te,e&&(e.src=te),await M("Updated profile picture","Saved to PawTrack Storage","fa-camera"),b("Success","Profile photo uploaded to PawTrack Storage!")}catch(S){b("Error","Failed to upload photo: "+S.message,!0)}});const i=document.getElementById("profileFullName"),r=document.getElementById("profileUsername"),n=document.getElementById("profileEmail"),s=document.getElementById("profilePhone"),o=P.fullName||T,l=P.username||T;i&&(i.innerText=o),r&&(r.innerText="@"+l),n&&(n.innerText=ze),s&&(s.innerText=ie||"None");const[u="",...p]=(o||"").split(" "),g=p.join(" "),f=document.getElementById("editFirstName"),h=document.getElementById("editLastName"),E=document.getElementById("editContact"),B=document.getElementById("editEmail");f&&(f.value=u),h&&(h.value=g),E&&(E.value=ie||""),B&&(B.value=ze||"");const w=y.filter(S=>S.owner===T),C=x.filter(S=>S.status==="Approved").length,c=document.getElementById("countOwnedPets"),d=document.getElementById("countSuccessfulApps");c&&(c.innerText=w.length),d&&(d.innerText=C);const m=document.getElementById("privateRosterGrid"),I=document.getElementById("adoptionRosterGrid");m&&I&&(m.innerHTML="",I.innerHTML="",w.forEach(S=>{const te=`
                    <div class="roster-card">
                        <img src="${S.img}" class="roster-img">
                        <div class="roster-info"><h4>${S.name}</h4><p>${S.breed}</p></div>
                        <button class="btn-archive-pet" data-petid="${S.id}" title="Move to Bin" 
                ...
                `;S.status==="Private"?m.innerHTML+=te:I.innerHTML+=te})),await Ke();const _=document.getElementById("btnRefreshActivityLogs");_&&(_.onclick=async()=>{_.classList.add("spinning"),_.disabled=!0,await Ke(),setTimeout(()=>{_.classList.remove("spinning"),_.disabled=!1},450)})}async function Ke(){const e=document.getElementById("recentActivityLogs");if(e){e.innerHTML='<div class="activity-loading"><i class="fa-solid fa-circle-notch fa-spin"></i> Loading activity logs...</div>';try{const a=await L.listDocuments(k,gt,[Z.equal("user_id",$),Z.orderDesc("$createdAt"),Z.limit(20)]);e.innerHTML="";const t=a.documents;t&&t.length>0?t.forEach(i=>{const r=Pt(i.timestamp,i.$createdAt),n=Ct(r),s=Lt(i.action,i.icon),o=i.target?F(i.target):"",l=F(i.action||"Activity recorded");e.innerHTML+=`
            ...
            `}}}function Pt(e,a){if(e){const t=typeof e=="number"?e:parseInt(e,10);if(!isNaN(t)&&t>0)return t}if(a){const t=Date.parse(a);if(!isNaN(t)&&t>0)return t;const i=new Date(a).getTime();if(!isNaN(i)&&i>0)return i}return Date.now()}function F(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function Lt(e,a){const t=(e||"").toLowerCase();if(t.includes("log")||t.includes("auth")||t.includes("verified")||t.includes("session"))return{themeClass:"theme-emerald",icon:"fa-shield-halved",tagClass:"tag-emerald",tagLabel:"Security"};if(t.includes("bin")||t.includes("delet")||t.includes("dumpster")||t.includes("trash"))return{themeClass:"theme-rose",icon:"fa-trash-can",tagClass:"tag-rose",tagLabel:"Archive"};if(t.includes("pet")||t.includes("adopt")||t.includes("restor")||t.includes("breed"))return{themeClass:"theme-terracotta",icon:"fa-paw",tagClass:"tag-terracotta",tagLabel:"Pet Care"};if(t.includes("photo")||t.includes("avatar")||t.includes("profile")||t.includes("settings"))return{themeClass:"theme-honey",icon:"fa-user-pen",tagClass:"tag-honey",tagLabel:"Profile"};if(t.includes("shop")||t.includes("cart")||t.includes("item")||t.includes("buy"))return{themeClass:"theme-amber",icon:"fa-bag-shopping",tagClass:"tag-amber",tagLabel:"Shop"};let i=a||"fa-bell";return i==="fa-right-to-bracket"&&(i="fa-arrow-right-to-bracket"),{themeClass:"theme-sage",icon:i,tagClass:"tag-sage",tagLabel:"Activity"}}function Ct(e){if(!e||isNaN(e))return"Recently";const a=Date.now(),t=Math.max(0,a-e),i=60*1e3,r=60*i,n=24*r,s=new Date(e);if(isNaN(s.getTime()))return"Recently";const o=s.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});return t<2*i?"Just now":t<60*i?`${Math.floor(t/i)}m ago`:t<24*r?`${Math.floor(t/r)}h ago (${o})`:new Date(a-n).toDateString()===s.toDateString()?`Yesterday at ${o}`:s.toLocaleDateString(void 0,{month:"short",day:"numeric"})+` at ${o}`}const Qe=document.getElementById("btnLogout");Qe&&Qe.addEventListener("click",async()=>{try{await ae.deleteSession("current")}catch(e){console.warn(e)}window.location.href="/PawTrackLogin.html"});let G=null;Object.keys(A).length===0&&(A.PawTrackCommunity=[{text:"Welcome to PawTrack! Connect with pet lovers, adopters, and arrange playdates right here.",type:"received",time:"10:00 AM"}]);function et(){return`
        ...
        `}function tt(e=null){const a=document.getElementById("messengerInboxList"),t=document.getElementById("messengerChatPanel"),i=document.getElementById("btnPageNewChat"),r=document.getElementById("messengerSearchInput");if(!a||!t)return;function n(p=""){a.innerHTML="";const g=Object.keys(A).filter(f=>f.toLowerCase().includes(p.toLowerCase()));if(g.length===0){a.innerHTML=`
                    <div class="inbox-empty-state">
                        <i class="fa-solid fa-comments"></i>
                        <p>No conversations found.<br>Click "New Conversation" to start chatting!</p>
                    </div>
                `;return}g.forEach(f=>{const h=A[f]||[],E=h.length>0?h[h.length-1].text:"No messages yet",B=h.length>0?h[h.length-1].time:"",w=f===G?"active":"",C=document.createElement("div");C.className=`inbox-item ${w}`,C.setAttribute("data-user",f),C.innerHTML=`
                    ...
                    <div class="inbox-info">
                        ...
                        <p class="inbox-snippet">${F(E)}</p>
                    </div>
                `,C.onclick=()=>s(f),a.appendChild(C)})}function s(p){G=p,document.querySelectorAll(".inbox-item").forEach(w=>{w.classList.toggle("active",w.getAttribute("data-user")===p)});const g=A[p]||[];t.innerHTML=`
                ...
                <div class="chat-pane-messages" id="activeChatStream">
                    ${g.length===0?'<div class="empty-stream"><p>This is the start of your message history with @'+F(p)+"</p></div>":""}
                </div>

                <div class="chat-pane-input-bar">
                    <div class="chat-input-pill">
                        <input type="text" id="activeChatInput" placeholder="Write a message to @${F(p)}...">
                    </div>
                    <button class="btn-chat-send" id="btnActiveSend">
                        <i class="fa-solid fa-paper-plane"></i>
                    </button>
                </div>
            `;const f=document.getElementById("activeChatStream");g.forEach(w=>{o(f,w.text,w.type,w.time)}),f.scrollTop=f.scrollHeight;const h=document.getElementById("activeChatInput"),E=document.getElementById("btnActiveSend");function B(){const w=h.value.trim();if(!w||!G)return;const C=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});o(f,w,"sent",C),h.value="",f.scrollTop=f.scrollHeight,A[G]||(A[G]=[]),A[G].push({text:w,type:"sent",time:C}),n(r.value.trim()),G==="PawTrackCommunity"&&setTimeout(()=>{const c=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),d="We're glad to have you! Feel free to connect with pet owners across PawTrack.";A.PawTrackCommunity.push({text:d,type:"received",time:c}),G==="PawTrackCommunity"&&(o(f,d,"received",c),f.scrollTop=f.scrollHeight),n(r.value.trim())},1e3)}E.onclick=B,h.onkeypress=w=>{w.key==="Enter"&&B()},h.focus()}function o(p,g,f,h){const E=document.createElement("div");E.className=`stream-bubble-row row-${f}`,E.innerHTML=`
                <div class="stream-bubble bubble-${f}">
                    <div class="bubble-text">${F(g)}</div>
                    <div class="bubble-time">${F(h||"")}</div>
                </div>
            `,p.appendChild(E)}r.oninput=p=>{n(p.target.value.trim())},i.onclick=()=>{yt("Start Conversation","Enter the PawTrack username you'd like to message:",p=>{if(p&&p.trim()){const g=p.trim().replace("@","");A[g]||(A[g]=[]),n(),s(g)}})};const l=Object.keys(A),u=e||(l.length>0?l[0]:null);n(),u?s(u):t.innerHTML=`
            ...
            `}v.addEventListener("click",e=>{const a=e.target.closest(".btn-message-owner");if(a){const t=a.getAttribute("data-owner"),i=document.querySelector('.sidebar-nav .nav-btn[data-target="messages"]');i&&(W.forEach(r=>r.classList.remove("active")),i.classList.add("active")),v.scrollTo({top:0,behavior:"smooth"}),v.innerHTML=et(),tt(t)}}),v.addEventListener("input",e=>{if(e.target.id==="radarDistanceRange"){const a=document.getElementById("radarDistanceValue");a&&(a.innerText=`${e.target.value} km`)}}),v.addEventListener("click",async e=>{var a,t,i,r,n;if(e.target.closest("#btnSaveAllSettings")){const s=((a=document.getElementById("notifVet"))==null?void 0:a.checked)??!0,o=((t=document.getElementById("notifMatch"))==null?void 0:t.checked)??!0,l=((i=document.getElementById("notifAdoption"))==null?void 0:i.checked)??!0,u=((r=document.getElementById("prefIncognito"))==null?void 0:r.checked)??!1,p=((n=document.getElementById("radarDistanceRange"))==null?void 0:n.value)||"25";try{await ae.updatePrefs({...P,notifVet:s,notifMatch:o,notifAdoption:l,incognito:u,radarDistance:p}),P.radarDistance=p,P.incognito=u,await M("Updated notification & discovery preferences","","fa-sliders"),b("Settings Saved","Your notification, privacy, and discovery preferences have been updated successfully!")}catch(g){b("Error",g.message,!0)}}});const H=[{title:"Your Care Dashboard 🏠",desc:"Welcome to PawTrack! From your Home dashboard, track registered pet counts, see active adoption statuses, review upcoming vet reminders, and get daily outdoor walk weather tips.",icon:"fa-house",targetSelector:".sidebar-nav [data-target='home']",tab:"home",placement:"right"},{title:"Live Adoption Board 🐾",desc:"Browse companions looking for a loving home! When other users register a pet for adoption, it shows up here in real-time without reloading. Click 'Details' to view medical notes, or 'Adopt Now' to send a digital application.",icon:"fa-paw",targetSelector:".sidebar-nav [data-target='pets']",tab:"pets",placement:"right"},{title:"Register Your Own Pet 📸",desc:"Add your furry family member with photo uploads to PawTrack Cloud Storage. Choose whether your companion is a private pet for health tracking or listed publicly for adoption.",icon:"fa-plus-circle",targetSelector:"#btnRegisterPet",tab:null,placement:"bottom"},{title:"Pet Matchmaker & Playdates ❤️",desc:"Find compatible playmates or breeding matches based on breed, age, and location. Give a Treat to express interest, or pass to browse more verified neighborhood companions.",icon:"fa-heart",targetSelector:".sidebar-nav [data-target='breeding']",tab:"breeding",placement:"right"},{title:"Verified Vet Appointments 🩺",desc:"Book check-ups and medical visits with licensed veterinarians. Select your preferred clinic and time slot, and manage upcoming appointments in one place.",icon:"fa-user-doctor",targetSelector:".sidebar-nav [data-target='vet']",tab:"vet",placement:"right"},{title:"Real-Time Messages & Alerts 💬",desc:"Chat directly with pet owners, caregivers, and adopters. Real-time notification toasts keep you updated the moment an application is approved or a new pet is posted!",icon:"fa-comment-dots",targetSelector:"#navMessagesBtn",tab:"messages",placement:"right"}];let q=0,we=!1,ge=null;function Fe(e){ge&&ge!==e&&ge.classList.remove("tour-target-elevated"),e?(e.classList.add("tour-target-elevated"),ge=e):ge=null}function xe(e){const a=document.getElementById("pawtrackTourOverlay"),t=document.getElementById("tourSpotlightBox"),i=document.getElementById("tourCard");if(!a||!t||!i)return;let r=e.targetSelector?document.querySelector(e.targetSelector):null;if(!r||r.offsetParent===null){t.style.display="none",a.classList.add("no-target"),Fe(null),i.style.top="50%",i.style.left="50%",i.style.transform="translate(-50%, -50%)";return}a.classList.remove("no-target"),Fe(r),t.style.display="block";const n=r.getBoundingClientRect(),s=6,o=Math.max(0,n.top-s),l=Math.max(0,n.left-s),u=n.width+s*2,p=n.height+s*2;t.style.top=`${o}px`,t.style.left=`${l}px`,t.style.width=`${u}px`,t.style.height=`${p}px`;try{const w=window.getComputedStyle(r),C=parseFloat(w.borderRadius)||12;t.style.borderRadius=`${Math.max(10,C+4)}px`}catch{t.style.borderRadius="14px"}if(window.innerWidth<=768){i.style.top="",i.style.left="",i.style.transform="";return}i.style.transform="none";const g=390,f=i.offsetHeight||220,h=16;let E,B;e.placement==="right"?(B=n.right+h,E=Math.max(20,n.top+n.height/2-f/2),B+g>window.innerWidth-20&&(B=Math.max(20,n.left-g-h))):e.placement==="bottom"?(E=n.bottom+h,B=n.left+n.width/2-g/2,B+g>window.innerWidth-20&&(B=window.innerWidth-g-24),B<20&&(B=20),E+f>window.innerHeight-20&&(E=Math.max(20,n.top-f-h))):(E=Math.max(20,window.innerHeight/2-f/2),B=Math.max(20,window.innerWidth/2-g/2)),E=Math.min(Math.max(20,E),window.innerHeight-f-20),B=Math.min(Math.max(20,B),window.innerWidth-g-20),i.style.top=`${E}px`,i.style.left=`${B}px`}function He(e){if(e<0||e>=H.length){Ee();return}q=e;const a=H[e];if(a.tab){const u=document.querySelector(`.sidebar-nav [data-target="${a.tab}"]`);u&&!u.classList.contains("active")&&u.click()}const t=document.getElementById("tourStepBadge"),i=document.getElementById("tourStepTitle"),r=document.getElementById("tourStepDesc"),n=document.getElementById("tourIconBubble"),s=document.getElementById("tourDots"),o=document.getElementById("tourBtnPrev"),l=document.getElementById("tourBtnNext");t&&(t.innerText=`STEP ${e+1} OF ${H.length}`),i&&(i.innerText=a.title),r&&(r.innerText=a.desc),n&&(n.innerHTML=`<i class="fa-solid ${a.icon}"></i>`),s&&(s.innerHTML=H.map((u,p)=>`<div class="tour-dot ${p===e?"active":""}"></div>`).join("")),o&&(o.style.display=e>0?"inline-block":"none"),l&&(e===H.length-1?l.innerHTML='Finish Tour <i class="fa-solid fa-check"></i>':l.innerHTML='Next <i class="fa-solid fa-arrow-right"></i>'),setTimeout(()=>{xe(a)},60),setTimeout(()=>{xe(a)},220)}function _e(e=0){const a=document.getElementById("tourWelcomeModal");a&&(a.style.display="none");const t=document.getElementById("howItWorksModal");t&&(t.style.display="none");const i=document.getElementById("pawtrackTourOverlay");i&&(i.style.display="block",we=!0,He(e))}async function fe(){if($)try{localStorage.setItem("pawtrack_tour_completed_"+$,"true"),localStorage.removeItem("pawtrack_is_new_user_"+$),sessionStorage.removeItem("pawtrack_just_registered_user"),sessionStorage.removeItem("pawtrack_is_new_registration"),P.tour_completed=!0,P.is_new_user=!1,await ae.updatePrefs({...P,tour_completed:!0,is_new_user:!1})}catch(e){console.warn("Could not persist tour completion to Appwrite:",e)}}function Ee(e=!0){const a=document.getElementById("pawtrackTourOverlay");a&&(a.style.display="none"),we=!1,Fe(null),e&&fe()}function $t(){q<H.length-1?He(q+1):(Ee(!0),j("Tour Complete! 🌟","You're all set! Click 'Guide & Tour' anytime at the top of your screen to replay.","fa-circle-check",null,"app"))}function St(){q>0&&He(q-1)}function Be(){const e=document.getElementById("howItWorksModal");e&&(e.style.display="none")}function At(){try{if(!$)return;const e=P.is_new_user===!0||localStorage.getItem("pawtrack_is_new_user_"+$)==="true"||sessionStorage.getItem("pawtrack_just_registered_user")===$||sessionStorage.getItem("pawtrack_is_new_registration")==="true",a=P.tour_completed===!0||localStorage.getItem("pawtrack_tour_completed_"+$)==="true";if(e&&!a)setTimeout(()=>{const t=document.getElementById("tourWelcomeModal"),i=document.getElementById("welcomeTourHeading"),r=P.username||T;i&&r&&(i.innerText=`Welcome, ${r}! 👋`),t&&(t.style.display="flex"),fe()},1200);else{const t=document.getElementById("tourWelcomeModal");t&&(t.style.display="none")}}catch(e){console.warn("First-time tour check warning:",e)}}const at=document.getElementById("btnStartTour");at&&at.addEventListener("click",()=>{_e(0)});const it=document.getElementById("btnSidebarAiAssistant");it&&it.addEventListener("click",()=>{j("AI Assistant 🤖","This feature is still under development. Stay tuned for our AI-powered assistant!","fa-robot",null,"info")});const st=document.getElementById("btnWelcomeTourStart");st&&st.addEventListener("click",()=>{fe(),_e(0)});const nt=document.getElementById("btnWelcomeTourSkip");nt&&nt.addEventListener("click",()=>{const e=document.getElementById("tourWelcomeModal");e&&(e.style.display="none"),fe()});const ot=document.getElementById("tourBtnClose");ot&&ot.addEventListener("click",()=>Ee(!0));const rt=document.getElementById("tourBtnSkip");rt&&rt.addEventListener("click",()=>Ee(!0));const lt=document.getElementById("tourBtnNext");lt&&lt.addEventListener("click",$t);const dt=document.getElementById("tourBtnPrev");dt&&dt.addEventListener("click",St);const ct=document.getElementById("btnHiwClose");ct&&ct.addEventListener("click",Be);const pt=document.getElementById("btnHiwDismiss");pt&&pt.addEventListener("click",Be);const mt=document.getElementById("btnHiwStartInteractiveTour");mt&&mt.addEventListener("click",()=>{Be(),_e(0)});const Re=document.getElementById("howItWorksModal");Re&&Re.addEventListener("click",e=>{e.target===Re&&Be()});const Ie=document.getElementById("tourWelcomeModal");Ie&&Ie.addEventListener("click",e=>{e.target===Ie&&(Ie.style.display="none",fe())}),document.querySelectorAll(".hiw-tab-btn").forEach(e=>{e.addEventListener("click",function(){document.querySelectorAll(".hiw-tab-btn").forEach(i=>i.classList.remove("active")),document.querySelectorAll(".hiw-tab-content").forEach(i=>i.classList.remove("active")),this.classList.add("active");const a=this.getAttribute("data-tab"),t=document.getElementById(a);t&&t.classList.add("active")})}),window.addEventListener("resize",()=>{we&&H[q]&&xe(H[q])}),window.addEventListener("scroll",()=>{we&&H[q]&&xe(H[q])},!0);function Mt(){const e=document.querySelector(".sidebar"),a=document.getElementById("sidebarTogglePill"),t=document.getElementById("btnHeaderSidebarToggle"),i=document.getElementById("sidebarLogoWrapper");if(!e)return;localStorage.getItem("pawtrack_sidebar_collapsed")==="true"&&window.innerWidth>768&&(e.classList.add("collapsed"),a&&(a.title="Maximize sidebar",a.setAttribute("aria-label","Maximize sidebar")),t&&(t.title="Maximize sidebar"));function n(s=null){if(window.innerWidth<=768)return;const o=s!==null?s:!e.classList.contains("collapsed");e.classList.toggle("collapsed",o);try{localStorage.setItem("pawtrack_sidebar_collapsed",o?"true":"false")}catch{}a&&(a.title=o?"Maximize sidebar":"Minimize sidebar",a.setAttribute("aria-label",o?"Maximize sidebar":"Minimize sidebar")),t&&(t.title=o?"Maximize sidebar":"Minimize sidebar"),setTimeout(()=>{window.dispatchEvent(new Event("resize"))},310)}a&&a.addEventListener("click",s=>{s.stopPropagation(),n()}),t&&t.addEventListener("click",s=>{s.stopPropagation(),n()}),i&&i.addEventListener("click",()=>{e.classList.contains("collapsed")&&n(!1)})}Mt(),At()});
